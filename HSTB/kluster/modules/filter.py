...
class FilterManager:
    ...
    def __init__(self, fqpr=None, external_filter_directory: str = None):
        ...
        self.base_filter_directory = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "plugins", "filters"
        )

        self.filter_names = []  # ex: ['filter_by_angle']
        self.filter_descriptions = (
            {}
        )  # ex: {'filter_by_angle': 'Reject all soundings that are greater than maximum beam angle and less than minimum beam angle.  Only retain soundings within the given minimum/maximum beam angle envelope.'}
        self.filter_lookup = (
            {}
        )  # ex: {'filter_by_angle': <filter_by_angle.Filter at 0x28e1b6afa00>}
        self.reverse_filter_lookup = (
            {}
        )  # ex: {<filter_by_angle.Filter at 0x28e1b6afa00>: 'filter_by_angle'}
        self.filter_file = (
            {}
        )  # ex: {'filter_by_angle': "C:\\Pydro21_Dev\\noaa\\site-packages\\python38\\git_repos\\hstb_kluster\\HSTB\\kluster\\plugins\\filters\\filter_by_angle.py"}
    ...
    def initialize_filters(self):
        ...
        if self.external_filter_directory:
            if os.path.exists(self.external_filter_directory):
                ...
                print(
                    f"initialize_filters: skipping {self.external_filter_directory}, as {self.external_filter_directory} is a path to a folder that does not exist."
                )
            ...
            if os.path.exists(dirpath):
                for filtername in os.listdir(dirpath):
                    ...
                    if filterbase in self.filter_names:
                        print(
                            f"initialize_filters: skipping {filterpath}, as {filterbase} is already in the list of loaded filters"
                        )
                        continue
                    if os.path.splitext(filtername)[1] == ".py":
                        ...
                        if filtermodule is not None and filterclass is not None:
                            ...
                            print(
                                f"initialize_filters: skipping {filterpath}, Unable to load Filter class"
                            )
    ...
    def _get_filter_class(self, filepath: str):
        ...
        try:
            ...
            print(
                f"get_filter_class: Unable to find filter that is supposed to be at {filepath}"
            )
            return None, None
        try:
            ...
            print(
                f'get_filter_class: Found filter {filepath}, but did not find the required "Filter" class'
            )
            return None, None
...
class BaseFilter:
    ...
    def _run_algorithm(self, **kwargs):
        ...
        raise NotImplementedError(
            "BaseFilter: you must create a Filter class and implement this method"
        )
    ...
    def run(self, **kwargs):
        ...
        try:
            ...
            print(
                f"BaseFilter: expect new_status returned from filter to be 2 dimensional, got {[ns.shape for ns in self.new_status]}"
            )
    ...
    def save(self):
        if not isinstance(self.new_status, list) or not self.new_status:
            print(
                "BaseFilter: unable to save new sounding flags, new_status should be a list of arrays, one array for "
        ...
        if self._selected_index and not isinstance(self._selected_index, list):
            print(
                "BaseFilter: unable to save new sounding flags, the optional selected_index should be a list of arrays, one array for "
                "each sonar head (len(self.selected_index) must equal len(self.fqpr.multibeam.raw_ping)"
            )
            return
        if (
            self._selected_index
        ):  # save when you have a subset selected, such as when you are filtering Points View points
            for cnt, rp in enumerate(self.fqpr.multibeam.raw_ping):
                rp_detect = rp[
                    "detectioninfo"
                ].load()  # convert to numpy and load in memory
                rp_detect_values = rp_detect.values
                sel_index = self._selected_index[cnt].reshape(
                    rp_detect.shape
                )  # reshape selected index to the 2d array
                rp_detect_values[sel_index] = self.new_status[cnt][
                    sel_index
                ]  # overwrite all values matching selected index with new status
                rp_detect[
                    :
                ] = rp_detect_values  # write the change back to the xarray dataarray
                # save to disk
                self.fqpr.write(
                    "ping",
