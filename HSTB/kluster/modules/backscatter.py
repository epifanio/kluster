...
def generate_avg_corrector(
    corrected_bscatter: xr.DataArray,
    beam_angles_degrees: xr.DataArray,
    bin_size_degree: float = 1.0,
    ...
    meanvals = (
        corrected_bscatter.groupby_bins(beam_angles_degrees, bins, right=True)
        .mean()
        .values
    )
    ...
    lookup = {
        str(bins[idx]): str(meanvals[idx] - refval) for idx in range(len(meanvals))
    }
    return lookup
...
class BScatter:
    ...
    def fixed_gain(self):
        raise NotImplementedError(
            "Expected a sonar specific backscatter class to inherit this class to process"
        )
    ...
    def area_correction(self):
        ...
        area_pulse_limited = (
            ...
            * self.slant_range
            * (np.pi / 180)
        ) / (2 * np.sin(np.abs(self.beam_angle)))
    ...
    def return_settings(
        ...
        if transmission_loss_corrected:
            setts[
                "transmission_loss_corrector"
            ] = f"40 * log10(svcorr slant range) + (2 * {cls.absorption_description} * svcorr slant range)"
        else:
            setts["transmission_loss_corrector"] = "disabled by user"
        if area_corrected:
            setts[
                "area_beam_limited"
            ] = f"{cls.tx_beam_width_description} * {cls.rx_beam_width_description} * (svcorr slant range * PI/180)^2"
            setts[
                "area_pulse_limited"
            ] = f"(surface sound speed * {cls.pulse_length_description} * {cls.tx_beam_width_description} * svcorr slant range * PI/180) / (2 * sin(abs(beam angle)))"
            setts[
                "area_correction_added"
            ] = "10 * log10(minimum of (area_beam_limited, area_pulse_limited))"
        else:
    ...
    def plot_backscatter_components(self):
        if self.plot_backscatter:
            ...
            if os.path.isdir(self.plot_backscatter):
                bscat_fname = os.path.join(
                    self.plot_backscatter, "backscatter_firstping_sample.png"
                )
            elif os.path.isfile(self.plot_backscatter):
                bscat_fname = os.path.join(
                    os.path.splitext(self.plot_backscatter)[0] + "_sample.png"
                )
            else:
                raise ValueError(
                    f"plot_backscatter_components: Expected either a directory path, file path, or None, got plot_backscatter={self.plot_backscatter}"
                )
            ...
            for comp in self.plot_components.keys():
                if isinstance(self.plot_components[comp], (float, int)):
                    plt.axhline(
                        y=self.plot_components[comp], linestyle="dashed", label=comp
                    )
                else:
...
class S7kscatter(BScatter):
    ...
    def __init__(
        ...
        self.power_selection_db_re_1micropascal = float(
            self.runtime_parameters["power_selection_db_re_1micropascal"]
        )
    ...
    def tvg(self):
        ...
        """
        return 2 * (
            (self.spreading_loss_db * np.log10(self.slant_range)) + self.attenuation
        )
    ...
    def return_settings(
        ...
        if fixed_gain_corrected:
            setts[
                "fixed_gain_removed"
            ] = f"{cls.gain_selection_description} + {cls.power_selection_description}"
        else:
            setts["fixed_gain_removed"] = "disabled by user"
        if tvg_corrected:
            setts[
                "tvg_removed"
            ] = f"{cls.spreading_description} * log10(svcorr slant range) + (2 * {cls.absorption_description} * svcorr slant range)"
        else:
...
class Allscatter(BScatter):
    ...
    def __init__(
        ...
        tx_beam_width: float,
        rx_beam_width: float,
        near_normal_corrector: xr.DataArray,
        pulse_length: xr.DataArray,
        plot_backscatter: str = None,
    ):
        super().__init__(
            raw_intensity,
        ...
        #    chunk.
        self.runtime_parameters = runtime_parameters
        self.absorption_db_m = (
            float(self.runtime_parameters["AbsorptionCoefficent"]) / 1000
        )
        ...
        self.near_normal_corrector = (
            near_normal_corrector  # see near_normal_description
        )
    ...
    def tvg(self):
        ...
        """
        return (
            2 * ((20 * np.log10(self.slant_range)) + self.attenuation)
            - self.near_normal_corrector
        )
    ...
    def return_settings(
        ...
        if tvg_corrected:
            ...
            setts[
                "tvg_removed"
            ] = f"40 * log10(svcorr slant range) + (2 * {cls.absorption_description} * svcorr slant range) - (near_normal_corrector)"
        else:
            setts["tvg_removed"] = "disabled by user"
        return setts
...
def distrib_run_process_backscatter(worker_dat: list):
    ...
    if multibeam_extension == ".all":
        ...
        bclass = Kmallscatter(
            ...
            worker_dat[9],
            worker_dat[10],
            plot_backscatter=worker_dat[11],
        )
    else:
        raise NotImplementedError(
            f"distrib_run_process_backscatter: filetype {multibeam_extension} is not currently supported for backscatter processing"
        )
    pscatter = bclass.process(**backscatter_settings)
    return pscatter


def return_backscatter_settings(
    ...
    if multibeam_extension == ".s7k":
        setts = S7kscatter.return_settings(
            fixed_gain_corrected,
            tvg_corrected,
            transmission_loss_corrected,
            area_corrected,
        ...
        setts = Allscatter.return_settings(
            fixed_gain_corrected,
            tvg_corrected,
            transmission_loss_corrected,
            area_corrected,
        ...
        setts = Kmallscatter.return_settings(
            fixed_gain_corrected,
            tvg_corrected,
            transmission_loss_corrected,
            area_corrected,
        ...
        raise NotImplementedError(
            f"return_backscatter_settings: filetype {multibeam_extension} is not currently supported for backscatter processing"
        )
    return setts
