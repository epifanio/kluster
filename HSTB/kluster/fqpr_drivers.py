...
def _check_multibeam_file(multibeam_file: str):
    ...
    if fileext not in kluster_variables.supported_sonar:
        raise NotImplementedError(
            "fqpr_drivers: File ({}) is not a Kluster supported multibeam file ({})".format(
                multibeam_file, kluster_variables.supported_sonar
            )
        )
...
def _check_sbet_file(sbet_file: str):
    ...
    if fileext not in kluster_variables.supported_ppnav:
        raise NotImplementedError(
            "fqpr_drivers: File ({}) is not a Kluster supported post processed navigation (SBET) file ({})".format(
                sbet_file, kluster_variables.supported_ppnav
            )
        )
...
def _check_pos_file(pos_file: str):
    ...
    try:
        ...
        raise NotImplementedError(
            "fqpr_drivers: File ({}) is not a Kluster supported position (POS) file (.000 -> .999)".format(
                pos_file
            )
        )
...
def _check_export_log_file(log_file: str):
    ...
    if fileext not in kluster_variables.supported_ppnav_log:
        raise NotImplementedError(
            "fqpr_drivers: File ({}) is not a Kluster supported export log file ({})".format(
                log_file, kluster_variables.supported_ppnav_log
            )
        )
...
def _check_svp_file(svp_file: str):
    ...
    if fileext not in kluster_variables.supported_sv:
        raise NotImplementedError(
            "fqpr_drivers: File ({}) is not a Kluster supported sound velocity file ({})".format(
                svp_file, kluster_variables.supported_sv
            )
        )


def fast_read_multibeam_metadata(
    multibeam_file: str, gather_times: bool = True, gather_serialnumber: bool = True
):
    """
    ...
    if fileext == ".all":
        ...
        raise NotImplementedError(
            "fqpr_drivers: {} is supported by kluster, but not currently supported by fast_read_multibeam_metadata".format(
                multibeam_file
            )
        )
...
def return_xyz_from_multibeam(multibeam_file: str):
    ...
    if mbes_extension == ".all":
        ...
        raise NotImplementedError(
            "fqpr_drivers: {} is supported by kluster, but not currently supported by return_xyz_from_multibeam".format(
                multibeam_file
            )
        )
...
def _validate_sequential_read_attitude(recs: dict):
    ...
    try:
        ...
        raise ValueError(
            f'sequential_read: Unable to find required attitude records.  Required: {required_attitude}, Found: {list(recs["attitude"].keys())}'
        )
    try:
        assert all(
            [
                recs["attitude"][pms].size == recs["attitude"]["time"].size
                for pms in required_attitude
            ]
        )
    except AssertionError:
        raise ValueError(
            f'sequential_read: All attitude records must be of the same size. Records: {required_attitude}, Sizes: {[recs["attitude"][pms].size for pms in required_attitude]}'
        )
    try:
        ...
        raise ValueError(
            f"sequential_read: All attitude records must be of the required data type. Records: {required_attitude}, "
...
def _validate_sequential_read_installation(recs: dict):
    ...
    required_keys = [
        "sonar_model_number",
        "transducer_1_vertical_location",
        "transducer_1_along_location",
        "transducer_1_athwart_location",
        "transducer_1_heading_angle",
        "transducer_1_roll_angle",
        "transducer_1_pitch_angle",
        "position_1_time_delay",
        "position_1_vertical_location",
        "position_1_along_location",
        "position_1_athwart_location",
        "motion_sensor_1_time_delay",
        "motion_sensor_1_vertical_location",
        "motion_sensor_1_along_location",
        "motion_sensor_1_athwart_location",
        "motion_sensor_1_roll_angle",
        "motion_sensor_1_pitch_angle",
        "motion_sensor_1_heading_angle",
        "waterline_vertical_location",
        "active_position_system_number",
        "active_heading_sensor",
        "position_1_datum",
    ]
    try:
        assert all(
            [pms in recs["installation_params"] for pms in required_installation_params]
        )
    except AssertionError:
        raise ValueError(
            f'sequential_read: Unable to find required installation parameter records.  Required: {required_installation_params}, Found: {list(recs["installation_params"].keys())}'
        )
    try:
        assert (
            recs["installation_params"]["time"].size
            == recs["installation_params"]["installation_settings"].size
        )
    except AssertionError:
        raise ValueError(
            f'sequential_read: All installation parameter records must be of the same size. Records: {["time", "installation_settings"]}, Sizes: {[recs["installation_params"][pms].size for pms in ["time", "installation_settings"]]}'
        )
        ...
        for irec in recs["installation_params"]["installation_settings"]:
            for ky in required_keys:
                try:
                    ...
                    raise ValueError(
                        f"sequential_read: {ky} not found in installation parameters entry: {irec}"
                    )
...
def _validate_sequential_read_ping(recs: dict):
    ...
    try:
        ...
        raise ValueError(
            f'sequential_read: Unable to find required ping records.  Required: {required_ping}, Found: {list(recs["ping"].keys())}'
        )
    ...
    if recs["ping"]["time"].size != 0:
        try:
            assert all(
                [
                    recs["ping"][pms].shape == recs["ping"]["time"].shape
                    for pms in required_ping
                    if pms in kluster_variables.subset_variable_1d
                ]
            )
        except AssertionError:
            raise ValueError(
                f'sequential_read: All ping records must be of the same size. Records: {[rec for rec in required_ping if rec in kluster_variables.subset_variable_1d]}, Sizes: {[recs["ping"][pms].size for pms in [rec for rec in required_ping if rec in kluster_variables.subset_variable_1d]]}'
            )
        try:
            assert all(
                [
                    recs["ping"][pms].shape == recs["ping"]["frequency"].shape
                    for pms in required_ping
                    if pms in kluster_variables.subset_variable_2d
                ]
            )
        except AssertionError:
            raise ValueError(
                f'sequential_read: All ping records must be of the same size. Records: {[rec for rec in required_ping if rec in kluster_variables.subset_variable_2d]}, Sizes: {[recs["ping"][pms].size for pms in [rec for rec in required_ping if rec in kluster_variables.subset_variable_2d]]}'
            )
        if (
            "u2-u5" in required_ping_dtype
        ):  # this is a place holder to put in here to allow us to have a range of lengths for these variables
            for tmprec in ["mode", "modetwo", "yawpitchstab"]:
                try:
                    assert recs["ping"][tmprec].dtype in ["<U2", "<U3", "<U4", "<U5"]
                    required_ping_dtype[required_ping.index(tmprec)] = str(
                        recs["ping"][tmprec].dtype
                    )
                except AssertionError:
                    raise ValueError(
                        f"sequential_read: ping record {tmprec} must be of the required data type. "
                        f"Dtype: {recs['ping'][tmprec].dtype}, Required Dtype: {['<U2', '<U3', '<U4', '<U5']}"
                    )
        try:
            assert all(
                [
                    recs["ping"][pms].dtype == required_ping_dtype[cnt]
                    for cnt, pms in enumerate(required_ping)
                ]
            )
        except AssertionError:
            raise ValueError(
                f"sequential_read: All ping records must be of the required data type. Records: {required_ping}, "
        ...
        for prec, prectype in zip(optional_ping, optional_ping_dtype):
            if prec in recs["ping"]:
                try:
                    ...
                    raise ValueError(
                        f'sequential_read: expected {prec} record with dtype of {prectype}, found {recs["ping"][prec].dtype}'
                    )
...
def _validate_sequential_read_runtime(recs: dict):
    ...
    try:
        ...
        raise ValueError(
            f'sequential_read: Unable to find required runtime parameter records.  Required: {required_runtime_params}, Found: {list(recs["runtime_params"].keys())}'
        )
    try:
        assert all(
            [
                recs["runtime_params"][pms].size == recs["runtime_params"]["time"].size
                for pms in required_runtime_params
            ]
        )
    except AssertionError:
        raise ValueError(
            f'sequential_read: All runtime parameter records must be of the same size. Records: {required_runtime_params}, Sizes: {[recs["runtime_params"][pms].size for pms in required_runtime_params]}'
        )
    if (
        "u2-u5" in required_runtime_params_dtype
    ):  # this is a place holder to put in here to allow us to have a range of lengths for these variables
        for tmprec in ["mode", "modetwo", "yawpitchstab"]:
            try:
                assert recs["runtime_params"][tmprec].dtype in [
                    "<U2",
                    "<U3",
                    "<U4",
                    "<U5",
                ]
                required_runtime_params_dtype[
                    required_runtime_params.index(tmprec)
                ] = str(recs["runtime_params"][tmprec].dtype)
            except AssertionError:
                raise ValueError(
                    f"sequential_read: runtime parameters record {tmprec} must be of the required data type. "
                    f"Dtype: {recs['runtime_params'][tmprec].dtype}, Required Dtype: {['<U2', '<U3', '<U4', '<U5']}"
                )
    try:
        assert all(
            [
                recs["runtime_params"][pms].dtype == required_runtime_params_dtype[cnt]
                for cnt, pms in enumerate(required_runtime_params)
            ]
        )
    except AssertionError:
        raise ValueError(
            f"sequential_read: All runtime parameter records must be of the required data type. Records: {required_runtime_params}, "
...
def _validate_sequential_read_navigation(recs: dict):
    ...
    try:
        ...
        raise ValueError(
            f'sequential_read: Unable to find required navigation records.  Required: {required_navigation}, Found: {list(recs["navigation"].keys())}'
        )
    try:
        assert all(
            [
                recs["navigation"][pms].size == recs["navigation"]["time"].size
                for pms in required_navigation
            ]
        )
    except AssertionError:
        raise ValueError(
            f'sequential_read: All navigation records must be of the same size. Records: {required_navigation}, Sizes: {[recs["navigation"][pms].size for pms in required_navigation]}'
        )
    try:
        ...
        raise ValueError(
            f"sequential_read: All navigation parameter records must be of the required data type. Records: {required_navigation}, "
    ...
    if "altitude" in recs["navigation"]:
        try:
            ...
            raise ValueError(
                f'sequential_read: expected altitude record with dtype of "float32", found {recs["navigation"]["altitude"].dtype}'
            )
...
def _validate_sequential_read(recs: dict):
    ...
    try:
        ...
        raise ValueError(
            f"sequential_read: Unable to find all required categories in multibeam data.  Required: {required_categories}, Found: {list(recs.keys())}"
        )
...
def is_valid_multibeam_file(multibeam_file: str):
    ...
    if multibeam_extension == ".s7k":
        ...
        if sk.has_datagram([1012, 1013], 300) or sk.has_datagram(1016, 300):
            ...
            print(
                f"fqpr_drivers: {multibeam_file} - Reson s7k data must have either (1012 and 1013) or (1016) records, unable to find either"
            )
            sk.close()
            return False
    else:
...
def sequential_read_multibeam(
    ...
    if multibeam_extension == ".all":
        ...
        recs = rd.sequential_read_records(
            first_installation_rec=first_installation_rec,
            frequency_selection=kluster_variables.ek_frequency_selection,
            build_heave=kluster_variables.ek_build_heave,
        )
        rd.close()
    else:
        raise NotImplementedError(
            "fqpr_drivers: {} is supported by kluster, but not currently supported by sequential_read_multibeam".format(
                multibeam_file
            )
        )
...
def bscorr_generation(file_object, second_file_object):
    ...
    if isinstance(file_object, par3.AllRead):
        par3.build_BSCorr(
            file_object.infilename,
            second_file_object.infilename,
            show_fig=True,
            save_fig=True,
        )
    else:
...
def return_xarray_from_sbet(
    sbetfiles: list,
    smrmsgfiles: list = None,
    logfiles: list = None,
...
def return_xarray_from_posfiles(
    posfiles: list, weekstart_year: int, weekstart_week: int
):
    """
...
def return_offsets_from_posfile(posfile: str):
    ...
    try:
        ...
        data = {
            "tx_to_antenna_x": round(msg20[0][10], 3),
            "tx_to_antenna_y": round(msg20[0][11], 3),
            "tx_to_antenna_z": round(msg20[0][12], 3),
            "imu_h": round(msg20[0][21], 3),
            "imu_p": round(msg20[0][20], 3),
            "imu_r": round(msg20[0][19], 3),
            "imu_x": round(msg20[0][7], 3),
            "imu_y": round(msg20[0][8], 3),
            "imu_z": round(msg20[0][9], 3),
        }
        return data
    except KeyError:
        try:
...
def _xyz_from_allfile(filname: str):
    ...
    for i in range(num88):
        try:
            ...
            tms[i] = (
                rec88.time + rec78.tx_data.Delay[0]
            )  # match par sequential_read, ping time = timestamp + delay
...
def _xyz_from_kmallfile(filname: str):
    ...
    read_count = 0
    for offset, size, mtype in zip(
        km.Index["ByteOffset"], km.Index["MessageSize"], km.Index["MessageType"]
    ):
        ...
        if mtype == "b'#MRZ'":
            ...
            dpths[read_count, :] = np.array(dg["sounding"]["z_reRefPoint_m"]) - float(
                install["install_txt"]["transducer_1_vertical_location"]
            )
    ...
    if read_count != numpings:
        raise ValueError(
            "kmall index count for MRZ records does not match actual records read"
        )
    cntrsorted = np.argsort(
        cntrs
    )  # ideally this would do it, but we have to sort by prim/stbd arrays when cntr/times
