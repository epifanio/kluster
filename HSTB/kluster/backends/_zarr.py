...
class ZarrBackend(BaseBackend):
    ...
    def _get_zarr_path(self, dataset_name: str, sys_id: str = None):
        ...
        if dataset_name == "ping":
            if not sys_id:
                raise ValueError(
                    "Zarr Backend: No system id provided, cannot build ping path"
                )
            ...
            raise ValueError(
                "Zarr Backend: Not a valid dataset name: {}".format(dataset_name)
            )
    ...
    def _autodetermine_times(
        self, data: list, time_array: list = None, append_dim: str = "time"
    ):
        """
    ...
    def delete(self, dataset_name: str, variable_name: str, sys_id: str = None):
        ...
        if not os.path.exists(var_path):
            self.print(
                "Unable to remove variable {}, path does not exist: {}".format(
    ...
    def write(
        self,
        dataset_name: str,
        data: Union[list, xr.Dataset, Future],
        ...
        zarr_path = self._get_zarr_path(dataset_name, sys_id)
        data_indices, final_size, push_forward = self._get_zarr_indices(
            zarr_path, time_array, append_dim
        )
        chunks = self._get_chunk_sizes(dataset_name)
        fpths = distrib_zarr_write(
            zarr_path,
            data,
            attributes,
            chunks,
    ...
    def write_attributes(self, dataset_name: str, attributes: dict, sys_id: str = None):
        ...
        if zarr_path is not None:
            ...
            self.debug_print(
                "Writing attributes is disabled for in-memory processing", logging.INFO
            )
    ...
    def remove_attribute(self, dataset_name: str, attribute: str, sys_id: str = None):
        ...
        if zarr_path is not None:
            ...
            self.debug_print(
                "Removing attributes is disabled for in-memory processing", logging.INFO
            )
...
def _get_indices_dataset_exists(input_time_arrays: list, zarr_time: zarr.Array):
    ...
    for (
        ...
        input_time_arrays
    ):  # for each chunk of data that we are wanting to write, look at the times to see where it fits
        input_time_len = len(input_time)
        input_is_in_zarr = np.isin(
            input_time, zarr_time
        )  # where is there overlap with existing data
        if isinstance(input_time, xr.DataArray):
            input_time = input_time.values
        if (
            input_is_in_zarr.any()
        ):  # this input array is at least partly in this datastore already
            if (
                not input_is_in_zarr.all()
            ):  # this input array is only partly in this datastore
                starter_indices = np.full_like(
                    input_time, -1
                )  # -1 for values that are outside the existing values
                inside_indices = search_not_sorted(
                    zarr_time, input_time[input_is_in_zarr]
                )  # get the indices for overwriting where there is overlap
                starter_indices[input_is_in_zarr] = inside_indices
                count_outside = len(starter_indices) - len(
                    inside_indices
                )  # the number of values that do not overlap
                if (
                    starter_indices[-1] == -1
                ):  # this input_time contains times after the existing values
                    ...
                    starter_indices[~input_is_in_zarr] = np.arange(
                        max_inside_index + 1, max_inside_index + count_outside + 1
                    )
                    if (
                        input_time[-1] < max_zarr_time
                    ):  # data partially overlaps and is after existing data, but not at the end of the existing dataset
                        push_forward.append(
                            [max_inside_index + 1 + total_push, count_outside]
                        )
                    else:
                    ...
                    starter_indices[0] == -1
                ):  # this input_time contains times before the existing values
                    if input_time[0] < min_zarr_time:
                        ...
                        push_forward.append(
                            [min_inside_index + total_push, count_outside]
                        )
                    write_indices.append(starter_indices + total_push)
                    total_push += count_outside
                else:
                    raise NotImplementedError(
                        "_get_indices_dataset_exists: Found a gap in the overlap between the data provided and the existing dataset on disk"
                    )
            else:
                ...
                write_indices.append(
                    search_not_sorted(zarr_time, input_time) + total_push
                )
        else:  # zarr datastore exists, but this data is not in it.  Append to the existing datastore
            if (
                input_time[0] < min_zarr_time
            ):  # data is before existing data, have to push existing data up
                ...
                input_time[0] > max_zarr_time
            ):  # data is after existing data, just tack it on
                write_indices.append(
                    [
                        zarr_time_len + running_total + total_push,
                        zarr_time_len + input_time_len + running_total + total_push,
                    ]
                )
                ...
                write_indices.append(
                    [
                        next_value_index + total_push,
                        next_value_index + input_time_len + total_push,
                    ]
                )
...
def get_write_indices_zarr(output_pth: str, input_time_arrays: list, index_dim="time"):
    ...
    if not (np.diff(mintimes) > 0).all():  # input arrays are not time sorted
        raise NotImplementedError(
            "get_write_indices_zarr: input arrays are out of order in time"
        )
    push_forward = []
    total_push = 0
    if os.path.exists(output_pth):
        ...
        write_indices, push_forward, total_push = _get_indices_dataset_exists(
            input_time_arrays, zarr_time
        )
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                ...
                print(
                    "WARNING: attempted {} retries at {} second interval, unable to complete process".format(
                        retries, wait
                    )
                )
                return callabl(*args, **kwargs)
...
class ZarrWrite:
    ...
    def __init__(
        self,
        zarr_path: str,
        desired_chunk_shape: dict = None,
        append_dim: str = "time",
        expand_dim: str = "beam",
        float_no_data_value: float = np.nan,
        int_no_data_value: int = 999,
    ):
        """
        ...
        if self.zarr_path is not None:
            ...
            print(
                "WARNING: starting zarr_write with an empty rootgroup, writing to disk not supported"
            )
    ...
    def _attributes_only_unique_runtime(self, attrs: dict):
        ...
        try:
            ...
            curr_settings = [
                x for x in self.rootgroup.attrs.keys() if x[0:7] == "runtime"
            ]
    ...
    def _attributes_only_unique_settings(self, attrs: dict):
        ...
        try:
            ...
            curr_settings = [
                x for x in self.rootgroup.attrs.keys() if x[0:7] == "install"
            ]
            current_vals = [self.rootgroup.attrs[p] for p in curr_settings]
            for sett in new_settings:
                val = attrs[sett]
    ...
    def _check_fix_rootgroup_expand_dim(self, xarr: xr.Dataset):
        ...
        if (self.expand_dim in self.rootgroup) and (self.expand_dim in xarr):
            ...
            if last_expand < xarr[self.expand_dim].shape[0]:
                ...
                return (
                    False  # there is a chunk there, but it is of size equal to desired
                )
        else:
    ...
    def fix_rootgroup_expand_dim(self, xarr: xr.Dataset):
        ...
        for var in self.zarr_array_names:
            ...
            if var == self.expand_dim:
                ...
                if (
                    self.rootgroup[var].shape[1] == curr_expand_dim_size
                ):  # you found an array with a beam dimension
                    nodata_value = self._get_arr_nodatavalue(self.rootgroup[var].dtype)
                    newdat = self._inflate_expand_dim(
                        self.rootgroup[var],
                        xarr[self.expand_dim].shape[0],
                        nodata_value,
                    )
    ...
    def _inflate_expand_dim(
        self,
        input_arr: Union[np.array, zarr.Array, xr.DataArray],
        expand_dim_size: int,
        nodata: Union[int, float, str],
    ):
        """
        ...
        if input_arr.ndim == 3:
            appended_data = np.full(
                (
                    input_arr.shape[0],
                    expand_dim_size - input_arr.shape[1],
                    input_arr.shape[2],
                ),
                nodata,
            )
            ...
            appended_data = np.full(
                (input_arr.shape[0], expand_dim_size - input_arr.shape[1]), nodata
            )
    ...
    def _write_determine_shape(
        self, var: str, dims_of_arrays: dict, finalsize: int = None
    ):
        """
        ...
        if var in ["beam", "xyz"]:
            ...
            startingshp = tuple(
                finalsize if dims_of_arrays[var][1].index(x) == timaxis else x
                for x in dims_of_arrays[var][1]
            )
        return timaxis, timlength, startingshp

    def _push_existing_data_forward(
        self,
        variable_name: str,
        dims_of_arrays: dict,
        timaxis: int,
        ...
        for push_idx, push_amount in push_forward:
            ...
            push_amount_chunked = push_amount_chunked[
                ::-1
            ]  # we push starting at the end to not overwrite data as we push
            push_chunk_loc = starting_size
            for pushchunk in push_amount_chunked:
                data_range = slice(
                    push_chunk_loc - pushchunk + total_push, push_chunk_loc + total_push
                )
                data_chunk_idx = tuple(
                    data_range
                    if dims_of_arrays[variable_name][1].index(i) == timaxis
                    else slice(0, i)
                    for i in dims_of_arrays[variable_name][1]
                )
                final_location_range = slice(
                    push_chunk_loc - pushchunk + push_amount + total_push,
                    push_chunk_loc + push_amount + total_push,
                )
                loc_chunk_idx = tuple(
                    final_location_range
                    if dims_of_arrays[variable_name][1].index(i) == timaxis
                    else slice(0, i)
                    for i in dims_of_arrays[variable_name][1]
                )
                self.rootgroup[variable_name][loc_chunk_idx] = self.rootgroup[
                    variable_name
                ][data_chunk_idx]
                push_chunk_loc -= pushchunk
            empty_range = slice(push_idx, push_idx + push_amount)
            empty_chunk_idx = tuple(
                empty_range
                if dims_of_arrays[variable_name][1].index(i) == timaxis
                else slice(0, i)
                for i in dims_of_arrays[variable_name][1]
            )
            self.rootgroup[variable_name][empty_chunk_idx] = self.rootgroup[
                variable_name
            ].fill_value
            total_push += push_amount

    def _overwrite_existing_rootgroup(
        self,
        xarr_data: np.array,
        data_loc_copy: Union[list, np.ndarray],
        var_name: str,
        dims_of_arrays: dict,
        chunksize: tuple,
        timlength: int,
        ...
        chunk_idx = tuple(
            chunk_time_range if cnt == timaxis else slice(0, i)
            for cnt, i in enumerate(array_dims)
        )
        self.rootgroup[var_name][chunk_idx] = zarr.array(
            xarr_data, shape=dims_of_arrays[var_name][1], chunks=chunksize
        )

    def _write_existing_rootgroup(
        self,
        xarr: xr.Dataset,
        data_loc_copy: Union[list, np.ndarray],
        var_name: str,
        ...
        if startingshp is not None:
            ...
            if push_forward is not None:
                self._push_existing_data_forward(
                    var_name, dims_of_arrays, timaxis, push_forward, starting_size
                )

        if isinstance(data_loc_copy, list):  # [start index, end index]
            self._overwrite_existing_rootgroup(
                xarr_data,
                data_loc_copy,
                var_name,
                dims_of_arrays,
            ...
            contiguous_chunks = [
                list(g)
                for k, g in groupby(data_loc_copy, key=lambda i, j=count(): i - next(j))
            ]
            idx_start = 0
            for chnk in contiguous_chunks:
                ...
                self._overwrite_existing_rootgroup(
                    chnkdata, chnkidx, var_name, chnkdims, chunksize, timlength, timaxis
                )
                idx_start += len(chnk)
    ...
    def _write_new_dataset_rootgroup(
        ...
        # if self.zarr_path:
        #     sync = zarr.ProcessSynchronizer(self.zarr_path + '.sync')
        newarr = self.rootgroup.create_dataset(
            var_name,
            shape=dims_of_arrays[var_name][1],
            chunks=chunksize,
            dtype=xarr[var_name].dtype,
            synchronizer=sync,
    ...
    def write_to_zarr(
        ...
        for var in dims_of_arrays:
            ...
            timaxis, timlength, startingshp = self._write_determine_shape(
                var, dims_of_arrays, finalsize
            )
            ...
            if var in self.zarr_array_names:
                if (
                    finalsize is not None
                ):  # appending data, first write contains the final shape of the data
                    self._write_existing_rootgroup(
                        xarr,
                        data_loc_copy,
                        var,
                        dims_of_arrays,
                ...
                self._write_new_dataset_rootgroup(
                    xarr, var, dims_of_arrays, chunksize, startingshp
                )
...
def zarr_write(
    ...
    zarr_path = retry_call(
        zw.write_to_zarr,
        (xarr, attrs, dataloc),
        {"finalsize": finalsize, "push_forward": push_forward},
        exceptions=(PermissionError,),
    )
    return zarr_path


def distrib_zarr_write(
    zarr_path: str,
    xarrays: list,
    attributes: dict,
    chunk_sizes: dict,
    ...
    if skip_dask:  # run the zarr write process without submitting to a dask client
        ...
        if len(xarrays) > 1:
            for i in range(len(xarrays) - 1):
                ...
                if (
                    not write_in_parallel
                ):  # wait on each future, write one data chunk at a time
                    wait(futs)
            if (
                write_in_parallel
            ):  # don't wait on the futures until you append all of them
                # if show_progress:
                #     progress(futs, multi=False)
                wait(futs)
    return futs
            ...
            if att not in rootgroup.attrs:
                ...
                if isinstance(attrs[att], list):
                    try:
                        ...
                        print(
                            "Unable to append to {} with value {}".format(
                                att, attrs[att]
                            )
                        )
...
def _my_xarr_to_zarr_build_arraydimensions(xarr: xr.Dataset):
    ...
    for arr in arrays_in_xarr:
        if (
            xarr[arr].dims and xarr[arr].shape
        ):  # only return arrays that have dimensions/shape
