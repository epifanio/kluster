...
if __name__ == "__main__":  # run from command line
    ...
    subparsers = parser.add_subparsers(
        help="Available processing commands within Kluster currently",
        dest="kluster_function",
    )
    ...
    allprochelp += (
        "example (relying on default arguments): all_processing -f fileone.all\n"
    )
    ...
    allproc.add_argument(
        ...
        "--navigation_datum",
        required=False,
        help="for use with --navigation, if export log is not provided, this is the datum identifier of the imported navigation data ({})".format(
            kluster_variables.coordinate_systems
        ),
    )
    allproc.add_argument(
        "-ng",
    ...
    allproc.add_argument(
        ...
        required=False,
        nargs="?",
        const=kluster_variables.default_coordinate_system,
        default=kluster_variables.default_coordinate_system,
        help="a valid datum identifier that pyproj CRS will accept {}, default is {}".format(
            kluster_variables.coordinate_systems,
    ...
    allproc.add_argument(
        ...
        const="waterline",
        default="waterline",
        help="the vertical reference point, one of {}, default is waterline".format(
            kluster_variables.vertical_references
        ),
    )
    allproc.add_argument(
        "-cast",
    ...
    allproc.add_argument(
        ...
        help="the method used to select the cast that goes with each chunk of the dataset, one of {}, default {}".format(
            ...
            kluster_variables.cast_selection_methods,
        ),
    )
    ...
    intelproc.add_argument(
        ...
        const=True,
        default=False,
        help="If true, will use the epsg code provided to build the coordinate system",
    )
    intelproc.add_argument(
        "-epsg",
        "--epsg_code",
    ...
    intelservicehelp += (
        "example (relying on default arguments): intel_service -f data/directory/path\n"
    )
    intelservice = subparsers.add_parser("intel_service", help=intelservicehelp)
    intelservice.add_argument(
        "-f",
        "--folder",
        nargs="+",
        required=True,
    ...
    procnavproc.add_argument(
        "converted_data_folder", help="Path to converted kluster dataset"
    )
    procnavproc.add_argument(
        "-nf",
        "--navfiles",
        nargs="+",
        required=True,
        help="list of postprocessed navigation file paths (sbet) (.out, .sbet)",
    )
    procnavproc.add_argument(
        "-ef",
        "--errorfiles",
        nargs="+",
        required=False,
        help="list of postprocessed error file paths (smrmsg) (.out, .sbet)",
    )
    procnavproc.add_argument(
        "-lf",
        "--logfiles",
        nargs="+",
        required=False,
        help="list of export log file paths (export.log)",
    )
    procnavproc.add_argument(
        "-yr",
        "--gps_year",
    ...
    rawnavproc.add_argument(
        ...
        nargs="+",
        required=True,
        help="list of raw navigation file paths (POSMV) (.000)",
    )
    rawnavproc.add_argument(
        "-yr",
        "--gps_year",
    ...
    svpproc.add_argument(
        ...
        nargs="+",
        required=True,
        help="list of processed sound velocity file paths (Caris SVP) (.svp)",
    )

    processhelp = (
        "R|Process converted Kluster multibeam datasets with the options provided\n"
    )
    ...
    processproc.add_argument(
        ...
        const=True,
        default=True,
        help="If true, runs the Orientation process, the first step to processing in Kluster",
    )
    processproc.add_argument(
        "-bpv",
        "--run_beam_vector",
        ...
        const=True,
        default=True,
        help="If true, builds the corrected beam vectors, the second step to processing in Kluster",
    )
    processproc.add_argument(
        "-sv",
        "--run_sv_correct",
        ...
        const=True,
        default=True,
        help="If true, sound velocity corrects the beams, the third step to processing in Kluster",
    )
    processproc.add_argument(
        "-geo",
        "--run_georeference",
        ...
        const=True,
        default=True,
        help="If true, georeferences the sound velocity corrected beams, the fourth step to processing in Kluster",
    )
    processproc.add_argument(
        "-tpu",
        "--run_tpu",
    ...
    processproc.add_argument(
        ...
        const=True,
        default=True,
        help="If true, will use the coordinate system identifier provided to build the coordinate system",
    )
    processproc.add_argument(
        "-epsg",
        "--epsg_code",
    ...
    processproc.add_argument(
        ...
        const=kluster_variables.default_coordinate_system,
        default=kluster_variables.default_coordinate_system,
        help="If -co is True, will use this identifier to build the coordinate system, automatically picking a UTM zone, default is {}".format(
            kluster_variables.default_coordinate_system
        ),
    )
    processproc.add_argument(
        ...
        const="waterline",
        default="waterline",
        help="Will use this identifier to build the vertical reference, one of {}, default is waterline".format(
            kluster_variables.vertical_references
        ),
    )
    processproc.add_argument(
    ...
    gridhelp = (
        "R|Generate a new grid from the processed Kluster multibeam datasets provided\n"
    )
    ...
    gridproc.add_argument(
        "-df",
        "--dataset_folders",
        nargs="+",
        required=False,
        help="list of Kluster processed folders",
    )
    gridproc.add_argument(
        "-gt",
        "--grid_type",
        ...
        required=False,
        nargs="?",
        const="single_resolution",
        default="single_resolution",
        help="one of 'single_resolution', 'variable_resolution_tile'",
    )
    gridproc.add_argument(
        "-ts",
        "--tile_size",
        ...
        const=1024.0,
        default=1024.0,
        help="main tile size, the size in meters of the tiles within the grid, a larger tile size will improve performance, but size should be at most 1/2 the length/width of the survey area",
    )
    gridproc.add_argument(
        "-sts",
        "--subtile_size",
        ...
        const=128.0,
        default=128.0,
        help="sub tile size, only used for variable resolution, the size of the subtiles within the tiles, subtiles are the smallest unit within the grid that is single resolution",
    )
    gridproc.add_argument(
        "-gr",
        "--gridding_algorithm",
    ...
    validproc.add_argument(
        "multibeam_file", help="path to an existing multibeam file (*.all, *.kmall)"
    )
    validproc.add_argument(
        "-m",
        "--analysis_mode",
    ...
    if not args.kluster_function:
        ...
        if funcname in [
            ...
            if not reloaded_data:
                print(
                    "{}: unable to reload from {}, not a valid converted kluster dataset".format(
                        funcname, args.converted_data_folder
                    )
                )
                sys.exit()
        ...
        if funcname == "all_processing":
            ...
            intel_process(
                ...
                vdatum_directory=args.vdatum_directory,
                cast_selection_method=args.cast_selection_method,
                designated_surface=args.designated_surface,
            )
            ...
            import_processed_navigation(
                reloaded_data,
                navfiles=args.navfiles,
                errorfiles=args.errorfiles,
                logfiles=args.logfiles,
                weekstart_year=args.gps_year,
                weekstart_week=args.gps_week,
                override_datum=args.datum,
                max_gap_length=args.max_allowable_gap,
            ...
            process_multibeam(
                ...
                run_beam_vec=args.run_beam_vector,
                run_svcorr=args.run_sv_correct,
                run_georef=args.run_georeference,
                run_tpu=args.run_tpu,
                use_epsg=args.use_epsg,
                use_coord=args.use_coord,
                epsg=args.epsg_code,
                coord_system=args.coordinate_identifier,
                vert_ref=args.vertical_identifier,
                vdatum_directory=args.vdatum_directory,
            ...
            if not output_path:
                output_path = os.path.join(
                    os.path.dirname(args.dataset_folders[0]), "grid"
                )
            generate_new_surface(
                ...
                gridding_algorithm=args.gridding_algorithm,
                auto_resolution_mode=args.auto_resolution_mode,
                resolution=args.resolution,
                output_path=output_path,
                export_path=args.export_folder,
                export_format=args.export_format,
                export_z_positive_up=args.export_z_positive_up,
                export_resolution=args.export_resolution,
            )
        elif funcname == "validate":
            output_file = os.path.join(
                os.path.split(args.multibeam_file)[0], "validation_export.tif"
            )
            validation_against_xyz88(
                args.multibeam_file,
                analysis_mode=args.analysis_mode,
                numplots=args.number_of_pings,
                export=output_file,
            )
        else:
