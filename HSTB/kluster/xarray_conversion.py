...
from HSTB.kluster.fqpr_drivers import (
    sequential_read_multibeam,
    fast_read_multibeam_metadata,
    return_offsets_from_posfile,
    sonar_reference_point,
    par_sonar_translator,
    kmall_sonar_translator,
)
...
sonar_translator = {
    "ek60": [None, "tx", "rx", None],
    "ek80": [None, "tx", "rx", None],
    "em122": [None, "tx", "rx", None],
    "em302": [None, "tx", "rx", None],
    "em304": [None, "tx", "rx", None],
    "em710": [None, "tx", "rx", None],
    "em712": [None, "tx", "rx", None],
    ...
    "em3020_dual": [None, "txrx_port", "txrx_stbd", None],
    "me70": [None, "txrx", None, None],
    "7125": [None, "tx", "rx", None],
    "t20": [None, "tx", "rx", None],
    "t50": [None, "tx", "rx", None],
    "t51": [None, "tx", "rx", None],
}
...
install_parameter_modifier = {
    "em2040_dual_tx_rx": {
        "rx_port": {
            "0": {"x": 0.011, "y": 0.0, "z": -0.006},
            "1": {"x": 0.011, "y": 0.0, "z": -0.006},
            "2": {"x": 0.011, "y": 0.0, "z": -0.006},
        },
        "tx_port": {
            "0": {"x": 0.0, "y": -0.0554, "z": -0.012},
    ...
    "em2045": {
        "rx": {
            "0": {"x": -0.0455, "y": 0.0, "z": -0.006},
            "1": {"x": -0.0455, "y": 0.0, "z": -0.006},
        },
        "tx": {
            "0": {"x": 0.0038, "y": 0.040, "z": -0.006},
            "1": {"x": 0.0038, "y": 0.040, "z": -0.006},
        },
    },
    "em2040p": {
        "rx": {
            "0": {"x": 0.204, "y": 0.0, "z": -0.0315},
            "1": {"x": 0.204, "y": 0.0, "z": -0.0315},
            "2": {"x": 0.204, "y": 0.0, "z": -0.0315},
        },
        "tx": {
            "0": {"x": 0.002, "y": -0.1042, "z": -0.0149},
...
def _assign_reference_points(fileformat: str, finalraw: dict, finalatt: xr.Dataset):
    ...
    try:
        if "." + fileformat in kluster_variables.supported_sonar:
            finalatt.attrs["reference"] = {
                "heading": "reference point",
                "heave": "transmitter",
                "pitch": "reference point",
                "roll": "reference point",
            }
            finalatt.attrs["units"] = {
                "heading": "degrees (+ clockwise)",
                "heave": "meters (+ down)",
                "pitch": "degrees (+ bow up)",
                "roll": "degrees (+ port up)",
            }
            for systemid in finalraw:
                finalraw[systemid].attrs["kluster_convention"] = {
                    "x": "+ Forward",
                    "y": "+ Starboard",
                    "z": "+ Down",
                    "roll": "+ Port Up",
                ...
                finalraw[systemid].attrs[
                    "sonar_reference_point"
                ] = sonar_reference_point["." + fileformat]
                finalraw[systemid].attrs["reference"] = {
                    "beampointingangle": "receiver",
                    "delay": "None",
                    "frequency": "None",
                    "soundspeed": "None",
                    "tiltangle": "transmitter",
                    "reflectivity": "None",
                    "traveltime": "None",
                    "latitude": "reference point",
                    "longitude": "reference point",
                    "altitude": "reference point",
                }
                finalraw[systemid].attrs["units"] = {
                    "beampointingangle": "degrees",
                    "delay": "seconds",
                    "frequency": "hertz",
                    "soundspeed": "meters per second",
                    "tiltangle": "degrees",
                    "reflectivity": "decibels",
                    "traveltime": "seconds",
                    "latitude": "degrees",
                    "longitude": "degrees",
                    "altitude": "meters (+ up)",
            ...
            raise ValueError(
                'Did not recognize format "{}" during xarray conversion'.format(
                    fileformat
                )
            )
    except KeyError:
...
def simplify_soundvelocity_profile(profile: np.ndarray):
    ...
    if profile.shape[0] > kluster_variables.max_profile_length:
        ...
        if (
            profile[-1, 0] == 12000.0
        ):  # this is an added on value by Kongsberg, linspace with the original profile depths to not throw off the step size
            new_depths = np.linspace(
                profile[0, 0],
                profile[-2, 0],
                num=kluster_variables.max_profile_length - 1,
            )
            new_depths = np.concatenate([new_depths, [12000.0]])
            extended_cast = True
        else:
            new_depths = np.linspace(
                profile[0, 0], profile[-1, 0], num=kluster_variables.max_profile_length
            )
            extended_cast = False
...
def _sequential_to_xarray(rec: dict):
    ...
    for r in rec:
        if r not in [
            ...
            "profile",
            "format",
        ]:  # These are going to be added as attributes later
            if (
                r == "ping"
            ):  # R&A is the only datagram we use that requires splitting by serial#
                ids, msk = _build_serial_mask(
                    rec
                )  # get the identifiers and mask for each serial#
                ...
                for systemid in ids:
                    ...
                    for ky in rec[r]:
                        ...
                        if ky not in ["time", "serial_num"]:
                            if (
                                ky == "counter"
                            ):  # counter is 16bit in raw data, we want 64 to handle zero crossing
                                datadtype = np.int64
                            else:
                                datadtype = rec[r][ky].dtype
                            arr = np.array(
                                rec["ping"][ky][msk[idx]]
                            )  # that part of the record for the given sect_id
                            ...
                            if ky in kluster_variables.subset_variable_2d:
                                ...
                                try:
                                    recs_to_merge[r][systemid][ky] = xr.DataArray(
                                        arr.astype(datadtype),
                                        coords=[tim, beam_idx],
                                        dims=["time", "beam"],
                                    )
                                except:
                                    raise ValueError(
                                        f"_sequential_to_xarray: Found record {ky} with shape {arr.shape}, expected a two dimensional array"
                                    )
                            #  everything else isn't by beam, proceed normally
                                ...
                                try:
                                    ...
                                    raise ValueError(
                                        f"_sequential_to_xarray: Found record {ky} with shape {arr.shape}, expected a one dimensional array"
                                    )
                    try:
                        recs_to_merge[r][systemid] = recs_to_merge[r][systemid].sortby(
                            "time"
                        )
                ...
                for ky in rec[r]:
                    if ky in ["heading", "heave", "pitch", "roll", "altitude"]:
                        recs_to_merge[r][ky] = xr.DataArray(
                            np.float32(rec[r][ky]),
                            coords=[rec[r]["time"]],
                            dims=["time"],
                        )
                        ...
                        recs_to_merge[r][ky] = xr.DataArray(
                            rec[r][ky], coords=[rec[r]["time"]], dims=["time"]
                        )
                try:
    ...
    for systemid in recs_to_merge["ping"]:
        if "mode" not in recs_to_merge["ping"][systemid]:
            ...
            chk = True
            if "runtime_params" not in list(recs_to_merge.keys()) or (
                recs_to_merge["runtime_params"].time.shape[0] == 0
            ):
                chk = False
            if not chk:
                ...
                recs_to_merge["runtime_params"] = xr.Dataset(
                    data_vars={
                        "mode": (["time"], np.array([""])),
                    ...
                    coords={
                        "time": np.array(
                            [float(recs_to_merge["ping"][systemid].time[0])]
                        )
                    },
                )

            _, index = np.unique(
                recs_to_merge["runtime_params"]["time"], return_index=True
            )
            recs_to_merge["runtime_params"] = recs_to_merge["runtime_params"].isel(
                time=index
            )
            recs_to_merge["runtime_params"] = recs_to_merge[
                "runtime_params"
            ].reindex_like(recs_to_merge["ping"][systemid], method="nearest")
            recs_to_merge["ping"][systemid] = xr.merge(
                [recs_to_merge["ping"][systemid], recs_to_merge["runtime_params"]],
                join="inner",
            )
    ...
    for systemid in recs_to_merge["ping"]:
        interp_nav = finalnav.reindex_like(
            recs_to_merge["ping"][systemid],
            method="nearest",
            tolerance=kluster_variables.max_nav_tolerance,
        )
        recs_to_merge["ping"][systemid] = xr.merge(
            [recs_to_merge["ping"][systemid], interp_nav]
        )
        # build attributes for the navigation/attitude records
        recs_to_merge["ping"][systemid].attrs["min_lat"] = float(
            finalnav.latitude.min()
        )
        recs_to_merge["ping"][systemid].attrs["max_lat"] = float(
            finalnav.latitude.max()
        )
        recs_to_merge["ping"][systemid].attrs["min_lon"] = float(
            finalnav.longitude.min()
        )
        recs_to_merge["ping"][systemid].attrs["max_lon"] = float(
            finalnav.longitude.max()
        )
    ...
    if "profile" in rec:
        for t in rec["profile"]["time"]:
            ...
            profile = np.dstack(
                [rec["profile"]["depth"][idx][0], rec["profile"]["soundspeed"][idx][0]]
            )[0]
            profile = simplify_soundvelocity_profile(profile)
            for systemid in recs_to_merge["ping"]:
                cst_name = "profile_{}".format(int(t))
                attrs_name = "attributes_{}".format(int(t))
                recs_to_merge["ping"][systemid].attrs[cst_name] = json.dumps(
                    profile.tolist()
                )
                nearestnav = recs_to_merge["ping"][systemid].sel(
                    time=int(t), method="nearest"
                )
                castlocation = [float(nearestnav.latitude), float(nearestnav.longitude)]
                recs_to_merge["ping"][systemid].attrs[attrs_name] = json.dumps(
                    {"location": castlocation, "source": "multibeam"}
                )
    ...
    for systemid in recs_to_merge["ping"]:
        ...
        if "installation_params" in rec:
            recs_to_merge["ping"][systemid].attrs["system_serial_number"] = np.unique(
                rec["installation_params"]["serial_one"]
            )
            recs_to_merge["ping"][systemid].attrs[
                "secondary_system_serial_number"
            ] = np.unique(rec["installation_params"]["serial_two"])
            for t in rec["installation_params"]["time"]:
                idx = np.where(rec["installation_params"]["time"] == t)
                recs_to_merge["ping"][systemid].attrs[
                    "installsettings_{}".format(int(t))
                ] = json.dumps(
                    rec["installation_params"]["installation_settings"][idx][0]
                )
        if "runtime_params" in rec:
            for t in rec["runtime_params"]["time"]:
                idx = np.where(rec["runtime_params"]["time"] == t)
                if rec["runtime_params"]["runtime_settings"][idx][
                    0
                ]:  # this might be empty dict if we trimmed it in par3/kmall for being a duplicate
                    recs_to_merge["ping"][systemid].attrs[
                        "runtimesettings_{}".format(int(t))
                    ] = json.dumps(rec["runtime_params"]["runtime_settings"][idx][0])
...
def _drop_next_att_nav_blob(cur_xarr: xr.Dataset, next_xarr: xr.Dataset):
    ...
    if next_xarr is not None:
        blob_size = min(
            200, min(cur_xarr.time.size, cur_xarr.time.size)
...
def _correct_for_splits(cur_xarr: xr.Dataset, trim_the_array: bool):
    ...
    if trim_the_array:
        print(
            "remove first: {} to {}".format(
                len(cur_xarr.time), slice(1, len(cur_xarr.time))
            )
        )
...
def batch_read_configure_options():
    ...
    opts = {
        "ping": {
            "chunksize": kluster_variables.ping_chunk_size,
            "chunks": kluster_variables.ping_chunks,
        ...
        "attitude": {
            "chunksize": kluster_variables.attitude_chunk_size,
            "chunks": kluster_variables.att_chunks,
...
class BatchRead(ZarrBackend):
    ...
    def __init__(
        self,
        filfolder: Union[str, list] = None,
        dest: str = None,
        address: str = None,
        client: Client = None,
        minchunksize: int = 40000000,
        max_chunks: int = 20,
        filtype: str = "zarr",
    ...
    def read(self, build_offsets: bool = True):
        ...
        if final_pths is not None:
            ...
            if self.filtype == "netcdf":
                ...
                self.read_from_zarr_fils(
                    final_pths["ping"], final_pths["attitude"][0], self.logfile
                )
                if build_offsets:
                    self.build_offsets(
                        save_pths=final_pths["ping"]
                    )  # write offsets to ping rootgroup
                    self.build_additional_line_metadata(save_pths=final_pths["ping"])
        else:
            self.logger.error(
                "Unable to start/connect to the Dask distributed cluster."
            )
    ...
    def read_from_netcdf_fils(self, ping_pths: list, attitude_pths: list):
        ...
        **Currently some issues with open_mfdataset that I've not resolved.  Using it with the dask distributed
        ...
        self.raw_ping = xr.open_mfdataset(
            ping_pths, chunks={}, concat_dim="time", combine="nested"
        )
        self.raw_att = xr.open_mfdataset(
            attitude_pths, chunks={}, concat_dim="time", combine="nested"
        )
    ...
    def return_raw_navigation(self, start_time: float = None, end_time: float = None):
        ...
        if self.raw_ping[0]:
            drop_these = [
                dvar
                for dvar in list(self.raw_ping[0].keys())
                if dvar not in desired_vars
            ]
            subset_nav = self.raw_ping[0].drop_vars(drop_these)
            subset_nav.attrs = {
                ky: self.raw_ping[0].attrs[ky]
                for ky in keep_these_attributes
                if ky in self.raw_ping[0].attrs
            }
            rnav = slice_xarray_by_dim(
                subset_nav, "time", start_time=start_time, end_time=end_time
            )
            return rnav
        return None
    ...
    def _batch_read_file_setup(self):
        ...
        if type(self.filfolder) == list:
            ...
            raise ValueError(
                "Only directory or file path is supported: {}".format(self.filfolder)
            )

        if not fils:
            raise ValueError(
                "Directory provided, but no {} files found: {}".format(
                    kluster_variables.supported_sonar, self.filfolder
                )
            )

        if (
            self.dest is not None
        ):  # path was provided, lets make sure it is an empty folder
            ...
            if os.path.exists(converted_pth):
                converted_pth = os.path.join(
                    self.filfolder,
                    "converted_{}".format(datetime.now().strftime("%H%M%S")),
                )
    ...
    def _batch_read_chunk_generation(self, fils: list):
        ...
        chnks = []
        for f in fils:
            finalchunksize = determine_good_chunksize(
                f, self.convert_minchunksize, self.convert_maxchunks
            )
        ...
        if self.skip_dask:
            self.logger.info(
                "{} file(s), Using {} chunk(s)".format(len(fils), len(chnks_flat))
            )
        else:
            self.logger.info(
                "{} file(s), Using {} chunk(s) in parallel".format(
                    len(fils), len(chnks_flat)
                )
            )
        for fil in fils:
    ...
    def _gather_file_level_metadata(self, fils: list):
        ...
        for f in fils:
            ...
            mtype, start_end_times, _ = fast_read_multibeam_metadata(
                f, gather_times=True, gather_serialnumber=False
            )
            dat[filname] = start_end_times
        return dat

    def _batch_read_merge_blocks(
        self, input_xarrs: list, datatype: str, chunksize: int
    ):
        """
        ...
        if self.client is not None:
            xlens = self.client.gather(
                self.client.map(_return_xarray_timelength, input_xarrs)
            )
        else:
            xlens = [_return_xarray_timelength(ix) for ix in input_xarrs]
        balanced_data, totallength = _return_xarray_constant_blocks(
            xlens, input_xarrs, chunksize
        )
        self.logger.info(
            "Rebalancing {} total {} records across {} blocks of size {}".format(
                totallength, datatype, len(balanced_data), chunksize
            )
        )
        ...
        if self.client is not None:
            ...
            time_arrs = self.client.gather(
                self.client.map(_return_xarray_time, output_arrs)
            )
        else:
    ...
    def _batch_read_sequential(self, chnks_flat: list):
        ...
        if self.client is not None:
            ...
            notempty = self.client.gather(
                self.client.map(_is_not_empty_sequential, recfutures)
            )
        else:
        ...
        for cnt, isnotempty in enumerate(notempty):
            if not isnotempty:  # this is an empty chunk, no ping records
                print(
                    "No ping or attitude records found in {}, startbyte:{}, endbyte:{}".format(
                        chnks_flat[cnt][0], chnks_flat[cnt][1], chnks_flat[cnt][2]
                    )
                )
    ...
    def _batch_read_sort_futures_by_time(self, input_xarrs: list):
        ...
        if self.client is not None:
            mintims = self.client.gather(
                self.client.map(_return_xarray_mintime, input_xarrs)
            )
        else:
    ...
    def _batch_read_write(
        ...
        if output_mode == "netcdf":
            ...
            fpths = self._batch_read_write_zarr(
                datatype, opts, converted_pth, sysid=sysid
            )
    ...
    def _batch_read_write_netcdf(self, datatype: str, opts: dict, converted_pth: str):
        ...
        output_attributes = [opts[datatype]["final_attrs"]] * len(
            opts[datatype]["output_arrs"]
        )
        fname_idxs = [i for i in range(len(opts[datatype]["output_arrs"]))]
        if self.client is not None:
            fpths = self.client.map(
                xarr_to_netcdf,
                opts[datatype]["output_arrs"],
                output_pths,
                output_fnames,
            ...
            fpths = [
                xarr_to_netcdf(oa, op, of, oatt, fi)
                for oa, op, of, oatt, fi in zip(
                    opts[datatype]["output_arrs"],
                    output_pths,
                    output_fnames,
                    output_attributes,
    ...
    def _batch_read_write_zarr(
        self, datatype: str, opts: dict, converted_pth: str, sysid: str = None
    ):
        """
        ...
        _, fpths = self.write(
            datatype,
            opts[datatype]["output_arrs"],
            time_array=opts[datatype]["time_arrs"],
            attributes=opts[datatype]["final_attrs"],
            skip_dask=self.skip_dask,
            sys_id=sysid,
        )
        fpth = fpths[
            0
        ]  # Pick the first element, all are identical so it doesnt really matter
        return fpth
    ...
    def _batch_read_correct_block_boundaries(self, input_xarrs: list):
        ...
        if self.client is not None:
            trim_arr = self.client.map(
                _assess_need_for_split_correction, base_xarrfut, next_xarrfut
            )
            input_xarrs = self.client.map(
                _correct_for_splits, base_xarrfut, [trim_arr[-1]] + trim_arr[:-1]
            )
        else:
            trim_arr = [
                _assess_need_for_split_correction(bx, nx)
                for bx, nx in zip(base_xarrfut, next_xarrfut)
            ]
            input_xarrs = [
                _correct_for_splits(bx, sta)
                for bx, sta in zip(base_xarrfut, [trim_arr[-1]] + trim_arr[:-1])
            ]
    ...
    def _batch_read_drop_duplicate_blobs(self, input_xarrs: list):
        ...
        if self.client is not None:
            input_xarrs = self.client.map(
                _drop_next_att_nav_blob, base_xarrfut, next_xarrfut
            )
        else:
            input_xarrs = [
                _drop_next_att_nav_blob(bx, nx)
                for bx, nx in zip(base_xarrfut, next_xarrfut)
            ]
    ...
    def _batch_read_ping_specific_attribution(self, combattrs: dict):
        ...
        combattrs[
            "multibeam_files"
        ] = fil_start_end_times  # override with start/end time dict
        ...
        if "skip_to_georeferencing" in combattrs:
            ...
            os.path.splitext(self.fils[0])[1] in kluster_variables.supported_singlebeam
        ):
    ...
    def batch_read(self, output_mode: str = "zarr"):
        ...
        if output_mode not in ["zarr", "netcdf"]:
            msg = "Only zarr and netcdf modes are supported at this time: {}".format(
                output_mode
            )
        ...
        for datatype in ["ping", "attitude"]:
            if self.client is not None:
                input_xarrs = self.client.map(
                    _divide_xarray_futs, xarrfutures, [datatype] * len(xarrfutures)
                )
            else:
                input_xarrs = [
                    _divide_xarray_futs(xf, dxf)
                    for xf, dxf in zip(xarrfutures, [datatype] * len(xarrfutures))
                ]
            input_xarrs = self._batch_read_sort_futures_by_time(input_xarrs)
            if self.client is not None:
                finalattrs = self.client.gather(
                    self.client.map(gather_dataset_attributes, input_xarrs)
                )
            else:
            ...
            if datatype == "ping":
                ...
                if self.client is not None:
                    system_ids = self.client.gather(
                        self.client.map(_return_xarray_system_ids, input_xarrs)
                    )
                else:
                    system_ids = [_return_xarray_system_ids(ix) for ix in input_xarrs]
                totalsystems = sorted(
                    np.unique([s for system in system_ids for s in system])
                )
                for system in totalsystems:
                    self.logger.info("Operating on system identifier {}".format(system))
                    input_xarrs_by_system = self._batch_read_return_xarray_by_system(
                        input_xarrs, system
                    )
                    ...
                    if len(input_xarrs_by_system) > 1:
                        ...
                        input_xarrs_by_system = (
                            self._batch_read_correct_block_boundaries(
                                input_xarrs_by_system
                            )
                        )
                    (
                        opts[datatype]["output_arrs"],
                        opts[datatype]["time_arrs"],
                        opts[datatype]["chunksize"],
                        totallen,
                    ) = self._batch_read_merge_blocks(
                        input_xarrs_by_system, datatype, opts[datatype]["chunksize"]
                    )
                    del input_xarrs_by_system
                    finalpths[datatype].append(
                        self._batch_read_write(
                            "zarr", datatype, opts, self.converted_pth, sysid=system
                        )
                    )
                    del opts
            else:
                ...
                del input_xarrs
                finalpths[datatype].append(
                    self._batch_read_write("zarr", datatype, opts, self.converted_pth)
                )
                del opts
    ...
    def return_runtime_and_installation_settings_dicts(self):
        ...
        for sett in runtimesetts:
            runtimesettdict[sett.split("_")[1]] = json.loads(
                self.raw_ping[0].attrs[sett]
            )
    ...
    def _batch_read_return_xarray_by_system(self, input_xarrs: list, sysid: str):
        ...
        if self.client is not None:
            input_xarrs_by_sec = self.client.map(
                _divide_xarray_return_system, input_xarrs, [sysid] * len(input_xarrs)
            )
            empty_mask = self.client.gather(
                self.client.map(
                    _divide_xarray_indicate_empty_future, input_xarrs_by_sec
                )
            )
        else:
            input_xarrs_by_sec = [
                _divide_xarray_return_system(ix, sysix)
                for ix, sysix in zip(input_xarrs, [sysid] * len(input_xarrs))
            ]
            empty_mask = [
                _divide_xarray_indicate_empty_future(ix) for ix in input_xarrs_by_sec
            ]
        valid_input_xarrs = [
            in_xarr for cnt, in_xarr in enumerate(input_xarrs_by_sec) if empty_mask[cnt]
        ]
    ...
    def build_offsets(self, save_pths: str = None):
        ...
        (
            settdict,
            runtimesettdict,
        ) = self.return_runtime_and_installation_settings_dicts()
        ...
        if len(settdict) > 0:
            ...
            if len(snrmodels) > 1:
                self.logger.error(
                    "Found multiple sonars types in data provided: {}".format(snrmodels)
                )
            ...
            if snrmodels[0] not in sonar_translator:
                self.logger.error(
                    'Sonar model not understood "{}"'.format(snrmodels[0])
                )
                raise NotImplementedError(
                    'Sonar model not understood "{}"'.format(snrmodels[0])
                )
            active_system = np.unique(
                [settdict[x]["active_position_system_number"] for x in settdict]
            )
            if len(active_system) > 1:
                self.logger.error(
                    "Found the active positioning system changed across files, found: {}".format(
                        active_system
                    )
                )
                raise NotImplementedError(
            ...
            if len(input_datum) > 1:
                self.logger.error(
                    "Found the input position datum changed across files, found: {}".format(
                        input_datum
                    )
                )
                raise NotImplementedError(
            ...
            if float(mintime) > float(minactual):
                self.logger.warning(
                    "Installation Parameters minimum time: {}".format(mintime)
                )
                self.logger.warning(
                    "Actual data minimum time: {}".format(float(minactual))
                )
                self.logger.warning(
                    "First Installation Parameters does not cover the start of the dataset."
            ...
            self.xyzrph = build_xyzrph(
                settdict, runtimesettdict, self.sonartype, logger=self.logger
            )

            if save_pths is not None:
                for svpth in save_pths:  # write the new attributes to disk
                    my_xarr_add_attribute(
                        {
                            "input_datum": input_datum[0],
                            "xyzrph": self.xyzrph,
                            "sonartype": self.sonartype,
                        },
                        svpth,
                    )
            for (
                rp
            ) in self.raw_ping:  # set the currently loaded dataset attribution as well
    ...
    def build_additional_line_metadata(self, save_pths: str = None):
        ...
        for line_name, line_times in line_dict.items():
            if (
                len(line_times) == 2
            ):  # this line needs the additional line metadata, otherwise it must have been computed already
                line_time_start, line_time_end = line_times[0], line_times[1]
                try:
                    current_index = None
                    dstart = rp.interp(
                        time=np.array([max(line_time_start, rp.time.values[0])]),
                        method="nearest",
                        assume_sorted=True,
                    )
                    ...
                    while np.isnan(
                        start_position
                    ).any():  # first position is NaN, scroll through to find a good one
                        if not current_index:
                            current_index = np.where(rp.time.values == dstart.time)[0]
                        current_index += 1
                        dstart = rp.isel(time=current_index)
                        start_position = [
                            dstart.latitude.values,
                            dstart.longitude.values,
                        ]
                        count += 1
                        if count == 100:
                            raise ValueError(
                                "Found bad position in the first 100 pings!"
                            )
                except ValueError:
                    self.print(
                        f"Unable to determine start position for line {line_name}",
                        logging.ERROR,
                    )
                    continue
                try:
                    current_index = None
                    dend = rp.interp(
                        time=np.array([min(line_time_end, rp.time.values[-1])]),
                        method="nearest",
                        assume_sorted=True,
                    )
                    ...
                    while np.isnan(
                        ...
                        if count == 100:
                            raise ValueError(
                                "Found bad position in the last 100 pings!"
                            )
                except ValueError:
                    self.print(
                        f"Unable to determine end position for line {line_name}",
                        logging.ERROR,
                    )
                    continue
                line_az = self.raw_att.interp(
                    time=np.array(
                        [line_time_start + (line_time_end - line_time_start) / 2]
                    ),
                    method="nearest",
                    assume_sorted=True,
                ).heading.values
                line_nav = rp.sel(
                    time=slice(float(dstart.time.values), float(dend.time.values))
                )
                samp_idx = np.arange(
                    0, line_nav.time.size, 15
                ).tolist()  # downsample to every 15 pings
                if (
                    samp_idx[-1] != line_nav.time.size - 1
                ):  # ensuring you keep the last ping
                    samp_idx += [line_nav.time.size - 1]
                line_nav.isel(time=samp_idx)
                line_dist = np.nansum(
                    distance_between_coordinates(
                        line_nav.latitude[:-1],
                        line_nav.longitude[:-1],
                        line_nav.latitude[1:],
                        line_nav.longitude[1:],
                    )
                )
                line_dict[line_name] += [
                    ...
                    float(end_position[1]),
                    round(float(line_az), 3),
                    round(float(line_dist)),
                ]
    ...
    def return_tpu_parameters(self, timestamp: str):
        ...
        if self.xyzrph is None:
            raise ValueError(
                "You must run build_offsets first, no installation parameters found."
            )
    ...
    def _interp_nan_nearest(self, arr: xr.DataArray):
        ...
        if np.any(nanvals):
            if len(uniquevals) == 1:
                self.logger.info(
                    "_interp_nan_nearest: ({}) Applying simple fillna interp for array with one unique entry".format(
                        arr.name
                    )
                )
                ...
                self.logger.info(
                    "_interp_nan_nearest: ({}) Propagating previous value forward for array with multiple unique entries".format(
                        arr.name
                    )
                )
                arr = arr.ffill("time")
                ...
                if np.any(new_nanvals):
                    ...
                    self.logger.info(
                        "_interp_nan_nearest: ({}) Found isolated chunks without values...".format(
                            arr.name
                        )
                    )
                    for ct, chunk in enumerate(arr.chunks):
                        ...
                        if np.any(new_nanvals[start_idx:end_idx]):
                            if ct == 0:
                                self.logger.info(
                                    "_interp_nan_nearest: ({}) Filling chunk 0 with next values".format(
                                        arr.name
                                    )
                                )
                                goodvalsearch = np.argwhere(
                                    nanvals[start_idx : end_idx + 1] is False
                                )[-1][0]
                            else:
                                self.logger.info(
                                    "_interp_nan_nearest: ({}) Filling chunk {}} with previous values".format(
                                        arr.name, ct
                                    )
                                )
                                goodvalsearch = np.argwhere(
                                    nanvals[start_idx - 1 : end_idx] is False
                                )[-1][0]
                            lastgoodval = arr[goodvalsearch]
                            arr[start_idx:end_idx] = arr[start_idx:end_idx].fillna(
                                lastgoodval
                            )
        return arr
    ...
    def correct_for_counter_reset(self):
        ...
        for cnt, rp in enumerate(self.raw_ping):
            if (
                rp.counter.dtype == np.uint16
            ):  # expects 64bit signed to accomodate zero crossing
                rp["counter"] = rp.counter.astype(np.int64)
            zero_crossing = (
                np.where(np.diff(rp["counter"]) < 0)[0] + 1
            )  # find the negative spike (65535 to 0) of reset
        ...
        if unique_zero_crossings:
            self.logger.info(
                "Found {} ping counter resets, correcting...".format(
                    len(unique_zero_crossings)
                )
            )
    ...
    def is_dual_head(self):
        ...
        if self.xyzrph is None:
            self.build_offsets()
        if ("tx_port_x" in list(self.xyzrph.keys())) or (
            "rx_port_x" in list(self.xyzrph.keys())
        ):
            return True
        else:
            return False
    ...
    def return_tx_xyzrph(self, time_idx: Union[int, str, float]):
        ...
        if "tx_r" in self.xyzrph:
            corr_timestmp = str(
                int(
                    _closest_prior_key_value(list(self.xyzrph["tx_r"].keys()), time_idx)
                )
            )
            return {
                corr_timestmp: {
                    "tx_roll": float(self.xyzrph["tx_r"][corr_timestmp]),
            ...
            corr_timestmp = str(
                int(
                    _closest_prior_key_value(
                        list(self.xyzrph["tx_port_r"].keys()), time_idx
                    )
                )
            )
            return {
                corr_timestmp: {
                    "tx_port_roll": float(self.xyzrph["tx_port_r"][corr_timestmp]),
            ...
            raise NotImplementedError(
                "Sonartype not supported: {}".format(self.sonartype)
            )
    ...
    def return_rx_xyzrph(self, time_idx: Union[int, str, float]):
        ...
        if "rx_r" in self.xyzrph:
            corr_timestmp = str(
                int(
                    _closest_prior_key_value(list(self.xyzrph["rx_r"].keys()), time_idx)
                )
            )
            return {
                corr_timestmp: {
                    "rx_roll": float(self.xyzrph["rx_r"][corr_timestmp]),
            ...
            corr_timestmp = str(
                int(
                    _closest_prior_key_value(
                        list(self.xyzrph["rx_port_r"].keys()), time_idx
                    )
                )
            )
            return {
                corr_timestmp: {
                    "rx_port_roll": float(self.xyzrph["rx_port_r"][corr_timestmp]),
    ...
    def return_nearest_soundspeed_profile(self, time_idx: Union[int, str, float]):
        ...
        if len(profs) == 0:
            self.logger.error(
                "No settings attributes found, possibly no install params in multibeam files"
            )
            raise ValueError(
                "No settings attributes found, possibly no install params in multibeam files"
            )
    ...
    def return_all_profiles(self):
        ...
        if prof_keys:
            for prof in prof_keys:
                ...
                try:
                    matching_attribute = json.loads(
                        self.raw_ping[0].attrs["attributes_{}".format(tme)]
                    )
    ...
    def return_waterline(self, time_idx: Union[int, str, float]):
        ...
        if len(settrecs) == 0:
            self.logger.error(
                "No settings attributes found, possibly no installation parameters in source files"
            )
            raise ValueError(
                "No settings attributes found, possibly no installation parameters in source files"
            )
        ...
        return float(
            json.loads(self.raw_ping[0].attrs["settings_" + closest_tim])[
                "waterline_vertical_location"
            ]
        )
    ...
    def return_system_time_indexed_array(self, subset_time: list = None):
        ...
        for cnt, ra in enumerate(self.raw_ping):
            if (
                ra is None
            ):  # get here if we turn one of the heads off but just setting the dataset to None
            ...
            for tstmp in tstmps:
                ...
                tx_idx = np.logical_and(
                    ra.time >= float(newtstmp), ra.time <= float(maxtime)
                )

                if (
                    subset_time is not None
                ):  # only include times that fall within the subset ranges
                    final_tx_idx = np.zeros_like(ra.time, dtype=bool)
                    if (
                        type(subset_time[0]) is not list
                    ):  # fix for when user provides just a list of floats [start,end]
                    ...
                    for (
                        ...
                        subset_time
                    ):  # build the mask for each subset and add it to the final idx
                        start_msk = np.logical_and(
                            ra.time >= float(minimum_time),
                            ra.time <= float(maximum_time),
                        )
    ...
    def return_xyz_prefixes_for_systems(self):
        ...
        if "tx_r" in self.xyzrph and "rx_r" in self.xyzrph:
            ...
            print(
                "Warning: The dual tx / single rx configuration is not tested and could create issues with the Kluster datasets."
            )
        else:
        ...
        for ra in self.raw_ping:
            ...
            if len(leverarms) > 1:
                if systemident == str(ra.system_serial_number[0]):
                    ...
                    self.logger.error(
                        "Found serial number attribute not included in sector"
                    )
                    raise NotImplementedError(
                        "Found serial number attribute not included in sector"
                    )
    ...
    def return_prefix_for_rp(self):
        ...
        try:
            ...
            print(
                "WARNING: Unable to find the sonar_reference_point attribute that was added in Kluster 0.8.5, defaulting to Kongsberg convention"
            )
            refpt = [0, 0, 0]
        return refpt
    ...
    def return_utm_zone_number(self):
        ...
        zne = return_zone_from_min_max_long(
            self.raw_ping[0].min_lon, self.raw_ping[0].max_lon, self.raw_ping[0].min_lat
        )
        return zne


def determine_good_chunksize(
    fil: str, minchunksize: int = 40000000, max_chunks: int = 20
):
    """
    ...
    if filesize <= minchunksize:
        ...
        finalchunksize = int(
            minchunksize + (((min_chunks % 1) * minchunksize) / max_chunks)
        )
    else:
        ...
        finalchunksize = int(
            minchunksize + ((overflowchunks * minchunksize) / max_chunks)
        )

    return finalchunksize


def return_chunked_fil(
    fil: str, startoffset: int = 0, chunksize: int = 20 * 1024 * 1024
):
    """
    With given file, determine the best size of the chunk to read from it, given a minimum chunksize and a max
    number of chunks.
    ...
    filesize = os.path.getsize(fil)
    midchunks = [
        (t * chunksize + chunksize - startoffset)
        for t in range(int(filesize / chunksize))
    ]
    ...
    for chnk in chunks:
        if (
            chunks.index(chnk) < len(chunks) - 1
        ):  # list is a range, skip the last one as prev ended at the last index
...
def get_nearest_runtime(timestamp: str, runtime_settdict: dict):
    ...
    try:
        ...
        runtime_param = runtime_settdict[
            [tstmp for tstmp in runtime_settdict][nearest_idx]
        ]
    except:
        runtime_param = {}
...
def build_xyzrph(
    ...
    for tme in settdict:
        ...
        xyzrph[tme] = {}
        for val in [
            v for v in sonar_translator[sonartype] if v is not None
        ]:  # tx, rx, etc.
            ...
            if val.find("txrx") != -1:
                ...
                if val == "txrx_port":
                    ...
                    tx_ident = "tx"
                    rx_ident = "rx"
                xyzrph[tme][tx_ident + "_x"] = float(
                    settdict[tme]["transducer_{}_along_location".format(ky)]
                )
                xyzrph[tme][tx_ident + "_y"] = float(
                    settdict[tme]["transducer_{}_athwart_location".format(ky)]
                )
                xyzrph[tme][tx_ident + "_z"] = float(
                    settdict[tme]["transducer_{}_vertical_location".format(ky)]
                )
                xyzrph[tme][tx_ident + "_r"] = float(
                    settdict[tme]["transducer_{}_roll_angle".format(ky)]
                )
                xyzrph[tme][tx_ident + "_p"] = float(
                    settdict[tme]["transducer_{}_pitch_angle".format(ky)]
                )
                xyzrph[tme][tx_ident + "_h"] = float(
                    settdict[tme]["transducer_{}_heading_angle".format(ky)]
                )
                xyzrph[tme][rx_ident + "_r"] = float(
                    settdict[tme]["transducer_{}_roll_angle".format(ky)]
                )
                xyzrph[tme][rx_ident + "_p"] = float(
                    settdict[tme]["transducer_{}_pitch_angle".format(ky)]
                )
                xyzrph[tme][rx_ident + "_h"] = float(
                    settdict[tme]["transducer_{}_heading_angle".format(ky)]
                )
                try:  # kmall workflow, rx offset is tacked on to the trans1 record
                    xyzrph[tme][rx_ident + "_x"] = float(
                        settdict[tme]["transducer_{}_along_location".format(ky)]
                    ) + float(settdict[tme]["transducer_{}_rx_forward".format(ky)])
                    xyzrph[tme][rx_ident + "_y"] = float(
                        settdict[tme]["transducer_{}_athwart_location".format(ky)]
                    ) + float(settdict[tme]["transducer_{}_rx_starboard".format(ky)])
                    xyzrph[tme][rx_ident + "_z"] = float(
                        settdict[tme]["transducer_{}_vertical_location".format(ky)]
                ...
                try:  # kmall
                    xyzrph[tme][tx_ident + "_opening_angle"] = float(
                        settdict[tme]["transducer_{}_sounding_size_deg".format(ky)]
                    )
                    xyzrph[tme][rx_ident + "_opening_angle"] = float(
                        settdict[tme]["transducer_{}_sounding_size_deg".format(ky)]
                    )
                except KeyError:
                    try:  # .all workflow reading from runtime parameters
                        xyzrph[tme][tx_ident + "_opening_angle"] = float(
                            runtime_params["TransmitBeamWidth"]
                        )
                        xyzrph[tme][rx_ident + "_opening_angle"] = float(
                            runtime_params["ReceiveBeamWidth"]
                        )
                    except:
                        if logger:
                            logger.warning(
                                "build_xyzrph: Warning, unable to decode transducer beam width, using default value of {}".format(
                        ...
                        xyzrph[tme][
                            tx_ident + "_opening_angle"
                        ] = kluster_variables.default_beam_opening_angle
                        xyzrph[tme][
                            rx_ident + "_opening_angle"
                        ] = kluster_variables.default_beam_opening_angle
            else:
                xyzrph[tme][val + "_x"] = float(
                    settdict[tme]["transducer_{}_along_location".format(ky)]
                )
                xyzrph[tme][val + "_y"] = float(
                    settdict[tme]["transducer_{}_athwart_location".format(ky)]
                )
                xyzrph[tme][val + "_z"] = float(
                    settdict[tme]["transducer_{}_vertical_location".format(ky)]
                )
                xyzrph[tme][val + "_r"] = float(
                    settdict[tme]["transducer_{}_roll_angle".format(ky)]
                )
                xyzrph[tme][val + "_p"] = float(
                    settdict[tme]["transducer_{}_pitch_angle".format(ky)]
                )
                xyzrph[tme][val + "_h"] = float(
                    settdict[tme]["transducer_{}_heading_angle".format(ky)]
                )
                try:  # kmall
                    try:
                        xyzrph[tme][val + "_opening_angle"] = float(
                            settdict[tme]["transducer_{}_sounding_size_deg".format(ky)]
                        )
                        ...
                        try:
                            xyzrph[tme][val + "_opening_angle"] = float(
                                settdict[tme][
                                    "transducer_{}_sounding_size_deg".format(ky - 2)
                                ]
                            )
                            ...
                            for chkval in [
                                ...
                                if (
                                    "transducer_{}_sounding_size_deg".format(chkky)
                                    in settdict[tme]
                                ):
                                    xyzrph[tme][val + "_opening_angle"] = float(
                                        settdict[tme][
                                            "transducer_{}_sounding_size_deg".format(
                                                chkky
                                            )
                                        ]
                                    )
        ...
        if sonartype in install_parameter_modifier:
            for val in [
                v for v in install_parameter_modifier[sonartype] if v is not None
            ]:
                for sec in install_parameter_modifier[sonartype][val]:
                    xyzrph[tme][val + "_x_" + sec] = float(
                        install_parameter_modifier[sonartype][val][sec]["x"]
                    )
                    xyzrph[tme][val + "_y_" + sec] = float(
                        install_parameter_modifier[sonartype][val][sec]["y"]
                    )
                    xyzrph[tme][val + "_z_" + sec] = float(
                        install_parameter_modifier[sonartype][val][sec]["z"]
                    )
        ...
        pos_motion_ident = (
            pos_motion_ident[0] + "_sensor_" + pos_motion_ident[1]
        )  # 'motion_sensor_1'
        ...
        if "waterline_vertical_location" in settdict[tme]:
            xyzrph[tme]["waterline"] = float(
                settdict[tme]["waterline_vertical_location"]
            )
        ...
        )  # 1 sigma standard deviation for the heave data (meters)
        xyzrph[tme][
            "roll_sensor_error"
        ] = (
            kluster_variables.default_roll_sensor_error
        )  # 1 sigma standard deviation in the roll sensor (degrees)
        xyzrph[tme][
            "pitch_sensor_error"
        ] = (
            kluster_variables.default_pitch_sensor_error
        )  # 1 sigma standard deviation in the pitch sensor (degrees)
        xyzrph[tme][
            "heading_sensor_error"
        ] = (
        ...
        xyzrph[tme][
            ...
            kluster_variables.default_tx_to_antenna_x
        )  # 1 sigma standard deviation in your measurement of x lever arm (meters)
        xyzrph[tme][
            "tx_to_antenna_y"
        ] = (
            kluster_variables.default_tx_to_antenna_y
        )  # 1 sigma standard deviation in your measurement of y lever arm (meters)
        xyzrph[tme][
            "tx_to_antenna_z"
        ] = (
            kluster_variables.default_tx_to_antenna_z
        )  # 1 sigma standard deviation in your measurement of z lever arm (meters)
        xyzrph[tme][
            "x_offset_error"
        ] = (
            kluster_variables.default_x_offset_error
        )  # 1 sigma standard deviation in your measurement of x lever arm (meters)
        xyzrph[tme][
            "y_offset_error"
        ] = (
            kluster_variables.default_y_offset_error
        )  # 1 sigma standard deviation in your measurement of y lever arm (meters)
        xyzrph[tme][
            "z_offset_error"
        ] = (
            kluster_variables.default_z_offset_error
        )  # 1 sigma standard deviation in your measurement of z lever arm (meters)
        xyzrph[tme][
            "surface_sv_error"
        ] = (
            kluster_variables.default_surface_sv_error
        )  # 1 sigma standard deviation in surface sv sensor (meters/second)
        xyzrph[tme][
            "roll_patch_error"
        ] = (
            kluster_variables.default_roll_patch_error
        )  # 1 sigma standard deviation in your roll angle patch test procedure (degrees)
        xyzrph[tme][
            "pitch_patch_error"
        ] = (
            kluster_variables.default_pitch_patch_error
        )  # 1 sigma standard deviation in your roll angle patch test procedure (degrees)
        xyzrph[tme][
            "heading_patch_error"
        ] = (
            kluster_variables.default_heading_patch_error
        )  # 1 sigma standard deviation in your roll angle patch test procedure (degrees)
        xyzrph[tme][
            "latency_patch_error"
        ] = (
            kluster_variables.default_latency_patch_error
        )  # 1 sigma standard deviation in your latency calculation (seconds)
        xyzrph[tme][
            "timing_latency_error"
        ] = (
            kluster_variables.default_timing_latency_error
        )  # 1 sigma standard deviation of the timing accuracy of the system (seconds)
        xyzrph[tme][
            "separation_model_error"
        ] = (
            kluster_variables.default_separation_model_error
        )  # 1 sigma standard deivation in the sep model (tidal, ellipsoidal, etc) (meters)
        xyzrph[tme][
            "waterline_error"
        ] = (
            kluster_variables.default_waterline_error
        )  # 1 sigma standard deviation of the waterline (meters)
        xyzrph[tme][
            "vessel_speed_error"
        ] = (
            kluster_variables.default_vessel_speed_error
        )  # 1 sigma standard deviation of the vessel speed (meters/second)
        xyzrph[tme][
            "horizontal_positioning_error"
        ] = (
            kluster_variables.default_horizontal_positioning_error
        )  # 1 sigma standard deviation of the horizontal positioning (meters)
        xyzrph[tme][
            "vertical_positioning_error"
        ] = (
            kluster_variables.default_vertical_positioning_error
        )  # 1 sigma standard deviation of the vertical positioning (meters)
...
def sort_and_drop_duplicates(dset: xr.Dataset, dsetpath: str):
    ...
    if dset["time"].size != index.size:
        print(
            "Dataset {} contains duplicate times, forced to drop duplicates on reload".format(
                dsetpath
            )
        )
...
def return_xyzrph_from_mbes(mbesfil: str, logger: logging.Logger = None):
    ...
    recs = sequential_read_multibeam(
        mbesfil, start_pointer=0, end_pointer=0, first_installation_rec=True
    )
    try:
        settings_dict = {
            str(int(recs["installation_params"]["time"][0])): recs[
                "installation_params"
            ]["installation_settings"][0]
        }
        runtime_dict = {}
        snrmodels = np.unique(
            [settings_dict[x]["sonar_model_number"] for x in settings_dict]
        )
        if len(snrmodels) > 1:
            if logger:
                logger.error(
        ...
        if len(serialnum) > 1:
            if logger:
                logger.error(
                    "Found multiple sonar serial numbers in data provided: {}".format(
                        snrmodels
                    )
                )
            raise NotImplementedError(
        ...
        if logger:
            logger.error(
                "Unable to read from {}: data not found for installation records".format(
                    mbesfil
                )
            )
