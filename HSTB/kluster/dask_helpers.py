...
worker_temp_space = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "dask-worker-space"
)
...
def dask_find_or_start_client(
    ...
    try:
        ...
        if not multiprocessing:
            ...
            if (
                mem_total_gb > 24
            ):  # basic test to see if we have enough memory, using an approx necessary amount of memory
...
def get_max_cluster_allocated_memory(client: Client):
    ...
    mem_per_worker = [
        client.scheduler_info()["workers"][wrk]["memory_limit"] for wrk in worker_ids
    ]
...
def client_needs_restart(client: Client):
    ...
    worker_ids = list(client.scheduler_info()["workers"].keys())
    total_mem_limit = round(
        sum([worker_data[wrk]["memory_limit"] for wrk in worker_ids]) / 1073741824, 3
    )  # get it in GB
    total_mem_used = round(
        sum([worker_data[wrk]["metrics"]["memory"] for wrk in worker_ids]) / 1073741824,
        3,
    )
...
def determine_optimal_chunks(
    client: Client,
    beams_per_ping: float,
    safety_margin: float = 0.75,
    chunks_per_worker: int = 2,
    mem_per_beam: float = 0.000015,
):
    """
...
def split_array_by_number_of_workers(
    client: Client, dataarray: DataArray, max_len: int = None
):
    """
    ...
    if max_len is not None:
        if (
            len(dataarray) > max_len * numworkers
        ):  # here we apply max_len, but only if necessary based on the length of the array
            max_split_count = np.ceil(
                len(dataarray) / max_len
            )  # round up to ensure you are below max_len
