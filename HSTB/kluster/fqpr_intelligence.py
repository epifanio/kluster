...
from HSTB.kluster.fqpr_drivers import (
    ...
    fast_read_errorfile_metadata,
    read_pospac_export_log,
    is_sbet,
    is_smrmsg,
    read_soundvelocity_file,
)
...
class FqprIntel(LoggerClass):
    ...
    def set_settings(self, settings: dict):
        ...
        self.processing_settings.update(
            {ky: settings[ky] for ky in desired_keys if ky in settings}
        )
    ...
    def set_designated_surface(self, surf_name: str):
        ...
        if (
            surf_name in self.project.surface_instances
        ):  # you got the path to the surface instance right, it is a relative path
            self.set_settings(
                {
                    "designated_surface": self.project.absolute_path_from_relative(
                        surf_name
                    )
                }
            )
        elif (
            self.project.path_relative_to_project(surf_name)
            in self.project.surface_instances
        ):  # you gave an absolute path that is in the project
            self.set_settings({"designated_surface": surf_name})
        else:
            self.print_msg(
                f"Unable to add {surf_name} as new designated surface, surface not found in project, please add to project first",
                logging.ERROR,
            )
            return
    ...
    def start_folder_monitor(self, folderpath, is_recursive=True):
        ...
        if os.path.isdir(folderpath):
            ...
            self.monitors[folderpath] = monitor.DirectoryMonitor(
                folderpath, is_recursive
            )
            ...
            print(
                "Unable to start monitoring, path provided is not a valid directory: {}".format(
                    folderpath
                )
            )
    ...
    def add_file(self, infile: str, silent: bool = True):
        ...
        if filename in excluded_files:
            if not silent:
                self.print_msg(
                    "File is listed as an exluded file: {}".format(infile),
                    logging.ERROR,
                )
        elif fileext in supported_mbes:
            try:
                new_data, updated_type, rerun_mbes_file_match = self._add_to_intel(
                    gather_multibeam_info(infile), self.multibeam_intel, "multibeam"
                )
            except:
                self.print_msg(
                    f"Tried adding {infile} as a multibeam file based on file extension {fileext}, failed to add to intel module."
                )
        elif fileext in supported_svp:
            try:
                new_data, updated_type, rerun_svp_file_match = self._add_to_intel(
                    gather_svp_info(infile), self.svp_intel, "svp"
                )
            except:
                self.print_msg(
                    f"Tried adding {infile} as a sound velocity file based on file extension {fileext}, failed to add to intel module."
                )
        elif (
            fileext in supported_sbet
        ):  # sbet and smrmsg have the same file extension sometimes ('.out') depending on what the user has done
            if is_sbet(infile):
                try:
                    new_data, updated_type, rerun_nav_file_match = self._add_to_intel(
                        gather_navfile_info(infile), self.nav_intel, "navigation"
                    )
                except:
                    self.print_msg(
                        f"Tried adding {infile} as a post processed navigation file based on file extension {fileext}, failed to add to intel module."
                    )
            elif is_smrmsg(infile):
                try:
                    new_data, updated_type, rerun_nav_file_match = self._add_to_intel(
                        gather_naverrorfile_info(infile),
                        self.naverror_intel,
                        "naverror",
                    )
                except:
                    self.print_msg(
                        f"Tried adding {infile} as a post processed navigation error file based on file extension {fileext}, failed to add to intel module."
                    )
        elif fileext in supported_export_log:
            try:
                new_data, updated_type, rerun_nav_file_match = self._add_to_intel(
                    gather_exportlogfile_info(infile), self.navlog_intel, "navlog"
                )
            except:
                self.print_msg(
                    f"Tried adding {infile} as an export log file based on file extension {fileext}, failed to add to intel module."
                )
        else:
            if not silent:
                self.print_msg(
                    "File is not of a supported type: {}".format(infile), logging.ERROR
                )
        ...
        if new_data:
            if self.project.path is None:
                ...
                potential_project_file = os.path.join(
                    parent_dir, "kluster_project.json"
                )
    ...
    def _regenerate_multibeam_actions(self):
        ...
        self._buffered_multibeam_line_groups = deepcopy(
            self.multibeam_intel.line_groups
        )

        # remove actions that do not match any fqpr instances that are in the project
        curr_acts, cur_dests = self.action_container.update_action_from_list(
            "multibeam", list(self._buffered_multibeam_line_groups.keys())
        )
        ...
        for destination, line_list in self._buffered_multibeam_line_groups.items():
            if destination in cur_dests:
                ...
                if len(action) == 1:
                    settings = fqpr_actions.update_kwargs_for_multibeam(
                        destination,
                        line_list,
                        self.project.client,
                        self.general_settings,
                    ...
                    raise ValueError(
                        "Multibeam actions found with the same destinations, {}".format(
                            destination
                        )
                    )
            else:
                newaction = fqpr_actions.build_multibeam_action(
                    destination, line_list, self.project.client, self.general_settings
                )
    ...
    def _regenerate_nav_actions(self):
        ...
        self._buffered_naverror_matching_sbet = deepcopy(
            self.naverror_intel.matching_sbet
        )

        # remove actions that do not match any fqpr instances that are in the project
        curr_acts, cur_dests = self.action_container.update_action_from_list(
            "navigation", list(self._buffered_nav_groups.keys())
        )
            ...
            if destination in cur_dests:
                ...
                if len(action) == 1:
                    if not final_nav_files:
                        ...
                        settings = fqpr_actions.update_kwargs_for_navigation(
                            destination,
                            fqpr_instance,
                            final_nav_files,
                            error_files,
                    ...
                    raise ValueError(
                        "Navigation import actions found with the same destinations, {}".format(
                            destination
                        )
                    )
            else:
                newaction = fqpr_actions.build_nav_action(
                    destination, fqpr_instance, final_nav_files, error_files, log_files
                )
                self.action_container.add_action(newaction)
    ...
    def _regenerate_svp_actions(self):
        ...
        # remove actions that do not match any fqpr instances that are in the project
        curr_acts, cur_dests = self.action_container.update_action_from_list(
            "svp", list(self._buffered_svp_groups.keys())
        )
        ...
        for destination, svfiles in self._buffered_svp_groups.items():
            ...
            if destination in cur_dests:
                action = [a for a in curr_acts if a.output_destination == destination]
                if len(action) == 1:
                    settings = fqpr_actions.update_kwargs_for_svp(
                        destination, fqpr_instance, svfiles
                    )
                    self.action_container.update_action(action[0], **settings)
                elif len(action) > 1:
                    raise ValueError(
                        "Sound Velocity import actions found with the same destinations, {}".format(
                            destination
                        )
                    )
            else:
                newaction = fqpr_actions.build_svp_action(
                    destination, fqpr_instance, svfiles
                )
                self.action_container.add_action(newaction)

    def _update_offsets(
        self, fqpr_instance, vessel_file, keep_waterline_changes: bool = True
    ):
        """
        ...
        new_data = None
        if vessel_file:
            new_xyzrph = vessel_file.return_data(
                fqpr_instance.multibeam.raw_ping[0].system_identifier,
                int(fqpr_instance.calc_min_var("time")),
                int(fqpr_instance.calc_max_var("time")),
            )
            if not new_xyzrph:
                print(
                    "WARNING: Unable to find a vessel file entry for {}".format(
                        fqpr_instance.output_folder
                    )
                )
            if (
                new_xyzrph
            ):  # there is an applicable entry in the vessel file for this dataset
                ...
                (
                    ...
                    data_matches,
                    new_waterline,
                ) = compare_dict_data(new_xyzrph, existing_xyzrph)
                ...
                if not identical_offsets or not identical_angles or not identical_tpu:
                    if new_waterline:
                        ...
                        if len(list(new_xyzrph["waterline"].keys())) == 1:
                            ...
                            print(
                                "WARNING: Unable to update with new waterline value, found multiple timestamped entries covering this dataset"
                            )
            ...
            if new_data:
                ...
                system_identifier = fqpr_instance.multibeam.raw_ping[
                    0
                ].system_identifier
                source_identifier = os.path.split(fqpr_instance.output_folder)[1]
                data = convert_from_fqpr_xyzrph(
                    new_data, sonar_model, system_identifier, source_identifier
                )
    ...
    def _build_new_crs(self, fqpr_instance):
        ...
        err = ""
        if (
            "use_epsg" in self.processing_settings
        ):  # if someone setup the project with a default coord system
            ...
            # force coordinate match takes effect if they don't specify EPSG, we use the most common EPSG of the loaded days of data for all days
                ...
                for relative_path, fqpr_instance in self.project.fqpr_instances.items():
                    if "horizontal_crs" in fqpr_instance.multibeam.raw_ping[0].attrs:
                        try:
                            existing_epsg = int(
                                fqpr_instance.multibeam.raw_ping[0].horizontal_crs
                            )
                            epsgs.append(existing_epsg)
                        except:
                            self.print_msg(
                                "Unable to generate EPSG from {}".format(
                                    fqpr_instance.multibeam.raw_ping[0].horizontal_crs
                                ),
                                logging.WARNING,
                            )
                if epsgs:
                    ...
                    self.print_msg(
                        "Unable to generate EPSG for any currently loaded fqpr instances in this project",
                        logging.WARNING,
                    )
                if (
                    new_coord_system is None
                ):  # no valid coord systems in the project, have to auto pick this one
                    self.print_msg(
                        "Force coordinate system match was used, but no existing coordinate systems found, defaulting to auto utm.",
                        logging.WARNING,
                    )
                    new_coord_system, err = build_crs(
                        zone_num=fqpr_instance.multibeam.return_utm_zone_number(),
                        datum=self.processing_settings["coord_system"],
                    )
                else:
                    self.print_msg(
                        'Forcing all Converted data to use EPSG:{}, uncheck "Force all days to have the same Coordinate System" to disable this.'.format(
                            new_coord_system.to_epsg()
                        ),
                        logging.WARNING,
                    )
    ...
    def _regenerate_processing_actions(
        self, reprocess_fqpr: str = None, keep_waterline_changes: bool = True
    ):
        """
        ...
        if self.project:
            ...
            existing_actions = self.action_container.return_actions_by_type(
                "processing"
            )
            all_current_project_paths = [
                self.project.absolute_path_from_relative(pth)
                for pth in self.project.fqpr_instances
            ]
            for action in existing_actions:
                if (
                    action.action_type == "processing"
                    and action.output_destination not in all_current_project_paths
                ):
            ...
            for relative_path, fqpr_instance in self.project.fqpr_instances.items():
                ...
                new_coord_system, forced_coordinate_match = self._build_new_crs(
                    fqpr_instance
                )
                if (
                    "vert_ref" in self.processing_settings
                ):  # if someone setup the project with a default vert ref
                    new_vert_ref = self.processing_settings["vert_ref"]
                else:
                    new_vert_ref = None
                if (
                    "input_datum" in self.processing_settings
                ):  # someone setup the project to override the input datum
                    new_input_datum = self.processing_settings["input_datum"]
                else:
                    new_input_datum = None
                if (
                    "cast_selection_method" in self.processing_settings
                ):  # someone setup a new cast selection method
                    cast_selection_method = self.processing_settings[
                        "cast_selection_method"
                    ]
                else:
                    cast_selection_method = (
                        kluster_variables.default_cast_selection_method
                    )
                abs_path = self.project.absolute_path_from_relative(relative_path)
                action = [
                    a for a in existing_actions if a.output_destination == abs_path
                ]
                ...
                args, kwargs = fqpr_instance.return_next_action(
                    new_coordinate_system=new_coord_system,
                ...
                if (
                    len(action) == 1 and not action[0].is_running
                ):  # modify the existing processing action
                    if kwargs == {}:
                        self.action_container.remove_action(action[0])
                    else:
                        settings = fqpr_actions.update_kwargs_for_processing(
                            abs_path,
                            args,
                            kwargs,
                            self.processing_settings,
                    ...
                    if kwargs != {}:
                        newaction = fqpr_actions.build_processing_action(
                            abs_path,
                            args,
                            kwargs,
                            self.processing_settings,
    ...
    def _regenerate_gridding_actions(self):
        ...
        # remove actions that do not match any fqpr instances that are in the project
        curr_acts, cur_dests = self.action_container.update_action_from_list(
            "gridding", [self.designated_surface]
        )

        if self.designated_surface:
            try:
                if not self.project.path or (
                    self.project.path
                    and self.project.path_relative_to_project(self.designated_surface)
                    not in self.project.surface_instances
                ):
                    # self.print_msg(f'Designated surface {self.designated_surface} not currently loaded in project.', logging.WARNING)
                    return
            except:
                self.print_msg(
                    "Designated surface must be on the same drive letter as the project!",
                    logging.ERROR,
                )
                return
            ...
            ) = self.project.return_surface_containers(relpath_surf)
            ...
            for fq_path, fq_instance in self.project.fqpr_instances.items():
                if fq_instance.is_processed():
                    mfiles = list(
                        fq_instance.multibeam.raw_ping[0].multibeam_files.keys()
                    )
                    if fq_path in existing_container_names:
                        old_files = [
                            ec[:-1] if ec[-1] == "*" else ec
                            for ec in existing_container_names[fq_path]
                        ]
                        ...
                        if new_files:
                            ...
                            add_lines += [
                                [
                                    f
                                    for f in new_files
                                    if fq_instance.line_is_processed(f)
                                ]
                            ]
                    else:
                        add_fqpr += [fq_instance]
            ...
            if add_fqpr or remove_fqpr:
                if destination in cur_dests:
                    action = [
                        a for a in curr_acts if a.output_destination == destination
                    ]
                    if len(action) == 1:
                        settings = fqpr_actions.update_kwargs_for_surface(
                            destination,
                            surf,
                            add_fqpr,
                            add_lines,
                        ...
                        raise ValueError(
                            "Gridding actions found with the same destinations, {}".format(
                                destination
                            )
                        )
                else:
                    newaction = fqpr_actions.build_surface_action(
                        destination,
                        surf,
                        add_fqpr,
                        add_lines,
    ...
    def regenerate_actions(
        self, also_update_conversion: bool = False, reprocess_fqpr: str = None
    ):
        """
        ...
        self._regenerate_processing_actions(
            reprocess_fqpr=reprocess_fqpr,
            keep_waterline_changes=self.keep_waterline_changes,
        )
    ...
    def update_matches(self):
        ...
        if self.multibeam_intel.line_groups != self._buffered_multibeam_line_groups:
            self._regenerate_multibeam_actions()
        if (
            (self.naverror_intel.matching_sbet != self._buffered_naverror_matching_sbet)
            or (self.navlog_intel.matching_sbet != self._buffered_navlog_matching_sbet)
            or (self.nav_intel.nav_groups != self._buffered_nav_groups)
        ):
            self._regenerate_nav_actions()
    ...
    def _match_error_file_to_nav(self):
        ...
        for err_name in self.naverror_intel.file_path:
            ...
            err_time = [
                self.naverror_intel.weekly_seconds_start[err_path],
                self.naverror_intel.weekly_seconds_end[err_path],
            ]
            ...
            nav_times = [
                [
                    self.nav_intel.weekly_seconds_start[pth],
                    self.nav_intel.weekly_seconds_end[pth],
                ]
                for pth in nav_paths
            ]
    ...
    def match_multibeam_files_to_project(self):
        ...
        if not self.project:
            print(
                "FqprIntel: a project must be created before you can match multibeam files to project"
            )
        ...
        for mfilepath in sorted_mfilepath:
            ...
            if self.project:
                ...
                fqpr_path, fqpr_instance = self.project.get_fqpr_by_serial_number(
                    int(prim_serial), int(sec_serial), same_day_as=start_time
                )
                if (
                    fqpr_path
                ):  # add the file to the list of multibeam files that are to be converted to this fqpr
                    self.multibeam_intel.matching_fqpr[mfilepath] = fqpr_path
                    if (
                        mfilename
                        not in fqpr_instance.multibeam.raw_ping[0].multibeam_files
                    ):
                        if fqpr_path in self.multibeam_intel.line_groups:
                            self.multibeam_intel.line_groups[fqpr_path].append(
                                mfilepath
                            )
                        else:
    ...
    def match_navigation_files_to_project(self):
        ...
        if self.project.fqpr_instances:
            for navfilepath, navfilename in self.nav_intel.file_name.items():
                ...
                if (
                    errfile and logfile
                ):  # you need all three: sbet, error file, and log file
                    fqpr_match = []
                    already_imported = None
                    sbet_starttime_weekly = self.nav_intel.weekly_seconds_start[
                        navfilepath
                    ]
                    for relpath, fqpr_instance in self.project.fqpr_instances.items():
                        # skip navigation files that are already in this instance
                        if (
                            "nav_files" in fqpr_instance.multibeam.raw_ping[0].attrs
                            and navfilename
                            in fqpr_instance.multibeam.raw_ping[0].attrs["nav_files"]
                        ):
                            already_imported = relpath
                            break

                        starttime = fqpr_instance.multibeam.raw_ping[0].time.values[0]
                        starttime_weekly = datetime.utcfromtimestamp(
                            starttime
                        ) - datetime.strptime(
                            "1980-01-06 00:00:00", "%Y-%m-%d %H:%M:%S"
                        )
                        starttime_weekly = starttime_weekly.seconds + (
                            86400 * (starttime_weekly.days % 7)
                        )
                        starttime_daynum = np.floor(starttime_weekly / 86400)
                        sbet_starttime_weekly_daynum = np.floor(
                            sbet_starttime_weekly / 86400
                        )

                        serial_number = fqpr_instance.multibeam.raw_ping[
                            0
                        ].system_identifier
                        model_number = fqpr_instance.multibeam.raw_ping[0].sonartype

                        if (
                            sbet_starttime_weekly_daynum == starttime_daynum
                        ):  # sbet from same day of the week
                            fqpr_match += [relpath]
                    ...
                    if already_imported:
                        ...
                        unmatch_reason += (
                            "Files have already been imported in {}".format(
                                already_imported
                            )
                        )
                    elif fqpr_match:
                        ...
                        unmatch_reason += "- is of the same week day number as nav file (sbet from day number {})\n".format(
                            int(sbet_starttime_weekly / 86400)
                        )
                ...
                if unmatch_reason:
                    ...
                    for (
                        ...
                        self.nav_intel.nav_groups
                    ):  # have to go through and remove the nav file from all preexisting matches
            ...
            for navfilepath, navfilename in self.nav_intel.file_name.items():
                unmatch_reason = "Navigation file (SBET)\n\n"
                unmatch_reason += (
                    "Converted multibeam data must exist to match navigation files"
                )
                self.nav_intel.matching_fqpr[navfilepath] = ""
                self.nav_intel.unmatched_files[navfilepath] = unmatch_reason
                for (
        ...
        if self.project.fqpr_instances:
            for svpfilepath, svpfilename in self.svp_intel.file_name.items():
                ...
                for relpath, fqpr_instance in self.project.fqpr_instances.items():
                    ...
                    fqpr_cast_times = [
                        int(fqpr_casts[castname]["time"]) for castname in fqpr_casts
                    ]
                    file_cast_times = self.svp_intel.time_utc_seconds[
                        svpfilepath
                    ]  # list of cast times for each profile in file
                    if all(
                        int(cd) in fqpr_cast_times for cd in file_cast_times
                    ):  # if any of the profiles in the file are new, load them
                        continue
            ...
            for svpfilepath, svpfilename in self.svp_intel.file_name.items():
                unmatch_reason = "Sound Velocity Profile file (.svp)\n\n"
                unmatch_reason += (
                    "Converted multibeam data must exist to match sound velocity files"
                )
                self.svp_intel.unmatched_files[svpfilepath] = unmatch_reason
            ...
            if self.action_container.actions:
                ...
                if self.parent is not None:  # running from GUI
                    ...
                    if isinstance(output, Fqpr):  # if the output is fqpr data
                        ...
                        self.project.update_surface(
                            fq_surf.output_folder, fq_surf, relative_path=False
                        )
    ...
    def update_intel_for_action_results(self, action_type: str):
        ...
        ):  # generated a new fqpr instance, have to rematch to project
            self.match_multibeam_files_to_project()
            self.match_navigation_files_to_project()
            self.match_svp_files_to_project()
            also_update_conversion = (
                True  # conversion actions need to be updated as well
            )
...
class IntelModule(LoggerClass):
    ...
    def _initialize(self):
        ...
        self.time_added = (
            {}
        )  # {'C:\\data_dir\\fil.kmall': datetime.datetime(2020, 11, 19, 15, 35, 2, 44724, tzinfo=datetime.timezone.utc)}
        self.last_modified_time_utc = (
            {}
        )  # {'C:\\data_dir\\fil.kmall': datetime.datetime(2020, 9, 10, 13, 16, 54, 96522, tzinfo=datetime.timezone.utc)}
        self.created_time_utc = (
            {}
        )  # {'C:\\data_dir\\fil.kmall': datetime.datetime(2020, 11, 19, 15, 35, 1, 899690, tzinfo=datetime.timezone.utc)}
    ...
    def _check_files_same_size(self, attributes: OrderedDict):
        ...
        if new_file_size in list(self.file_size_kb.values()):
            ...
            for fpth in old_file_paths:
                if (
                    self.file_size_kb[fpth] == new_file_size
                    and os.path.split(fpth)[1] == new_file_name
                ):
                    return True
        return False
    ...
    def add_dict(self, attributes: OrderedDict):
        ...
        if "file_path" in list(attributes.keys()):
            ...
            if (
                norm_filepath not in self.file_paths
                and not self._check_files_same_size(attributes)
            ):
                ...
                for ky, val in attributes.items():
                    if ky != "file_path":  # we store file paths in a separate attribute
                        try:
                            ...
                            self.print_msg(
                                "{} is not an attribute of this module".format(ky),
                                logging.WARNING,
                            )
                self.print_msg(
                    "File {} added as {}".format(norm_filepath, attributes["type"])
                )
                return True
            else:
                self.print_msg(
                    "File already exists in Kluster Intelligence: {}".format(
            ...
            self.print_msg(
                "Input data dictionary does not have a file_path key, found {}".format(
    ...
    def remove_file(self, filepath: str):
        ...
        if norm_filepath in self.file_paths:
            ...
            for ky in vars(self):
                ...
                if (
                    ky == "added_data"
                ):  # added_data is a list of ordered dict for each entry
                    matching_data = [
                        dat for dat in attr if dat["file_path"] == norm_filepath
                    ]
                    if len(matching_data) > 1:
                        raise ValueError(
                            "IntelModule: found multiple added_data entries for one file"
                        )
                    else:
                        attr.remove(matching_data[0])
                elif isinstance(
                    attr, list
                ):  # all other lists we just try and remove the file path, see file_paths
...
class MultibeamModule(IntelModule):
    ...
    def _initialize(self):
        ...
        self.data_start_time_utc = (
            {}
        )  # {'C:\\data_dir\\fil.kmall': datetime.datetime(2020, 3, 17, 9, 48, 52, 577000, tzinfo=datetime.timezone.utc)}
        self.data_end_time_utc = (
            {}
        )  # {'C:\\data_dir\\fil.kmall': datetime.datetime(2020, 3, 17, 12, 55, 51, 577000, tzinfo=datetime.timezone.utc)}
        ...
        self.matching_fqpr = (
            {}
        )  # {'C:\\data_dir\\fil.kmall': 'C:\\data_dir\\converted\\em710_241'}
        self.line_groups = (
            {}
        )  # {'C:\\data_dir\\converted\\em710_241': ['C:\\data_dir\\fil.kmall', ...]}
    ...
    def file_path_sorted_by_time(self):
        ...
        if self.file_name:
            return [
                self.file_path[self.file_name[k]]
                for k in sorted(
                    self.data_start_time_utc, key=self.data_start_time_utc.get
                )
            ]
        else:
            return []
...
class NavigationModule(IntelModule):
    ...
    def _initialize(self):
        ...
        self.matching_fqpr = (
            {}
        )  # {'C:\\data_dir\\sbet.out': 'C:\\data_dir\\converted\\em710_241'}
        self.nav_groups = (
            {}
        )  # {'C:\\data_dir\\converted\\em710_241': ['C:\\data_dir\\sbet.out', ...]}
...
class NavErrorModule(IntelModule):
    ...
    def _initialize(self):
        ...
        self.matching_sbet = (
            {}
        )  # {'C:\\data_dir\\smrmsg.out': 'C:\\data_dir\\sbet.out'}
...
class NavExportLogModule(IntelModule):
    ...
    def _initialize(self):
        ...
        self.mission_date = (
            {}
        )  # {'C:\\data_dir\\export.log': datetime.datetime(2020, 3, 17, 0, 0)}
        self.datum = {}  # {'C:\\data_dir\\export.log': 'NAD83'}
        self.ellipsoid = {}  # {'C:\\data_dir\\export.log': 'GRS 1980'}
        self.matching_sbet = (
            {}
        )  # {'C:\\data_dir\\export.log': 'C:\\data_dir\\sbet.out'}
        ...
        self.input_sbet_file = (
            {}
        )  # {'C:\\data_dir\\export.log': 'sbet_2020_2097_S222_B.out'}
        ...
        self.exported_sbet_file = (
            {}
        )  # {'C:\\data_dir\\export.log': 'export_2020_2097_S222_B.out'}
...
class SvpModule(IntelModule):
    ...
    def _initialize(self):
        ...
        self.profiles = (
            {}
        )  # {'C:\\data_dir\\2020_077_053406.svp': [[(0.283, 1475.243261), (1.283, 1473.0167), (2.283, 1471.911006),...]]}
        ...
        self.time_utc = (
            {}
        )  # {'C:\\data_dir\\2020_077_053406.svp': ['2020-077', '2020-077']}
        self.time_utc_seconds = (
            {}
        )  # {'C:\\data_dir\\2020_077_053406.svp': ['1584426535', '1584426987']}
        self.latitude = (
            {}
        )  # {'C:\\data_dir\\2020_077_053406.svp': [37.24027778, 37.2625]}
        self.longitude = (
            {}
        )  # {'C:\\data_dir\\2020_077_053406.svp': [-76.085, -76.07583333]}
        ...
        self.svp_groups = (
            {}
        )  # {'C:\\data_dir\\converted\\em710_241': ['C:\\data_dir\\svdata.svp', ...]}
    ...
    info_data = OrderedDict(
        {
            ...
            "type": mtype,
            "data_start_time_utc": datetime.fromtimestamp(
                start_end[0], tz=timezone.utc
            ),
            "data_end_time_utc": datetime.fromtimestamp(start_end[1], tz=timezone.utc),
            "primary_serial_number": serialnums[0],
            "secondary_serial_number": serialnums[1],
            "sonar_model_number": serialnums[2],
            "last_modified_time_utc": basic["last_modified_time_utc"],
            "created_time_utc": basic["created_time_utc"],
            "file_size_kb": basic["file_size_kb"],
...
def gather_navfile_info(ppnav_file: str):
    ...
    if tms is None:
        raise IOError(
            "File ({}) is not a valid postprocessed navigation file".format(ppnav_file)
        )
    mtype = "POSPac sbet"
    info_data = OrderedDict(
        {
            "file_path": basic["file_path"],
            "type": mtype,
            "weekly_seconds_start": tms[0],
            "weekly_seconds_end": tms[1],
            "last_modified_time_utc": basic["last_modified_time_utc"],
            "created_time_utc": basic["created_time_utc"],
            "file_size_kb": basic["file_size_kb"],
            "time_added": basic["time_added"],
...
def gather_naverrorfile_info(pperror_file: str):
    ...
    if tms is None:
        raise IOError(
            "File ({}) is not a valid postprocessed error file".format(pperror_file)
        )
    mtype = "POSPac smrmsg"
    info_data = OrderedDict(
        {
...
def gather_exportlogfile_info(exportlog_file: str):
    ...
    if loginfo is not None:
        info_data = OrderedDict(
            {
                "file_path": basic["file_path"],
                "type": "sbet_export_log",
                "exported_sbet_file": loginfo["exported_sbet_file"],
                "mission_date": loginfo["sbet_mission_date"],
                "datum": loginfo["sbet_datum"],
                "ellipsoid": loginfo["sbet_ellipsoid"],
                "last_modified_time_utc": basic["last_modified_time_utc"],
                "created_time_utc": basic["created_time_utc"],
                "file_size_kb": basic["file_size_kb"],
                "time_added": basic["time_added"],
...
def gather_svp_info(svp_file: str):
    ...
    formatted_time_utc = [
        datetime.fromtimestamp(tm, tz=timezone.utc) for tm in svp_dict["svp_time_utc"]
    ]
    info_data = OrderedDict(
        {
            "file_path": basic["file_path"],
            "type": "caris_svp",
            "profiles": svp_dict["profiles"],
            "number_of_profiles": svp_dict["number_of_profiles"],
            "number_of_layers": svp_dict["number_of_layers"],
            "julian_day": svp_dict["svp_julian_day"],
            "time_utc": formatted_time_utc,
            "time_utc_seconds": svp_dict["svp_time_utc"],
            "latitude": svp_dict["latitude"],
            "longitude": svp_dict["longitude"],
            "source_epsg": svp_dict["source_epsg"],
            "utm_zone": svp_dict["utm_zone"],
...
def likelihood_start_end_times_close(
    filetimes: list, compare_times: list, allowable_diff: int = 2
):
    """
...
def intel_process(
    ...
    force_coordinate_system: bool = True,
    cast_selection_method: str = "nearest_in_time",
    designated_surface: str = "",
...
def intel_process_service(
    folder_path: Union[list, str],
