...
class Filter(BaseFilter):
    def __init__(self, fqpr, selected_index=None):
        ...
        self.controls = [
            [
                ...
                "min_depth",
                0.0,
                {"minimum": -100, "maximum": 99999999, "singleStep": 0.1},
            ],
            [
                "float",
                "max_depth",
    ...
    def _run_algorithm(self, min_depth: float = None, max_depth: float = None):
        ...
        if min_depth is None and max_depth is None:
            raise ValueError(
                "filter_by_depth: Filter must have either min or max depth set"
            )
        if min_depth is not None and max_depth is not None and min_depth >= max_depth:
            raise ValueError(
                f"filter_by_depth: minimum depth {min_depth} cannot be greater than maximum depth {max_depth}"
            )
        if "z" not in self.fqpr.multibeam.raw_ping[0]:
            raise ValueError(
                f"filter_by_depth: unable to find georeferenced depths, have you processed this data?"
            )

        print(
            f"Running filter_by_depth ({min_depth},{max_depth}) on {self.fqpr.output_folder}"
        )
        self.new_status = (
            []
