...
class Filter(BaseFilter):
    def __init__(self, fqpr, selected_index=None):
        ...
        self.controls = [
            [
                ...
                "min_angle",
                -45.0,
                {"minimum": -180, "maximum": 180, "singleStep": 0.1},
            ],
            [
                "float",
                "max_angle",
    ...
    def _run_algorithm(self, min_angle: float = None, max_angle: float = None):
        ...
        if min_angle is None and max_angle is None:
            raise ValueError(
                "filter_by_angle: Filter must have either min or max angle set"
            )
        if min_angle is not None and max_angle is not None and min_angle >= max_angle:
            raise ValueError(
                f"filter_by_angle: minimum angle {min_angle} cannot be greater than maximum angle {max_angle}"
            )
        if "corr_pointing_angle" not in self.fqpr.multibeam.raw_ping[0]:
            raise ValueError(
                f"filter_by_angle: unable to find corrected beam angles, have you processed this data?"
            )

        print(
            f"Running filter_by_angle ({min_angle},{max_angle}) on {self.fqpr.output_folder}"
        )
        self.new_status = (
            []
