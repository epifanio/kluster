...
class Filter(BaseFilter):
    def __init__(self, fqpr, selected_index=None):
        ...
        self.description = (
            "Reject all soundings that are greater i.e. deeper than (percent_depth / 100) * 2 * nadir depth in the ping.\n"
    ...
    def _run_algorithm(self, percent_depth: float):
        ...
        if "z" not in self.fqpr.multibeam.raw_ping[0]:
            raise ValueError(
                f"second_return: unable to find georeferenced depths, have you processed this data?"
            )
        ...
        for cnt, rp in enumerate(self.fqpr.multibeam.raw_ping):
            ...
            filt_nadir[0:8] = filt_nadir[
                9
            ]  # extend the rolling min to cover the missing windowed values
            filter_mask = (
                rp.z > ((percent_depth / 100) * 2 * filt_nadir)
            ).values.flatten()
            print(
                f"second_return: rejecting {np.count_nonzero(filter_mask)} out of {filter_mask.size} points"
            )
