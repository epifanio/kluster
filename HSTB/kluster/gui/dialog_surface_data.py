...
class SurfaceDataDialog(SaveStateDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.mark_for_update_button.setToolTip(
            'Mark one of the "In the Surface" containers as needing to be re-added to the grid'
        )
    ...
    def mark_for_update(self):
        ...
        if (
            curitem_text in self.listdata.left_tree.tree_data["Data"][1:]
        ):  # selected a container, mark all lines for update
            ...
            curitem.parent().data() in self.listdata.left_tree.tree_data["Data"][1:]
        ):  # selected a line, mark for update
            idx = self.listdata.left_tree.tree_data["Data"][1:].index(
                curitem.parent().data()
            )
            container = self.listdata.left_tree.tree_data["Data"][0].child(idx)
            child_idx = [
                rw
                for rw in range(container.rowCount())
                if container.child(rw).text() == curitem_text
            ]
            if child_idx:
    ...
    def return_processing_options(self):
        ...
        if regrid_option == "The whole grid":
            ...
            raise ValueError(
                "Expected regrid option to be one of ['The whole grid', 'Only where points have changed'], found {}".format(
                    regrid_option
                )
            )
        ...
        for container, lines in self.original_current.items():
            fmatlines = [ln[:-1] for ln in lines if ln[-1] == "*"] + [
                ln for ln in lines if ln[-1] != "*"
            ]
            remove_lines = []
            if container in current_containers:
                remove_lines = [
                    ln for ln in fmatlines if ln not in current_containers[container]
                ]
            if (
                container not in current_containers and container not in remove_fqpr
            ):  # whole container was removed
                remove_fqpr[container] = fmatlines
            elif remove_lines:  # some original lines are no longer there
                if container in remove_fqpr:
                    remove_fqpr[container] += [
                        ln for ln in remove_lines if ln not in remove_fqpr[container]
                    ]
                else:
        ...
        for container, lines in current_containers.items():
            ...
            if container in self.original_current:
                new_lines = [
                    ln for ln in fmatlines if ln not in self.original_current[container]
                ]
            if (
                container not in self.original_current and container not in add_fqpr
            ):  # whole container was added
                add_fqpr[container] = fmatlines
            elif new_lines:  # some new lines are there
                if container in add_fqpr:
                    add_fqpr[container] += [
                        ln for ln in new_lines if ln not in add_fqpr[container]
                    ]
                else:
    ...
    def start_processing(self):
        if not self.listdata.return_left_tree_data():
            self.status_msg.setText(
                "Error: You must include at least one point source to continue"
            )
        else:
            self.canceled = False
            self.save_settings()
