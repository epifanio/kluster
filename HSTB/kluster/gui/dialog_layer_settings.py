...
class LayerSettingsDialog(SaveStateDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.layer_dropdown.addItems(
            [
                "None",
                "Default",
                "VDatum Coverage (VDatum required)",
                "OpenStreetMap (internet required)",
                "Satellite (internet required)",
                "NOAA RNC (internet required)",
    ...
    def return_layer_options(self):
        ...
        if not self.canceled:
            try:
                ...
                self.print(
                    "Layer_Settings: transparency={} is invalid, must be an integer between 0 and 100, defaulting to 0".format(
                ...
                transp = 0
            opts = {
                "layer_background": self.layer_dropdown.currentText(),
    ...
    def save_settings(self):
        ...
        settings.setValue(
            "Kluster/layer_settings_background", new_sets["layer_background"]
        )
        settings.setValue(
            "Kluster/layer_settings_transparency", new_sets["layer_transparency"]
        )
    ...
    def read_settings(self):
        ...
        try:
            self.layer_dropdown.setCurrentText(
                settings.value("Kluster/layer_settings_background")
            )
            if settings.value("Kluster/layer_settings_transparency"):
                self.transparency.setText(
                    str(
                        min(
                            100,
                            int(
