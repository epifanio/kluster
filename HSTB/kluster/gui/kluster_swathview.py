...
def make_spiral(
    num_points=100, num_turns=4, height=12, radius=2.0, xnot=None, ynot=None, znot=None
):
    """
...
def make_spring(
    num_points=300, num_turns=4, height=12, radius=2.0, xnot=None, ynot=None, znot=None
):
    """
    ...
    for i in range(init_pts // 2):
        ...
        coords_list.append((xnot, ynot, znot))
    coords_list += make_spiral(
        num_points=num_points - 3 * init_pts,
        num_turns=num_turns,
...
class WigglyBar(app.Canvas):
    def __init__(
        ...
        app.Canvas.__init__(
            self, title="Wiggly Bar", size=(800, 800), create_native=False
        )
        ...
        self.rod = visuals.BoxVisual(
            width=self.px_len / 40,
    ...
    def on_timer(self, ev):
        ...
        trig_junk = np.sin(self.theta_not) * (np.cos(extra_term) - 1) + np.cos(
            self.theta_not
        ) * np.sin(extra_term)
        ...
        self.spring_2.transform.translate(
            self.center + self.s2_loc + np.asarray([delta_x, 0])
        )
    ...
    def _set_up_system(
        ...
        self.method = (
            string.capwords(method, "-") if method.lower() in VALID_METHODS else "Euler"
        )
        ...
        self.j_term = (
            (1 / 3) * self.big_m * (self.d1 ** 3 + self.d2 ** 3) / (self.d1 + self.d2)
        )
        ...
        self.x_is_0 = (
            -self.d2 * self.px_per_m * np.sin(self.theta_not) - 1.5 * self.s1_l_not
        )
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, param=None):
        ...
        self.parameter_object.param = (
            param if param is not None else self.parameter_object.param
        )
