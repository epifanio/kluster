...
test_xyzrph = {
    "123": {
        ...
        "source": {
            "1503413148": "sonarfile1.all",
            "1503423148": "sonarfile2.all",
            "1503443148": "sonarfile3.all",
        },
        "heading_patch_error": {
            "1503413148": 0.5,
            "1503423148": 0.5,
        ...
        "vessel_file": {
            "1503413148": r"C:\\PydroXL_19\\NOAA\\site-packages\\Python38\\HSTB\\kluster\\gui\\vessel_stl_files\\westcoast_28ft_launch.obj",
...
test_xyzrph_dual = {
    "541": {
        "sonar_type": {
            "1583305645": "em2040_dual_rx",
            "1583315645": "em2040_dual_rx",
            "1583335645": "em2040_dual_rx",
        },
        "source": {
            "1583305645": "sonarfile4.all",
            "1583315645": "sonarfile5.all",
            "1583335645": "sonarfile6.all",
        },
        "heading_patch_error": {
            "1583305645": 0.5,
            "1583315645": 0.5,
        ...
        "vessel_file": {
            "1583305645": r"C:\\PydroXL_19\\NOAA\\site-packages\\Python38\\HSTB\\kluster\\gui\\vessel_stl_files\\westcoast_28ft_launch.obj",
...
def new_cube(parent, size, color, origin):
    ...
    """
    my_cube = scene.visuals.Cube(
        size=size * 1000, color=color, parent=parent
    )  # expects units in mm
...
def new_plane(parent, width, height, color, origin):
    ...
    """
    my_plane = scene.visuals.Plane(
        width * 1000, height * 1000, 1, 1, direction="+z", color=color, parent=parent
    )  # expects units in mm
...
class MovingObject:
    ...
    def set_position(
        self, pos, from_meters=True, flip_z=True, flip_y=True, flip_x=False
    ):
        """
        ...
        if self.debug:
            print(
                "-- {} = -{} + {} + {}".format(newpos, self.position, pos, self.origin)
            )
        ...
        if self.debug:
            print(
                "-- pos {} new transform {} currorigin {}".format(
                    self.position, newpos, self.origin
                )
            )

    def set_rotation(
        self, rot, from_deg=True, flip_r=False, flip_p=True, flip_yaw=True
    ):
        """
    ...
    def set_origin(
        ...
        if self.debug:
            print(
                "{} set_origin {} {}, old origin {}".format(
                    self.name, self.name, pos, from_meters
                )
            )
        if flip_z:
            pos[2] = -pos[2]
    ...
    def finalize_position(
        ...
        self.rotation_history = (
            []
        )  # holds previous rotations that have been performed (so we can reverse them)
...
class Waterline(MovingObject):
    ...
    def __init__(self, mesh_type, position, **kwargs):
        ...
        if self.mesh_type == "plane":
            self.mesh = new_plane(
                self.parent, self.width, self.height, self.color, self.origin
            )
        else:
...
class Sensor(MovingObject):
    ...
    def resize(self, new_size):
        ...
        self.set_position(
            newpos, from_meters=True, flip_x=False, flip_y=False, flip_z=False
        )
...
class Vessel(MovingObject):
    ...
    def __init__(self, vessel_file, position, **kwargs):
        ...
        self.mesh = scene.visuals.Mesh(
            vertices=None, faces=None, color=self.color, parent=self.parent
        )
...
class TimestampDialog(QtWidgets.QDialog):
    ...
    def return_new_timestamp(self):
        ...
        return (
            str(int(formatted_timestamp.timestamp())),
            self.remove_this_checkbox.isChecked(),
        )
...
class OptionsWidget(QtWidgets.QWidget):
    ...
    update_sensor_sig = Signal(
        str, float, float, float, float, float, float, float, float, float, float, float
    )  # user submitted a sensor update
    ...
    def __init__(self, parent=None):
        ...
        vessels = os.listdir(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "vessel_stl_files"
            )
        )
        vessels = [
            vess for vess in vessels if os.path.splitext(vess)[1] in [".obj", ".stl"]
        ]
        ...
        self.opening_angle_label, self.opening_angle = self.add_num_ctrl(
            "Opening Angle (degrees)",
            tooltip="beam opening angle, should auto populate from the multibeam data.",
        )
        ...
        for widg in [
            ...
            self.plabel,
            self.yawlabel,
            self.opening_angle_label,
            self.latencylabel,
        ]:
            third_option_sub_labels.addWidget(widg)
        for widg in [
            self.waterline_spacer,
            self.x,
            self.y,
            self.z,
            self.r,
        ...
        vessel_center_descrp = QtWidgets.QLabel(
            "For display only, align vessel with sensors"
        )
        ...
        vertlabel, self.vertical_positioning_error = self.add_num_ctrl(
    ...
    def get_currently_selected_time(self):
        ...
        if (
            tstmp
        ):  # on loading new xyzrph, vesselcenter is updated before timestamps are loaded.  But it gets updated later so skip here
    ...
    def update_sensor_data(
        ...
        self.update_sensor_sig.emit(
            sensorname, x, y, z, r, p, h, extra, extra2, extra3, extra4, extra5
        )
    ...
    def update_button_pressed(self):
        ...
        if sens == "Display Config":
            ...
            pos = [
                float(self.vcenter_x.text()),
                float(self.vcenter_y.text()),
                float(self.vcenter_z.text()),
                float(self.vcenter_r.text()),
                float(self.vcenter_p.text()),
                float(self.vcenter_yaw.text()),
                float(sensor_size),
            ]
            ...
            pos = [
                float(self.heave_error.text()),
                float(self.roll_sensor_error.text()),
                float(self.pitch_sensor_error.text()),
                float(self.heading_sensor_error.text()),
                float(self.surface_sv_error.text()),
                float(self.roll_patch_error.text()),
                float(self.waterline_error.text()),
                float(self.horizontal_positioning_error.text()),
        ...
        for serial_num in xyzrph:
            ...
            if "tx_port_x" in xyzrph[serial_num]:
                ...
                if "tx_port_opening_angle" not in xyzrph[serial_num]:
                    xyzrph[serial_num]["tx_port_opening_angle"] = deepcopy(
                        xyzrph[serial_num]["beam_opening_angle"]
                    )
                    xyzrph[serial_num]["tx_stbd_opening_angle"] = deepcopy(
                        xyzrph[serial_num]["beam_opening_angle"]
                    )
                    xyzrph[serial_num]["rx_port_opening_angle"] = deepcopy(
                        xyzrph[serial_num]["beam_opening_angle"]
                    )
                    xyzrph[serial_num]["rx_stbd_opening_angle"] = deepcopy(
                        xyzrph[serial_num]["beam_opening_angle"]
                    )
                for tstmp in tstmps:
                    self.data[serial_num][tstmp] = {}
                    self.data[serial_num][tstmp]["Dual Head"] = True
                    self.data[serial_num][tstmp]["Port Sonar Transmitter"] = [
                        float(xyzrph[serial_num]["tx_port_x"][tstmp]),
                    ...
                    self.data[serial_num][tstmp]["Port Sonar Receiver"] = [
                        float(xyzrph[serial_num]["rx_port_x"][tstmp]),
                    ...
                    self.data[serial_num][tstmp]["Stbd Sonar Transmitter"] = [
                        float(xyzrph[serial_num]["tx_stbd_x"][tstmp]),
                    ...
                    self.data[serial_num][tstmp]["Stbd Sonar Receiver"] = [
                        float(xyzrph[serial_num]["rx_stbd_x"][tstmp]),
                ...
                if "tx_opening_angle" not in xyzrph[serial_num]:
                    xyzrph[serial_num]["tx_opening_angle"] = deepcopy(
                        xyzrph[serial_num]["beam_opening_angle"]
                    )
                    xyzrph[serial_num]["rx_opening_angle"] = deepcopy(
                        xyzrph[serial_num]["beam_opening_angle"]
                    )
                for tstmp in tstmps:
                    self.data[serial_num][tstmp] = {}
                    self.data[serial_num][tstmp]["Dual Head"] = False
                    self.data[serial_num][tstmp]["Sonar Transmitter"] = [
                        float(xyzrph[serial_num]["tx_x"][tstmp]),
                    ...
                    self.data[serial_num][tstmp]["Sonar Receiver"] = [
                        float(xyzrph[serial_num]["rx_x"][tstmp]),
            ...
            for tstmp in tstmps:
                self.data[serial_num][tstmp]["Vessel Reference Point"] = [
                    0.0,
                    0.0,
                    0.0,
                    0.0,
                ...
                self.data[serial_num][tstmp]["Vessel File"] = os.path.normpath(
                    xyzrph[serial_num]["vessel_file"][tstmp]
                )
                self.data[serial_num][tstmp]["Sonar Type"] = xyzrph[serial_num][
                    "sonar_type"
                ][tstmp]
                self.data[serial_num][tstmp]["Source"] = xyzrph[serial_num]["source"][
                    tstmp
                ]
                try:
                    self.data[serial_num][tstmp]["IMU"] = [
                        float(xyzrph[serial_num]["imu_x"][tstmp]),
                ...
                try:
                    self.data[serial_num][tstmp]["Primary Antenna"] = [
                        float(xyzrph[serial_num]["tx_to_antenna_x"][tstmp]),
                ...
                self.data[serial_num][tstmp]["Waterline"] = [
                    0,
                    0,
                    xyzrph[serial_num]["waterline"][tstmp],
                    0,
                    0,
                    0,
                ]
                self.data[serial_num][tstmp]["Latency"] = [
                    0,
                    0,
                    0,
                    0,
                    0,
                    0,
                    xyzrph[serial_num]["latency"][tstmp],
                ]
                try:
                    self.data[serial_num][tstmp]["Vesselcenter"] = [
                        float(xyzrph[serial_num]["vess_center_x"][tstmp]),
                ...
                try:
                    self.data[serial_num][tstmp]["Uncertainty"] = [
                        ...
                        float(
                            xyzrph[serial_num]["horizontal_positioning_error"][tstmp]
                        ),
    ...
    def populate_from_xyzrph(self, xyzrph):
        ...
        if xyzrph is None:  # clear the data
            ...
            self.update_button.setEnabled(False)
            for sens in [
                "Port Sonar Transmitter",
                "Port Sonar Receiver",
                "Stbd Sonar Transmitter",
                "Stbd Sonar Receiver",
                "Sonar Transmitter",
                "Sonar Receiver",
                "IMU",
    ...
    def time_selected(self, evt, setup=False):
        ...
        if setup:
            ...
            self.timestamps_converted = [
                datetime.fromtimestamp(int(tstmp)).strftime("%m/%d/%Y %H%M")
                for tstmp in self.timestamps
            ]
            self.time_select.addItems(
                [
                    tstmp_conv
                    for _, tstmp_conv in sorted(
                        zip(self.timestamps, self.timestamps_converted)
                    )
                ]
            )
        else:
            ...
            if serial_num and curr_timestamp:
                ...
                if data["Dual Head"]:  # dual head
                    self.update_sensor_data(
                        ...
                        *[
                            np.round(float(x), 3)
                            for x in data["Port Sonar Transmitter"]
                        ],
                    )
                    self.update_sensor_data(
                        "Port Sonar Receiver",
                        *[np.round(float(x), 3) for x in data["Port Sonar Receiver"]],
                    )
                    self.update_sensor_data(
                        "Stbd Sonar Transmitter",
                        *[
                            np.round(float(x), 3)
                            for x in data["Stbd Sonar Transmitter"]
                        ],
                    )
                    self.update_sensor_data(
                        "Stbd Sonar Receiver",
                        *[np.round(float(x), 3) for x in data["Stbd Sonar Receiver"]],
                    )
                else:
                    ...
                    self.update_sensor_data(
                        "Sonar Transmitter",
                        *[np.round(float(x), 3) for x in data["Sonar Transmitter"]],
                    )
                    self.update_sensor_data(
                        "Sonar Receiver",
                        *[np.round(float(x), 3) for x in data["Sonar Receiver"]],
                    )
                self.update_sensor_data(
                    "IMU", *[np.round(float(x), 3) for x in data["IMU"]]
                )
                self.update_sensor_data(
                    "Primary Antenna",
                    *[np.round(float(x), 3) for x in data["Primary Antenna"]],
                )
                self.update_sensor_data(
                    "Waterline", *[np.round(float(x), 3) for x in data["Waterline"]]
                )
                self.update_sensor_data(
                    "Latency", *[np.round(float(x), 3) for x in data["Latency"]]
                )
                self.update_sensor_data(
                    "Vesselcenter",
                    *[np.round(float(x), 3) for x in data["Vesselcenter"]],
                )
                self.update_sensor_data(
                    "Uncertainty", *[np.round(float(x), 3) for x in data["Uncertainty"]]
                )
    ...
    def vessel_selected(self, evt):
        ...
        if (
            self.data
            and serial_num in self.data
            and curr_tstmp in self.data[serial_num]
        ):
            self.data[serial_num][curr_tstmp][
                "Vessel File"
            ] = self.vess_select.currentText()
        ...
        if self.data:
            ...
            if serial_num and tstmp:
                if sensor_label == "Display Config":
                    ...
                    self.dualhead_option.setChecked(
                        self.data[serial_num][tstmp]["Dual Head"]
                    )
                    ...
                    self.horizontal_positioning_error.setText(
                        format(float(data[7]), ".3f")
                    )
                    self.vertical_positioning_error.setText(
                        format(float(data[8]), ".3f")
                    )
                else:
                    data = self.data[serial_num][tstmp][sensor_label]
                    if sensor_label == "IMU":
                        if np.array_equal(
                            data[0:3], v4_target_sensing_center_offset
                        ) or np.array_equal(data[0:3], v5_target_sensing_center_offset):
...
class VesselView(QtWidgets.QWidget):
    ...
    def build_sensors(self):
        ...
        self.tx_primary = Sensor(
            "cube",
            hide_location,
            parent=self.vessview.scene,
            size=self.curr_sensor_size,
            name="tx_primary",
            color=Color("red", alpha=0.2),
            origin=self.origin,
        )
        self.tx_secondary = Sensor(
            "cube",
            hide_location,
            parent=self.vessview.scene,
            size=self.curr_sensor_size,
        ...
        self.antenna = Sensor(
            ...
            parent=self.vessview.scene,
            size=self.curr_sensor_size,
            name="primary_antenna",
            color=Color("yellow", alpha=0.2),
            origin=self.origin,
        )

        self.sensor_lookup = {
            "Sonar Transmitter": self.tx_primary,
            "Sonar Receiver": self.rx_primary,
            "Port Sonar Transmitter": self.tx_primary,
            "Port Sonar Receiver": self.rx_primary,
            "Stbd Sonar Transmitter": self.tx_secondary,
            "Stbd Sonar Receiver": self.rx_secondary,
            "IMU": self.imu,
            "Primary Antenna": self.antenna,
        }
    ...
    def build_axes(self):
        self.x_axis = scene.visuals.Arrow(
            pos=np.array([[0, 0], [50000, 0]]),
            color="r",
            parent=self.vessview.scene,
            arrows=np.array([[49000, 0, 50000, 0], [49000, 0, 50000, 0]]),
            arrow_size=8,
            arrow_color="r",
            arrow_type="triangle_60",
        )
        self.x_axis_lbl = scene.visuals.Text(
            "x + Forward",
            color="r",
            pos=(58000, 0, 0),
            font_size=1000000,
        ...
        self.y_axis = scene.visuals.Arrow(
            pos=np.array([[0, 0], [0, -50000]]),
            color="g",
            parent=self.vessview.scene,
            arrows=np.array([[0, -49000, 0, -50000], [0, -49000, 0, -50000]]),
            arrow_size=8,
            arrow_color="g",
            arrow_type="triangle_60",
        )
        self.y_axis_lbl = scene.visuals.Text(
            "y + Starboard",
            color="g",
            pos=(0, -59000, 0),
            font_size=1000000,
        ...
        self.z_axis = scene.visuals.Arrow(
            pos=np.array([[0, 0, 0], [0, 0, -50000]]),
            color="b",
            parent=self.vessview.scene,
            arrows=np.array(
                [[0, 0, -49000, 0, 0, -50000], [0, 0, -49000, 0, 0, -50000]]
            ),
            arrow_size=8,
            arrow_color="b",
            arrow_type="triangle_60",
        )
        self.z_axis_lbl = scene.visuals.Text(
            "z + Down",
            color="b",
            pos=(0, 0, -58000),
            font_size=1000000,
    ...
    def build_vessel(self, pth_to_vess_file):
        ...
        if os.path.isfile(
            pth_to_vess_file
        ):  # if the vessel control is blank for some reason, or the file goes missing, it will default back to the fist in the os.listdir init
        ...
        if pth_to_vess_file_ext not in [".obj", ".stl"]:
            raise ValueError(
                "Only .obj and .stl are currently supported.  Got {}".format(
                    pth_to_vess_file
                )
            )
        ...
        if self.show_vessel:
            self.vessel = Vessel(
                ...
                vesspos,
                parent=self.vessview.scene,
                color=Color("grey", alpha=0.3),
                name="vessel",
            )
            if self.current_vessel_position is not None:
                self.vessel.set_position(
                    self.current_vessel_position.copy(),
                    from_meters=True,
                    flip_z=True,
                    flip_y=True,
                    flip_x=False,
            ...
            if self.current_vessel_rotation is not None:
                self.vessel.set_rotation(
                    self.current_vessel_rotation.copy(), from_deg=True
                )
        ...
        if self.first_time_setup:
            self.vessview.camera = scene.cameras.TurntableCamera(
                parent=self.vessview.scene, center=(0, 0, 0)
            )
        ...
        if self.sensor_lookup:  # sensors must be built first
            if sensor_lbl == "Vesselcenter":
                ...
                if sensor:
                    ...
                    sensor.set_position(
                        newpos, from_meters=True, flip_z=True, flip_y=True, flip_x=False
                    )
...
class VesselWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.vessview_window.setSizePolicy(
        ...
        self.show_vessel_action.toggled.connect(
            self.vessview_window.show_vessel_triggered
        )
        ...
        self.opts_window.sensor_selected_sig.connect(
            self.vessview_window.sensor_selected
        )
        ...
        self.instructions.setStyleSheet(
            "QLabel { font-style : italic ; font-size : 12px }"
        )
        ...
        if self.xyzrph:
            ...
            if dlog.exec_() and not dlog.canceled:
                ...
                if new_timestamp in self.opts_window.data[orig_serial]:
                    self.print(
                        "{} is an existing timestamp, cannot change {} to {}".format(
                            new_timestamp, orig_time, new_timestamp
                        ),
                        logging.ERROR,
                    )
                for sensor in self.xyzrph[orig_serial]:
    ...
    def new_entry(self):
        ...
        if self.xyzrph:
            ...
            if dlog.exec_() and not dlog.canceled:
                ...
                tstmps = np.array(
                    [float(tst) for tst in self.opts_window.data[orig_serial].keys()]
                )
                nearest = str(
                    int(tstmps[np.abs(tstmps - float(new_timestamp)).argmin()])
                )
                for sensor in self.xyzrph[orig_serial]:
                    self.xyzrph[orig_serial][sensor][new_timestamp] = self.xyzrph[
                        orig_serial
                    ][sensor][nearest]
                self.opts_window.data[orig_serial][
                    new_timestamp
                ] = self.opts_window.data[orig_serial][nearest]
                self.opts_window.time_selected(None, setup=True)
        else:
            self.print("No vessel file loaded", logging.ERROR)
    ...
    def update_xyzrph_vessel(self, pth_to_vessel):
        ...
        if self.xyzrph is not None:
            ...
            if orig_tstmp is not None:
                self.xyzrph[serial_num]["vessel_file"][orig_tstmp] = os.path.normpath(
                    pth_to_vessel
                )

    def update_xyzrph_sensorposition(
        self, sensor_lbl, x, y, z, r, p, h, extra=0.0, extra2=0.0, extra3=0.0
    ):
        """
                        ...
                        if entry is not None and self.xyzrph is not None:
                            try:
                                if entry in self.xyzrph[serial_num]:
                                    if float(
                                        self.xyzrph[serial_num][entry][orig_tstmp]
                                    ) != float(data[cnt]):
                                        self.xyzrph[serial_num][entry][
                                            orig_tstmp
                                        ] = format(data[cnt], ".3f")
                                elif entry in [
                                    "tx_opening_angle",
                                    "tx_port_opening_angle",
                                    "tx_starboard_opening_angle",
                                    "rx_opening_angle",
                                    "rx_port_opening_angle",
                                    "rx_starboard_opening_angle",
                                ]:
                                    ...
                                    self.xyzrph[serial_num][entry] = deepcopy(
                                        self.xyzrph[serial_num]["beam_opening_angle"]
                                    )
                                    self.xyzrph[serial_num][entry][orig_tstmp] = format(
                                        data[cnt], ".3f"
                                    )
                                else:
                                    # vesselcenter might not be in xyzrph, as it is created in the vessel view widget
                                    self.xyzrph[serial_num][entry] = {
                                        orig_tstmp: format(data[cnt], ".3f")
                                    }
                            except KeyError:
                                self.print(
                                    "Unable to update self.xyzrph for {} with time stamp {}: {} not in {}".format(
                                        entry, orig_tstmp, orig_tstmp, entry
                                    ),
                                    logging.ERROR,
                                )
    ...
    def import_from_posmv(self):
        ...
        msg, fil = RegistryHelpers.GetFilenameFromUserQT(
            self,
            RegistryKey="kluster",
            Title="Select a POS MV file (.000)",
            AppName="klusterbrowse",
            bMulti=False,
            bSave=False,
            fFilter="POS MV file (*.*)",
        )
                    ...
                    for sens in sensors:
                        for tstmp in self.xyzrph[serial_num][sens]:
                            if float(posxyzrph[sens]):
                                self.xyzrph[serial_num][sens][tstmp] = str(
                                    posxyzrph[sens]
                                )
                    self.populate_from_xyzrph()
                else:
                    self.print(
                        "Expect data to exist before loading from POS MV, please import from multibeam first or open config file",
                        logging.ERROR,
                    )
            else:
    ...
    def import_from_multibeam(self):
        ...
        if fil:
            ...
            mbesxyzrph, sonar_model, serial_number = return_xyzrph_from_mbes(
                fil, logger=self.logger
            )
            if mbesxyzrph is not None:
                ...
                if not self.xyzrph:
                    ...
                    self.xyzrph[str(serial_number)]["sonar_type"] = {
                        tstmps[0]: sonar_model
                    }
                    self.xyzrph[str(serial_number)]["source"] = {
                        tstmps[0]: os.path.split(fil)[1]
                    }
                    ...
                    for sensor in mbesxyzrph:
                        for tstmp in tstmps:
                            try:
                                self.xyzrph[str(serial_number)][sensor][
                                    tstmp
                                ] = mbesxyzrph[sensor][tstmp]
                            except:
                                raise ValueError(
                                    "ERROR: Unable to load SN{} with sensor {} at timestamp {}".format(
                                        serial_number, sensor, tstmp
                                    )
                                )
    ...
    def _update_xyzrph_vesselposition(self, serial_number, tstmps):
        for tstmp in tstmps:
            ...
            sensor_data = [
                {tstmp: os.path.normpath(self.vessview_window.pth_to_vessel_file)},
                {tstmp: "0.000"},
                {tstmp: "0.000"},
                {tstmp: "0.000"},
                {tstmp: "0.000"},
    ...
    def save_configuration(self, event=None):
        ...
        if (
            self.opts_window.config_name.text() == "None"
        ):  # this data loaded was not from a config file, so we can update or save a new file
            save_first = AcceptDialog(
                "Do you want to create a new vessel file or update the multibeam data offsets?\n\n"
    ...
    def load_from_existing_xyzrph(self):
        ...
        try:
            ...
            currindex = self.opts_window.vess_select.findText(
                self.opts_window.vess_select.currentText()
            )
            self.opts_window.vess_select.setCurrentIndex(vessindex)
            if vessindex == currindex:
                self.opts_window.vessel_selected(None)
            pos = [
                float(self.xyzrph[serial_num]["vess_center_x"][first_tstmp]),
                float(self.xyzrph[serial_num]["vess_center_y"][first_tstmp]),
                float(self.xyzrph[serial_num]["vess_center_z"][first_tstmp]),
                float(self.xyzrph[serial_num]["vess_center_r"][first_tstmp]),
                float(self.xyzrph[serial_num]["vess_center_p"][first_tstmp]),
                float(self.xyzrph[serial_num]["vess_center_yaw"][first_tstmp]),
    ...
    def save_to_config_file(self):
        ...
        if fil:
            if self.xyzrph is not None:
                ...
                self.print(
                    "No data found for xyzrph: {}".format(self.xyzrph), logging.ERROR
                )
        else:
    ...
    def _handle_close_event(self, event=None):
        ...
        if self.xyzrph:
            save_first = AcceptDialog(
                "Do you want to save your changes?", "Kluster Vessel Setup"
            )
            save_state = save_first.run()
            if save_state == "yes":
