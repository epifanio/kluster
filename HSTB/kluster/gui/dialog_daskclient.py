...
class DaskClientStart(SaveStateDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.local_box.setToolTip(
            "Uses your computer resources in starting the Local Cluster, this is what you want when running on a computer normally."
        )
        ...
        self.number_workers_checkbox.setToolTip(
            "Use this checkbox if you want to set a specific number of workers, default is the number of cores on your machine"
        )
        self.number_workers_checkbox.setChecked(False)
        self.checkbox_layout.addWidget(self.number_workers_checkbox)
        self.number_threads_checkbox = QtWidgets.QCheckBox(
            "Override Threads per Worker"
        )
        self.number_threads_checkbox.setToolTip(
            "Use this checkbox if you want to set a specific number of threads per worker, default is based on the number of cores on your machine"
        )
        self.number_threads_checkbox.setChecked(False)
        self.checkbox_layout.addWidget(self.number_threads_checkbox)
        self.number_memory_checkbox = QtWidgets.QCheckBox(
            "Override Memory (GB) per Worker"
        )
        self.number_memory_checkbox.setToolTip(
            "Use this amount of memory for each worker, default is the max memory available on your system"
        )
        ...
        self.remote_box.setToolTip(
            "Use this when you have set up a Dask Cluster on a remote server, the address given here is the address of that server."
        )
        ...
        self.noclient_box.setToolTip(
            "Use this when you want to skip Dask parallel processing."
        )
    ...
    def setup_client(self):
        ...
        self.canceled = False
        if (
            self.local_box.isChecked()
            or self.remote_box.isChecked()
            or self.noclient_box.isChecked()
        ):
            self.accept()
            self.save_settings()
            if self.local_box.isChecked():
                ...
                if self.number_workers_checkbox.isChecked():
                    try:
                        ...
                        self.print(
                            "Invalid number of workers provided, number must be an integer, ex: 4",
                            logging.ERROR,
                        )
                        return
                if self.number_threads_checkbox.isChecked():
                    try:
                        ...
                        self.print(
                            "Invalid number of threads provided, number must be an integer, ex: 2",
                            logging.ERROR,
                        )
                        return
                if self.number_memory_checkbox.isChecked():
                    try:
                        ...
                        self.print(
                            "Invalid memory per worker provided, number must be an integer, ex: 5",
                            logging.ERROR,
                        )
                        return
                self.cl = dask_find_or_start_client(
                    number_of_workers=numworker,
                    threads_per_worker=threadsworker,
                    memory_per_worker=memoryworker,
                ...
                if self.remote_ip_radio.isChecked():
                    full_address = (
                        self.remote_ip_address.text() + ":" + self.remote_ip_port.text()
                    )
                else:
                    full_address = (
                        self.remote_fqdn_address.text()
                        + ":"
                        + self.remote_fqdn_port.text()
                    )

                self.print(
                    "Starting client at address {}".format(full_address), logging.INFO
                )
                try:
                    self.cl = dask_find_or_start_client(
                        address=full_address, logger=self.logger
                    )
                except:  # throws dask socket.gaierror, i'm not bothering to make this explicit
                    self.print(
                        "Unable to connect to remote Dask instance", logging.ERROR
                    )
            else:
            ...
            self.status_msg.setText(
                "Please select one of the options above (Local, Remote, or No Client)"
            )
