...
class SettingsDialog(SaveStateDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.parallel_write.setToolTip(
            "If checked, Kluster will write to the hard drive in parallel, disabling this "
        ...
        self.draw_navigation.setToolTip(
            "Sets the type of navigation used when drawing the tracklines on screen.\n\n"
        ...
        self.keep_waterline_changes.setToolTip(
            "If checked (only applicable if you are using a Vessel File), Kluster will save all "
        ...
        self.force_coordinate_match = QtWidgets.QCheckBox(
            "Force all days to have the same Coordinate System"
        )
        self.force_coordinate_match.setChecked(True)
        self.force_coordinate_match.setToolTip(
            "By default, Kluster will assign an automatic UTM zone number to each day of data.  If you "
        ...
        self.auto_processing_mode.setToolTip(
            "Controls the processing actions that appear when new data is added or settings are changed.\n"
        ...
        self.vdatum_text.setToolTip(
            'Optional, this is required if you are using the "NOAA MLLW" or "NOAA MHW" vertical reference options.'
        )
        ...
        self.kvar_pass_color.setToolTip(
            "Color of the graphical labels and text where a test passes"
        )
        ...
        self.kvar_error_color.setToolTip(
            "Color of the graphical labels and text where a test fails"
        )
        ...
        self.kvar_warning_color.setToolTip(
            "Color of the graphical labels and text where a warning is raised"
        )
        ...
        self.kvar_amplitude_color.setToolTip(
            "Color of the Amplitude status points in Points View"
        )
        ...
        self.kvar_phase_color.setToolTip(
            "Color of the Phase status points in Points View"
        )
        ...
        self.kvar_reject_color.setToolTip(
            "Color of the Reject status points in Points View"
        )
        ...
        self.kvar_reaccept_color.setToolTip(
            "Color of the Reaccept status points in Points View"
        )
        ...
        self.kvar_convfiles.setToolTip(
            'Conversion will convert this many files at once (within "Max converted chunk size" limit), raising \n'
        ...
        self.kvar_convsize.setToolTip(
            'Conversion will convert "Files converted at once" files at a time, where the total group file size\n'
        ...
        self.kvar_pingslas.setToolTip(
            "LAS export will put this many pings in one file before starting a new file, raising this value can create overly large files"
        )
        ...
        self.kvar_pingscsv.setToolTip(
            "CSV export will put this many pings in one file before starting a new file, raising this value can create overly large files"
        )
        ...
        self.kvar_tiledisplay_label = QtWidgets.QLabel(
            "Display grid chunk size (meters)"
        )
        ...
        self.kvar_tiledisplay.setToolTip(
        ...
        self.kvar_tileexport.setToolTip(
        ...
        self.kvar_buildheave.setToolTip(
        ...
        self.kvar_ekfreqsel.setToolTip(
        ...
        self.kvar_heaveerror.setToolTip(
            "Default 1 sigma standard deviation in the heave sensor, generally found in manufacturer specifications."
        )
        self.kvar_rollerror_label = QtWidgets.QLabel(
            "Default Roll Sensor Error (meters)"
        )
        ...
        self.kvar_rollerror.setToolTip(
            "Default 1 sigma standard deviation in the roll sensor, generally found in manufacturer specifications."
        )
        ...
        self.kvar_pitcherror_label = QtWidgets.QLabel(
            "Default Pitch Sensor Error (meters)"
        )
        ...
        self.kvar_pitcherror.setToolTip(
            "Default 1 sigma standard deviation in the pitch sensor, generally found in manufacturer specifications"
        )
        ...
        self.kvar_yawerror.setToolTip(
            "Default 1 sigma standard deviation in the heading sensor, generally found in manufacturer specifications"
        )
        ...
        self.kvar_beamangle_label = QtWidgets.QLabel(
            "Default Beam Opening Angle (degrees)"
        )
        ...
        self.kvar_beamangle.setToolTip(
            "Default Transmitter/Receiver beam opening angle, should auto populate from the multibeam data, this value is used otherwise"
        )
        self.kvar_sverror_label = QtWidgets.QLabel(
            "Default Surface SV Error (meters/second)"
        )
        ...
        self.kvar_sverror.setToolTip(
            "Default 1 sigma standard deviation in surface sv sensor, generally found in manufacturer specifications"
        )
        ...
        self.kvar_rollpatch_label = QtWidgets.QLabel(
            "Default Roll Patch Error (degrees)"
        )
        ...
        self.kvar_rollpatch.setToolTip(
            "Default 1 sigma standard deviation in your roll angle patch test procedure"
        )
        ...
        self.kvar_waterline.setToolTip(
            "Default 1 sigma standard deviation of the waterline measurement, only used for waterline vertical reference"
        )
        ...
        self.kvar_horizontalerror_label = QtWidgets.QLabel(
            "Default Horizontal Positioning Error (meters)"
        )
        self.kvar_horizontalerror = QtWidgets.QLineEdit("")
        self.kvar_horizontalerror.setValidator(validator)
        self.kvar_horizontalerror.setText(
            str(kluster_variables.default_horizontal_positioning_error)
        )
        self.kvar_horizontalerror.editingFinished.connect(self.validate_numctrl)
        self.kvar_horizontalerror.setToolTip(
            "Default 1 sigma standard deviation of the horizontal positioning system, only used if SBET is not provided"
        )
        self.kvar_verticalerror_label = QtWidgets.QLabel(
            "Default Vertical Positioning Error (meters)"
        )
        self.kvar_verticalerror = QtWidgets.QLineEdit("")
        self.kvar_verticalerror.setValidator(validator)
        self.kvar_verticalerror.setText(
            str(kluster_variables.default_vertical_positioning_error)
        )
        self.kvar_verticalerror.editingFinished.connect(self.validate_numctrl)
        self.kvar_verticalerror.setToolTip(
            "Default 1 sigma standard deviation of the vertical positioning system, only used if SBET is not provided"
        )
        ...
        self.text_controls = [
            ["vdatum_directory", self.vdatum_text],
            ["auto_processing_mode", self.auto_processing_mode],
            ["filter_text", self.filter_text],
            ["log_text", self.log_text],
            ["draw_navigation", self.draw_navigation],
        ]
        self.checkbox_controls = [
            ["enable_parallel_writes", self.parallel_write],
    ...
    def set_vyperdatum_path(self):
        if self.vdatum_pth:
            ...
            self.vdatum_pth == "" and self.vdatum_pth != self.orig_vdatum_pth
        ):  # special case where the user is wanting to clear the vdatum directory path
    ...
    def return_options(self):
        ...
        if not self.canceled:
            opts = {
                "write_parallel": self.parallel_write.isChecked(),
    ...
    def return_kvars(self):
        ...
        if not self.canceled:
            opts = {
                "pass_color": self.kvar_pass_color.currentText(),
                "error_color": self.kvar_error_color.currentText(),
                "warning_color": self.kvar_warning_color.currentText(),
                "amplitude_color": self.kvar_amplitude_color.currentText(),
                "phase_color": self.kvar_phase_color.currentText(),
                "reject_color": self.kvar_reject_color.currentText(),
                "reaccept_color": self.kvar_reaccept_color.currentText(),
                "converted_files_at_once": self.kvar_convfiles.text(),
                "max_converted_chunk_size": self.kvar_convsize.text(),
                "pings_per_las": self.kvar_pingslas.text(),
                "pings_per_csv": self.kvar_pingscsv.text(),
                "chunk_size_display": self.kvar_tiledisplay.text(),
                "chunk_size_export": self.kvar_tileexport.text(),
                "ek_build_heave": self.kvar_buildheave.isChecked(),
                "ek_frequency_selection": self.kvar_ekfreqsel.currentText(),
                "default_heave_error": self.kvar_heaveerror.text(),
                "default_roll_sensor_error": self.kvar_rollerror.text(),
                "default_pitch_sensor_error": self.kvar_pitcherror.text(),
                "default_heading_sensor_error": self.kvar_yawerror.text(),
                "default_beam_opening_angle": self.kvar_beamangle.text(),
                "default_surface_sv_error": self.kvar_sverror.text(),
                "default_roll_patch_error": self.kvar_rollpatch.text(),
                "default_waterline_error": self.kvar_waterline.text(),
    ...
    def vdatum_browse(self):
        ...
        msg, vdatum_pth = RegistryHelpers.GetDirFromUserQT(
            self,
            RegistryKey="Kluster",
    ...
    def filter_browse(self):
        # dirpath will be None or a string
        msg, filter_pth = RegistryHelpers.GetDirFromUserQT(
            self,
            RegistryKey="Kluster",
            Title="Select filter directory",
            AppName="\\reghelp",
        )
        if filter_pth:
    ...
    def log_browse(self):
        # dirpath will be None or a string
        msg, log_pth = RegistryHelpers.GetFilenameFromUserQT(
            self,
            RegistryKey="kluster",
            Title="Save a new log file",
            AppName="klusterproj",
            bMulti=False,
            bSave=True,
            DefaultFile="logfile.txt",
            fFilter="text (*.txt)",
        )
        if log_pth:
    ...
    def set_to_default(self):
        ...
        if curidx == 0:
            ...
            self.kvar_pass_color.setCurrentText(
                kluster_variables.kvar_initial_state["pass_color"]
            )
            self.kvar_error_color.setCurrentText(
                kluster_variables.kvar_initial_state["error_color"]
            )
            self.kvar_warning_color.setCurrentText(
                kluster_variables.kvar_initial_state["warning_color"]
            )
            self.kvar_amplitude_color.setCurrentText(
                kluster_variables.kvar_initial_state["amplitude_color"]
            )
            self.kvar_phase_color.setCurrentText(
                kluster_variables.kvar_initial_state["phase_color"]
            )
            self.kvar_reject_color.setCurrentText(
                kluster_variables.kvar_initial_state["reject_color"]
            )
            self.kvar_reaccept_color.setCurrentText(
                kluster_variables.kvar_initial_state["reaccept_color"]
            )
        elif curidx == 2:
            self.kvar_convfiles.setValue(
                int(kluster_variables.kvar_initial_state["converted_files_at_once"])
            )
            self.kvar_convsize.setValue(
                int(kluster_variables.kvar_initial_state["max_converted_chunk_size"])
            )
            self.kvar_pingslas.setValue(
                int(kluster_variables.kvar_initial_state["pings_per_las"])
            )
            self.kvar_pingscsv.setValue(
                int(kluster_variables.kvar_initial_state["pings_per_csv"])
            )
            self.kvar_tiledisplay.setValue(
                int(kluster_variables.kvar_initial_state["chunk_size_display"])
            )
            self.kvar_tileexport.setValue(
                int(kluster_variables.kvar_initial_state["chunk_size_export"])
            )
            self.kvar_ekfreqsel.setCurrentText(
                kluster_variables.kvar_initial_state["ek_frequency_selection"]
            )
            self.kvar_buildheave.setChecked(
                bool(kluster_variables.kvar_initial_state["ek_build_heave"])
            )
        elif curidx == 3:
            self.kvar_beamangle.setText(
                str(kluster_variables.kvar_initial_state["default_beam_opening_angle"])
            )
            self.kvar_heaveerror.setText(
                str(kluster_variables.kvar_initial_state["default_heave_error"])
            )
            self.kvar_rollerror.setText(
                str(kluster_variables.kvar_initial_state["default_roll_sensor_error"])
            )
            self.kvar_pitcherror.setText(
                str(kluster_variables.kvar_initial_state["default_pitch_sensor_error"])
            )
            self.kvar_yawerror.setText(
                str(
                    kluster_variables.kvar_initial_state["default_heading_sensor_error"]
                )
            )
            self.kvar_sverror.setText(
                str(kluster_variables.kvar_initial_state["default_surface_sv_error"])
            )
            self.kvar_rollpatch.setText(
                str(kluster_variables.kvar_initial_state["default_roll_patch_error"])
            )
            self.kvar_waterline.setText(
                str(kluster_variables.kvar_initial_state["default_waterline_error"])
            )
            self.kvar_horizontalerror.setText(
                str(
                    kluster_variables.kvar_initial_state[
                        "default_horizontal_positioning_error"
                    ]
                )
            )
            self.kvar_verticalerror.setText(
                str(
                    kluster_variables.kvar_initial_state[
                        "default_vertical_positioning_error"
                    ]
                )
            )
        else:
            raise NotImplementedError(
                "Found a tab index above 3 in settings dialog, should only be four tabs."
            )
