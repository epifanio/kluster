...
class ExportGridDialog(SaveStateDialog):
    ...
    def _set_default_bag_options(self):
        ...
        self.bag_process_step.setText(
            "Generated By GDAL {} and Kluster {}".format(
                return_gdal_version(), kluster_version
            )
        )
    ...
    def update_input_path(self, foldername: str):
        if not any(
            [
                os.path.exists(os.path.join(foldername, allw))
                for allw in allowable_grid_root_names
            ]
        ):
            foldername = ""
        ...
        if self.input_pth:
            ...
            self.status_msg.setText(
                '"Export from the following" folder must contain a root folder (i.e. VRGridTile_Root)'
            )
    ...
    def output_file_browse(self):
        ...
        if curr_opts == "csv":
            ...
            raise ValueError(
                "dialog_export_grid: unrecognized method: {}".format(curr_opts)
            )
    ...
    def _event_update_status(self, combobox_text: str):
        ...
        if curr_opts == "bag":
            ...
            if not self.isbackscatter:
                self.zdirect_check.hide()
            if (int(majr) == 3 and int(minr) >= 2) or (
                int(majr) > 3
            ):  # If this is the pydro environment, we know it has Entwine
                self.status_msg.setStyleSheet(
    ...
    def return_processing_options(self):
        ...
        return {
            ...
            "export_format": self.export_opts.currentText(),
            "z_positive_up": self.zdirect_check.isChecked(),
            "individual_name": self.bag_individual.text(),
            "organizational_name": self.bag_organizational.text(),
            "position_name": self.bag_position.text(),
            "attr_date": self.bag_date.text(),
            "vert_crs": self.bag_vert_crs.text(),
            "abstract": self.bag_abstract.text(),
            "process_step_description": self.bag_process_step.text(),
            "attr_datetime": self.bag_datetime.text(),
            "restriction_code": self.bag_restriction.text(),
            "other_constraints": self.bag_constraints.text(),
            "classification": self.bag_classification.text(),
            "security_user_note": self.bag_security.text(),
        }
