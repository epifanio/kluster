...
class ManageSurfaceDialog(ManageDialog):
    ...
    def populate(self, surf):
        ...
        self.managelabel.setText(
            "Manage: {}".format(os.path.split(surf.output_folder)[1])
        )
        ...
        if self.surf.is_backscatter:
            ...
            allplots += [
                "Histogram, Depth (meters)",
                "Depth vs Density (count)",
    ...
    def plot_tooltip_config(self, e):
        ...
        if plotname == "Histogram, Intensity (dB)":
            self.plotdropdown.setToolTip(
                "Compute a histogram of the Intensity layer values in the gridded data in this surface"
            )
        elif plotname == "Histogram, Depth (meters)":
            self.plotdropdown.setToolTip(
                "Compute a histogram of the Depth layer values in the gridded data in this surface"
            )
        elif plotname in ["Histogram, Density (count)"]:
            self.plotdropdown.setToolTip(
                "Compute a histogram of soundings per cell across all tiles in the grid"
            )
        elif plotname in ["Histogram, Density (sq meters)"]:
            self.plotdropdown.setToolTip(
                "Compute a histogram of soundings per cell per square meter across all tiles in the grid"
            )
        elif plotname in ["Depth vs Density (count)"]:
            self.plotdropdown.setToolTip(
                "Plot the average depth vs density, where density is the number of soundings per cell"
            )
        elif plotname in ["Depth vs Density (sq meters)"]:
            self.plotdropdown.setToolTip(
                "Plot the average depth vs density, where density is the density per cell per square meter"
            )
        elif plotname in ["Histogram, vertical uncertainty (2 sigma, meters)"]:
            self.plotdropdown.setToolTip(
                "Compute a histogram of the Vertical Uncertainty layer values in the gridded data in this surface"
            )
        elif plotname in ["Histogram, horizontal uncertainty (2 sigma, meters)"]:
            self.plotdropdown.setToolTip(
                "Compute a histogram of the Horizontal Uncertainty layer values in the gridded data in this surface"
            )
        elif plotname in ["Histogram, total uncertainty (2 sigma, meters)"]:
            self.plotdropdown.setToolTip(
                "Compute a histogram of the Total Uncertainty layer values in the gridded data in this surface"
            )
    ...
    def calc_tooltip_config(self, e):
        ...
        if calcname == "area, sq nm":
            self.calcdropdown.setToolTip(
                "Calculate the total area of the grid (only the populated cells) in square nautical miles"
            )
        elif calcname == "area, sq meters":
            self.calcdropdown.setToolTip(
                "Calculate the total area of the grid (only the populated cells) in square meters"
            )
