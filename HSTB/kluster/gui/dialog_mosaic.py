...
class MosaicDialog(SurfaceDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.basic_surface_group.setTitle(
            "Run mosaic generation on the following files:"
        )
        ...
        self.surf_method.setToolTip(
            "The algorithm used when gridding, will use this to determine the intensity value of the cell"
        )
        ...
        self.fgcorrect.setToolTip(
            "Remove the fixed gain that the sonar manufacturer has applied to the raw reflectivity.  This should\n"
        ...
        self.tvgcorrect.setToolTip(
            "Remove the time varying gain that the sonar manufacturer has applied to the raw reflectivity.  This should\n"
            "always be checked unless you are experimenting with these values."
        )
        ...
        self.tlcorrect.setToolTip(
            "Add a calculated transmission loss corrector to the raw reflectivity.  This should\n"
            "always be checked unless you are experimenting with these values."
        )
        ...
        self.areacorrect.setToolTip(
            "Add a calculated insonified area correction to the raw reflectivity.  This should\n"
            "always be checked unless you are experimenting with these values."
        )
        ...
        self.avg_useexist.setToolTip(
            "Correct backscatter for AVG when creating mosaic, using the AVG Corrector previously generated."
        )
        self.avg_layout.addWidget(self.avg_useexist)

        validator = QtGui.QDoubleValidator(0, 90, 3)
        bstooltip = (
            "Size of the angle bins in the AVG corrector in degrees.  Smaller bins allow for more granular correction\n"
        ...
        mode_tooltip = (
            "There are three different functions within generate new mosaic:\n\n"
        ...
        self.checkbox_controls += [
            ["backscatter_groupbox", self.backscatter_groupbox],
            ["fgcorrect", self.fgcorrect],
            ["tvgcorrect", self.tvgcorrect],
            ["tlcorrect", self.tlcorrect],
            ["areacorrect", self.areacorrect],
            ["avg_groupbox", self.avg_groupbox],
            ["avg_useexist", self.avg_useexist],
            ["surf_options", self.surf_options],
        ]
    ...
    def return_processing_options(self):
        if not self.canceled:
            ...
            opts = {
                ...
                "resolution": rez,
                "process_backscatter": self.backscatter_groupbox.isChecked(),
                "create_mosaic": self.surf_options.isChecked(),
                "angle_varying_gain": self.avg_groupbox.isChecked(),
                "avg_angle": refangle,
                "avg_line": None,
                "avg_bin_size": bsize,
                "overwrite_existing_avg": not self.avg_useexist.isChecked(),
                ...
                "process_backscatter_area_corrected": self.areacorrect.isChecked(),
    ...
    def start_processing(self):
        ...
        if self.output_pth is None:
            ...
            self.status_msg.setText(
                'Error: You must either check "Run Mosaic Generation..." or "Only Selected Lines"'
            )
        elif (
            not self.backscatter_groupbox.isChecked()
            and not self.avg_groupbox.isChecked()
            and not self.surf_options.isChecked()
        ):
            self.status_msg.setText(
                'Error: You must check one of "Process Backscatter", "AVG Corrector", "Create Mosaic"'
            )
        elif not bsizechk:
            self.status_msg.setText(
                'Error: "Bin Size" must be a valid number between 0.0 and 90.0'
            )
        elif not refanglechk:
            self.status_msg.setText(
                'Error: "Reference Angle" must be a valid number between 0.0 and 90.0'
            )
        else:
            self.canceled = False
            self.save_settings()
