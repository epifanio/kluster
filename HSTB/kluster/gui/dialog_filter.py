...
class FilterDialog(SaveStateDialog):
    ...
    def __init__(self, filters: list, filter_descrip: dict, parent=None, settings=None):
        ...
        self.basic_filter_group.setToolTip(
            "Will run the filter on all datasets in the list."
        )
        ...
        self.points_view_filter.setToolTip(
            'Will only run on the points shown in Points View.  Note that you must have the datasets selected (they must be shown in the "Use the following datasets" box)'
        )
        ...
        self.save_to_disk_checkbox.setToolTip(
            "Points View filter allows you to run the filter and only change the points in the Points "
        ...
        self.checkbox_controls = [
            ["basic_filter_group", self.basic_filter_group],
            ["line_filter", self.line_filter],
            ["points_view_filter", self.points_view_filter],
            ["save_to_disk_checkbox", self.save_to_disk_checkbox],
        ]
        self.read_settings()
    ...
    def _event_update_status(self, combobox_text: str):
        ...
        try:
            ...
            self.print(
                f"Unable to load description for filter {combobox_text}",
                logging.WARNING,
            )
        self.status_msg.setText("")
    ...
    def start_filter(self):
        ...
        if self.basic_filter_group.isChecked() and not self.fqpr_inst:
            ...
            not self.basic_filter_group.isChecked()
            and not self.line_filter.isChecked()
            and not self.points_view_filter.isChecked()
        ):
            self.status_msg.setStyleSheet(
                "QLabel { color : " + kluster_variables.error_color + "; }"
            )
            self.status_msg.setText(
                "Error: You must select one of the three filter modes (filter datasets, filter lines, filter points)"
            )
        else:
            self.canceled = False
            self.save_settings()
        ...
        for cntrl in controls:
            ...
            if cntrl_type == "float":
                ...
                raise NotImplementedError(
                    f"AdditionalFilterOptionsDialog: type {cntrl_type} not currently supported"
                )
            ...
            for propkey, propvalue in cntrl_properties.items():
                try:
                    ...
                    self.print(
                        f"AdditionalFilterOptionsDialog: Error: unable to set property {propkey}:{propvalue} on {newcntrl}",
                        logging.ERROR,
                    )
            if cntrl_type == "float":
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            raise ValueError(
                                f"AdditionalFilterOptionsDialog: Unable to return value from {lbl}:{cntrl}"
                            )
        return kwargs
