...
class KlusterProjectTree(QtWidgets.QTreeView):
        ...
        for fq_proj in line_data:
            if fq_proj not in current_fq_proj:
                ...
                child_match = [
                    ...
                    if parent.child(r).text() == fq_proj
                ]
                if child_match:
                    proj_child = child_match[0]
                    if proj_child.rowCount() != len(line_data[fq_proj]):  # new lines
                        tree_lines = [
                            proj_child.child(rw).text()
                            for rw in range(proj_child.rowCount())
                        ]
                        for fq_line in line_data[fq_proj]:
        ...
        for surf in surf_data:
            ...
            if surf not in current_surfs:
                ...
                surf_child = [
                    ...
                    if parent.child(idx).text() == surf
                ][0]
                existing_surf_layer_names = [
                    surf_child.child(idx).text() for idx in range(surf_child.rowCount())
                ]
                no_update_needed = all(
                    [lname in existing_surf_layer_names for lname in layer_names]
                )
                if not no_update_needed:
                    needs_refresh.append(surf)
        if needs_refresh:
            force_removal = {
                k: None for k in surf_data.keys() if k not in needs_refresh
            }
    ...
    def _remove_fqpr_not_in_proj(self, parent, line_data):
        ...
        for remv in needs_removal:
            try:
                ...
                self.print(
                    "_remove_fqpr_not_in_proj: Unable to close {} in project tree, not found in kluster_project_tree.tree_data".format(
                        remv
                    ),
                    logging.ERROR,
                )
            ...
            if idx != -1:
                ...
                tree_idx = [
                    idx
                    for idx in range(parent.rowCount())
                    if parent.child(idx).text() == remv
                ]
                if tree_idx and len(tree_idx) == 1:
                    parent.removeRow(tree_idx[0])
                else:
                    self.print(
                        '_remove_fqpr_not_in_proj: Unable to remove "{}"'.format(remv),
                        logging.ERROR,
                    )
    ...
    def _remove_surf_not_in_proj(self, parent, surf_data):
        ...
        for remv in needs_removal:
            try:
                ...
                self.print(
                    "_remove_surf_not_in_proj: Unable to close {} in project tree, not found in kluster_project_tree.tree_data".format(
                        remv
                    ),
                    logging.ERROR,
                )
            ...
            if idx != -1:
                ...
                if tree_idx and len(tree_idx) == 1:
                    ...
                    self.print(
                        '_remove_surf_not_in_proj: Unable to remove "{}"'.format(remv),
                        logging.ERROR,
                    )
    ...
    def _remove_raster(self, parent, raster_path: str):
        ...
        if raster_path in current_rasters:
            ...
            tree_idx = [
                idx
                for idx in range(parent.rowCount())
                if parent.child(idx).text() == raster_path
            ]
            if tree_idx and len(tree_idx) == 1:
                parent.removeRow(tree_idx[0])
            else:
                self.print(
                    '_remove_raster: Unable to remove "{}"'.format(raster_path),
                    logging.ERROR,
                )
        else:
            self.print(
                '_remove_raster: Unable to find "{}" in project tree'.format(
                    raster_path
                ),
                logging.ERROR,
            )
    ...
    def _remove_vector(self, parent, vector_path: str):
        ...
        if vector_path in current_vectors:
            ...
            tree_idx = [
                idx
                for idx in range(parent.rowCount())
                if parent.child(idx).text() == vector_path
            ]
            if tree_idx and len(tree_idx) == 1:
                parent.removeRow(tree_idx[0])
            else:
                self.print(
                    '_remove_vector: Unable to remove "{}"'.format(vector_path),
                    logging.ERROR,
                )
        else:
            self.print(
                '_remove_vector: Unable to find "{}" in project tree'.format(
                    vector_path
                ),
                logging.ERROR,
            )
    ...
    def _remove_mesh(self, parent, mesh_path: str):
        ...
        if mesh_path in current_meshdata:
            ...
            tree_idx = [
                idx
                for idx in range(parent.rowCount())
                if parent.child(idx).text() == mesh_path
            ]
            if tree_idx and len(tree_idx) == 1:
                parent.removeRow(tree_idx[0])
            else:
                self.print(
                    '_remove_mesh: Unable to remove "{}"'.format(mesh_path),
                    logging.ERROR,
                )
        else:
            self.print(
                '_remove_mesh: Unable to find "{}" in project tree'.format(mesh_path),
                logging.ERROR,
            )
    ...
    def select_multibeam_lines(
        self, line_names: list, clear_existing_selection: bool = True
    ):
        ...
        if line_names:
            for cnt in range(num_containers):
                ...
                for lcnt in range(numlines):
                    ...
                    if lineitem.text() in line_names:
                        ...
                        if (
                            clrfirst
                        ):  # we programmatically select it with ClearAndSelect
            ...
            self.selectionModel().select(
                parent.index(),
                QtCore.QItemSelectionModel.Clear | QtCore.QItemSelectionModel.Rows,
            )
    ...
    def item_selected(self, index):
        ...
        if (
            top_lvl_name in self.categories
        ):  # this is a sub sub item, something like a line name or a surface name
            ...
            mid_lvl_name in self.categories
        ):  # this is a sub item, like a converted fqpr path
            if mid_lvl_name == "Converted":
    ...
    def update_ktree(self, fil):
        for f in fil:
            ...
            if fqpr_entry is None:
                self.print(
                    "update_ktree: Unable to add to Project: {}".format(f),
                    logging.ERROR,
                )
