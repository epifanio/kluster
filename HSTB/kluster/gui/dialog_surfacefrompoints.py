...
class SurfaceFromPointsDialog(SurfaceDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.input_fqpr.setup(
            ...
            app_name="pointfilebrowse",
            supported_file_extension=[".txt", ".csv", ".las", ".laz"],
            multiselect=True,
        ...
        self.basic_surface_group.setTitle(
            "Run surface generation on the following files:"
        )
        ...
        self.inepsg_val.setToolTip(
            "The integer EPSG code for these files, expects a UTM projected 2d or 2d Geographic coordinate system."
        )
        ...
        self.invertref_val.setToolTip(
            'The vertical reference for the files, ex: "Ellipse" or "MLLW"'
        )
        ...
        self.csv_label = QtWidgets.QLabel(
            'Set the Column Numbers for the CSV File (Starting with "1", XYZ is mandatory)'
        )
        ...
        self.text_controls += [
            ...
            ["invertref_val", self.invertref_val],
            ["x_entry", self.x_entry],
            ["y_entry", self.y_entry],
            ["z_entry", self.z_entry],
            ["thu_entry", self.thu_entry],
            ["tvu_entry", self.tvu_entry],
            ["append_output_text", self.append_output_text],
        ]
        self.checkbox_controls += [
            ["thu_include", self.thu_include],
            ["tvu_include", self.tvu_include],
            ["append_grid_checkbox", self.append_grid_checkbox],
            ["new_grid_checkbox", self.new_grid_checkbox],
        ]
        self.read_settings()
    ...
    def return_processing_options(self):
        ...
        if opts is not None:
            ...
            cvals = [
                ...
                self.y_entry.value(),
                self.z_entry.value(),
                self.thu_entry.value(),
                self.tvu_entry.value(),
            ]
            clbls = ["x", "y", "z", "thu", "tvu"]
            cenabled = [
                self.x_include.isChecked(),
                self.y_include.isChecked(),
                self.z_include.isChecked(),
                self.thu_include.isChecked(),
    ...
    def start_processing(self):
        if (self.output_pth is None and self.new_grid_checkbox.isChecked()) or (
            self.append_pth is None and self.append_grid_checkbox.isChecked()
        ):
            self.status_msg.setText("Error: You must insert an output path to continue")
        elif not self.fqpr_inst:
            self.status_msg.setText(
                "Error: You must provide at least one file in the box above"
            )
        elif (
            not self.append_grid_checkbox.isChecked()
            and not self.new_grid_checkbox.isChecked()
        ):
            self.status_msg.setText(
                "Error: You must select either Create New Surface or Add to Existing Surface to proceed."
            )
        elif not self.validate_epsg():
            pass
        elif not self.invertref_val.text():
            self.status_msg.setText(
                "Error: You must provide a vertical reference string for these files"
            )
        elif (
            self.surf_method.currentText() == "CUBE"
            and (
                self.filetype in [".csv", ".txt"]
                and (
                    not self.tvu_include.isChecked() or not self.thu_include.isChecked()
                )
            )
            or (self.filetype in [".las", ".laz"])
        ):
            self.status_msg.setText(
                "Error: Uncertainty must be provided to run the CUBE algorithm (not supported for las data)"
            )
        else:
            self.canceled = False
            self.save_settings()
