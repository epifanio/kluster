...
class FileAnalyzerDialog(SaveStateDialog):
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.start_msg = QtWidgets.QLabel(
            f"Select a raw file to analyze {kluster_variables.supported_sonar + kluster_variables.supported_ppnav}:"
        )
    ...
    def file_browse_two(self, e):
        ...
        if file_path:
            ...
            if self.filetype == "kongsberg_all" and fext != ".all":
                self.print(
                    "Expected .all file, got {}".format(file_path), logging.ERROR
                )
            elif self.filetype == "kongsberg_kmall" and fext != ".kmall":
                self.print(
                    "Expected .kmall file, got {}".format(file_path), logging.ERROR
                )
            elif self.filetype == "reson_s7k" and fext != ".s7k":
                self.print(
                    "Expected .s7k file, got {}".format(file_path), logging.ERROR
                )
            elif self.filetype == "kongsberg_raw" and fext != ".raw":
                self.print(
                    "Expected .raw file, got {}".format(file_path), logging.ERROR
                )
            elif self.filetype in ["applanix_sbet", "applanix_smrmsg"] and fext not in [
                ".out",
                ".sbet",
                "smrmsg",
            ]:
                ...
                if fext == ".all":
                    ...
                    try:
                        ...
                        self.print(
                            f"Not a recognized file type: {self.filename}",
                            logging.ERROR,
                        )
                        self.fileobjecttwo = None
        ...
        if file_path:
            ...
            if fext == ".all":
                ...
                try:
                    ...
                    self.print(
                        f"Not a recognized file type: {self.filename}", logging.ERROR
                    )
                    self.filetype = ""
                    self.fileobject = None
