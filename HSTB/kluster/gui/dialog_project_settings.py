...
geo_datum_descrip = [
    f"{k} = EPSG:{epsg_determinator(k)}"
    for k in kluster_variables.geographic_coordinate_systems
]
proj_datum_descrip = []
...
class ProjectSettingsDialog(SaveStateDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.georef_vertref.setToolTip(
            ...
            + "\n".join(
                [
                    f"{k} = {v}"
                    for k, v in kluster_variables.vertical_references_explanation.items()
                ]
            )
        )
        ...
        self.svmode.setCurrentIndex(
            kluster_variables.cast_selection_methods.index(
                kluster_variables.default_cast_selection_method
            )
        )
        self.svmode.setToolTip(
            f"Determines which sound velocity profiles will be used for each {kluster_variables.ping_chunk_size} pings during sound velocity correction.\n\n"
            + "\n".join(
                [
                    f"{k} = {v}"
                    for k, v in kluster_variables.cast_selection_explanation.items()
                ]
            )
        )
        ...
        designatesurf_ttip = (
            "Optional, if you set this path to an existing Kluster Surface, all new processed data will be added to the surface.\n"
        ...
        self.checkbox_controls = [
            ["infromdata_radio", self.infromdata_radio],
    ...
    def designatesurf_browse(self):
        ...
        msg, designate_surf = RegistryHelpers.GetDirFromUserQT(
            self, RegistryKey="Kluster", Title="Select grid folder", AppName="\\reghelp"
        )
        if designate_surf:
    ...
    def designatesurf_create(self):
        ...
        dlog.line_surface_checkbox.setChecked(
            True
        )  # trick the dialog into succeeding without actually providing any source data
        ...
        if self.epsg_radio.isChecked():
            ...
            try:
                ...
                if not ecrs.is_projected:
                    self.status_msg.setText(
                        f"ERROR: must be a projected CRS, Output EPSG:{epsg}"
                    )
                elif ecrs.coordinate_system.axis_list[0].unit_name not in [
                    "meters",
                    "metre",
                    "metres",
                ]:
                    self.status_msg.setText(
                        f"ERROR: CRS must be in units of meters, found {ecrs.coordinate_system.axis_list[0].unit_name}, Output EPSG:{epsg}"
                    )
                else:
                    self.status_msg.setText("")
            except:
                self.status_msg.setText(
                    f"Unknown Error: Unable to generate new CRS from Output EPSG:{epsg}"
                )
        else:
            self.status_msg.setText("")
        ...
        if self.inepsg_radio.isChecked():
            ...
            try:
                ...
                if ecrs.is_projected:
                    self.status_msg.setText(
                        f"ERROR: must be a Geographic CRS, Input EPSG:{epsg}"
                    )
                elif ecrs.coordinate_system.axis_list[0].unit_name not in [
                    "degree",
                    "degrees",
                    ...
                    self.status_msg.setText(
                        f"ERROR: CRS must be in units of degrees, found {ecrs.coordinate_system.axis_list[0].unit_name}, Input EPSG:{epsg}"
                    )
                else:
                    self.status_msg.setText("")
            except:
                self.status_msg.setText(
                    f"Unknown Error: Unable to generate new CRS from Input EPSG:{epsg}"
                )
        ...
        if curr_vert in kluster_variables.vdatum_vertical_references:
            ...
            if vdatum:
                if os.path.exists(vdatum):
                    if os.path.exists(os.path.join(vdatum, "vdatum.jar")):
                        ...
                        self.status_msg.setText(
                            "Unable to find vdatum.jar at {}".format(vdatum)
                        )
                else:
                    self.status_msg.setText(
                        "Unable to find vdatum folder at {}".format(vdatum)
                    )
            else:
            ...
            if fes_found:
                if fes_grids:
                    ...
                    self.status_msg.setText(
                        "Found Aviso module, but unable to find any supporting grids, is supplementals installed?"
                    )
            else:
                self.status_msg.setText(
                    "Unable to find Aviso module.  Aviso is only available through Pydro."
                )
        else:
            self.status_msg.setText("")
    ...
    def return_processing_options(self):
        ...
        if not self.canceled:
            ...
            if (
                self.new_surface_options
            ):  # update the output path in case the user modified it after "Create"
            ...
            opts = {
                ...
                "input_datum": inepsg,
                "designated_surface": self.designate_surf,
                "new_surface_options": self.new_surface_options,
            }
        else:
            opts = None
        return opts
