...
class SurfaceDialog(SaveStateDialog):
    ...
    def __init__(self, parent=None, title="", settings=None):
        ...
        self.basic_surface_group = QtWidgets.QGroupBox(
            "Run surface generation on the following datasets:"
        )
        ...
        self.surf_method.setToolTip(
            "The algorithm used when gridding, will use this to determine the depth/uncertainty value of the cell"
        )
        ...
        self.grid_type.setToolTip(
            "Single Resolution grids are a single large grid that covers the whole survey area\n"
        ...
        self.single_rez_tile_size.setToolTip(
            "The size of the single resolution tile in meters.  The default size of 1024 meters is a\n"
        ...
        self.single_rez_resolution.addItems(
            [
                "AUTO_depth",
                "AUTO_density",
                "0.25",
        ...
        self.single_rez_resolution.setToolTip(
            "The resolution of the grid within each single resolution tile in meters.  Higher resolution values allow for a more detailed grid,\n"
        ...
        self.variabletile_tile_size.setToolTip(
            "The size of the tile in the variable resolution grid in meters.  The tile is all the same resolution, so this is the\n"
        ...
        self.variabletile_resolution.setToolTip(
            "The resolution of the grid within each variable resolution tile.\n\n"
            "AUTO_depth will follow the NOAA specifications guidance, using the depth in the resolution lookup table:\n\n"
            "{}\n".format(depth_lookup_formatted)
            + "AUTO_density will base the resolution on the density/area of each tile using the following formula:\n\n"
            + "resolution_estimate=squareroot(2 * minimum_points_per_cell * 1.75 / cell_point_density)"
        ...
        self.cube_method_dropdown.setToolTip(
            "Method to use in determining the appropriate hypothesis value.\n"
        ...
        self.cube_ihoorder_dropdown.addItems(
            ["exclusive", "special", "order1a", "order1b", "order2"]
        )
        self.cube_ihoorder_dropdown.setCurrentText("order1a")
        self.cube_ihoorder_dropdown.setToolTip(
            "Sets the fixed and variable Total Vertical Uncertainty components using the different IHO order categories.\n"
        ...
        self.cube_variance_dropdown.setToolTip(
            "Controls the reported uncertainty.\n"
        ...
        self.output_text.setToolTip(
            "Path to the folder that contains the bathygrid instance.  Folder should contain a\n"
        ...
        self.single_rez_resolution.currentTextChanged.connect(
        ...
        self.text_controls = [
            ["method", self.surf_method],
            ["gridtype", self.grid_type],
            ["cube_method_dropdown", self.cube_method_dropdown],
            ["cube_variance_dropdown", self.cube_variance_dropdown],
            ["cube_ihoorder_dropdown", self.cube_ihoorder_dropdown],
            ["singlerez_tilesize", self.single_rez_tile_size],
            ["single_rez_resolution", self.single_rez_resolution],
            ["variabletile_tile_size", self.variabletile_tile_size],
            ["variabletile_resolution", self.variabletile_resolution],
        ]
        self.checkbox_controls = [
            ["basic_surface_group", self.basic_surface_group],
            ["line_surface_checkbox", self.line_surface_checkbox],
        ]
            ...
            if not self.output_path_edited:
                ...
                if curr_opts == "Single Resolution":
                    outpth = os.path.join(
                        self.output_pth,
                        "{}_{}_{}".format(
                            self.basesrgrid_name,
                            self.surf_method.currentText(),
                            self.single_rez_resolution.currentText(),
                        ).lower(),
                    )
                elif curr_opts == "Variable Resolution Tile":
                    outpth = os.path.join(
                        self.output_pth,
                        "vrtilegrid_{}".format(self.surf_method.currentText()).lower(),
                    )
                else:
                    raise NotImplementedError(
                        f"dialog_surface: Unable to autobuild output path from grid type {curr_opts}"
                    )
    ...
    def file_browse(self):
        msg, output_pth = RegistryHelpers.GetDirFromUserQT(
            self,
            RegistryKey="Kluster",
    ...
    def return_processing_options(self):
        if not self.canceled:
            curr_opts = self.grid_type.currentText()
            grid_parameters = {
                "variance_selection": self.cube_variance_dropdown.currentText().lower(),
            ...
            if curr_opts == "Single Resolution":
                ...
                if self.variabletile_resolution.currentText() == "AUTO_depth":
                    ...
                    raise ValueError(
                        "Should not get here, variable rez is only an auto resolution mode operation"
                    )
                ...
                raise ValueError(
                    "dialog_surface: unexpected grid type {}".format(curr_opts)
                )
        else:
            opts = None
        return opts
    ...
    def start_processing(self):
        if self.output_pth is None:
            ...
            not self.line_surface_checkbox.isChecked()
            and not self.basic_surface_group.isChecked()
        ):
            self.status_msg.setText(
                'Error: You must either check "Run Surface Generation..." or "Only Selected Lines"'
            )
        else:
            self.canceled = False
            self.save_settings()
