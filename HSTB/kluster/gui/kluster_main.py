...
from HSTB.kluster.gui.backends._qt import (
    QtGui,
    QtCore,
    QtWidgets,
    Signal,
...
from HSTB.kluster.gui import (
    dialog_vesselview,
    kluster_explorer,
    kluster_project_tree,
    kluster_3dview_v2,
    kluster_output_window,
    kluster_2dview,
    kluster_actions,
    kluster_monitor,
    dialog_daskclient,
    ...
    dialog_export,
    kluster_worker,
    kluster_interactive_console,
    dialog_basicplot,
    dialog_advancedplot,
    dialog_project_settings,
    dialog_export_grid,
    dialog_layer_settings,
    dialog_settings,
    dialog_importppnav,
    dialog_overwritenav,
    dialog_surface_data,
    dialog_about,
    dialog_patchtest,
    dialog_manualpatchtest,
    dialog_managedata,
    dialog_managesurface,
    dialog_reprocess,
    dialog_fileanalyzer,
    dialog_export_tracklines,
    dialog_filter,
    dialog_surfacefrompoints,
    dialog_mosaic,
)
...
settings_translator = {
    "Kluster/debug": {"newname": "debug", "defaultvalue": False},
    ...
    "Kluster/proj_settings_utmval": {
        "newname": "coord_system",
        "defaultvalue": kluster_variables.default_coordinate_system,
    },
    "Kluster/proj_settings_vertref": {
        "newname": "vert_ref",
        "defaultvalue": kluster_variables.default_vertical_reference,
    },
    "Kluster/proj_settings_svmode": {
        "newname": "cast_selection_method",
        "defaultvalue": kluster_variables.default_cast_selection_method,
    },
    "Kluster/proj_settings_designated_surf_path": {
        "newname": "designated_surface",
        "defaultvalue": "",
    },
    "Kluster/layer_settings_background": {
        "newname": "layer_background",
        "defaultvalue": "Default",
    },
    "Kluster/layer_settings_transparency": {
        "newname": "layer_transparency",
        "defaultvalue": "0",
    },
    "Kluster/layer_settings_surfacetransparency": {
        "newname": "surface_transparency",
        "defaultvalue": 0,
    },
    "Kluster/settings_keep_waterline_changes": {
        "newname": "keep_waterline_changes",
        "defaultvalue": True,
    },
    "Kluster/settings_draw_navigation": {
        "newname": "draw_navigation",
        "defaultvalue": "raw",
    },
    "Kluster/settings_enable_parallel_writes": {
        "newname": "write_parallel",
        "defaultvalue": True,
    },
    "Kluster/settings_vdatum_directory": {
        "newname": "vdatum_directory",
        "defaultvalue": "",
    },
    "Kluster/settings_filter_directory": {
        "newname": "filter_directory",
        "defaultvalue": "",
    },
    "Kluster/settings_main_log_file": {"newname": "main_log_file", "defaultvalue": ""},
    "Kluster/settings_auto_processing_mode": {
        "newname": "autoprocessing_mode",
        "defaultvalue": "normal",
    },
    "Kluster/settings_force_coordinate_match": {
        "newname": "force_coordinate_match",
        "defaultvalue": False,
    },
}
...
class KlusterProxyStyle(QtWidgets.QProxyStyle):
    ...
    def styleHint(self, *args, **kwargs):
        if (
            args[0] == QtWidgets.QStyle.SH_ToolTip_WakeUpDelay
        ):  # make tooltips show immediately
            return 0
...
class KlusterMain(QtWidgets.QMainWindow):
    ...
    def __init__(self, app=None, app_library="pyqt5"):
        ...
        self.tree_dock = self.dock_this_widget(
            "Project Tree", "project_dock", self.project_tree
        )
        ...
        self.points_dock = self.dock_this_widget(
            "Points View", "points_dock", self.points_view
        )
        ...
        self.explorer_dock = self.dock_this_widget(
            "Explorer", "explorer_dock", self.explorer
        )

        self.output_window_dock = self.dock_this_widget(
            "Output", "output_window_dock", self.output_window
        )

        self.attribute = kluster_explorer.KlusterAttribution(self)
        self.attribute_dock = self.dock_this_widget(
            "Attribute", "attribute_dock", self.attribute
        )

        self.actions = kluster_actions.KlusterActions(self)
        self.actions_dock = self.dock_this_widget(
            "Actions", "actions_dock", self.actions
        )
        self.actions.update_actions(process_mode=self.intel.autoprocessing_mode)

        self._monitor = kluster_monitor.KlusterMonitor(self)
        self.monitor_dock = self.dock_this_widget(
            "Monitor", "monitor_dock", self._monitor
        )
        self.monitor = self._monitor.widget

        self.console = kluster_interactive_console.KlusterConsole(self)
        self.console_dock = self.dock_this_widget(
            "Console", "console_dock", self.console
        )
        ...
        self.allthreads = [
            self.action_thread,
            self.import_ppnav_thread,
            self.overwrite_nav_thread,
            self.surface_thread,
            self.surface_update_thread,
            self.export_thread,
            self.export_grid_thread,
            self.open_project_thread,
            self.draw_navigation_thread,
            self.draw_surface_thread,
            self.load_points_thread,
            self.patch_test_load_thread,
        ...
        self.project_tree.surface_layer_selected.connect(
            self.tree_surface_layer_selected
        )
        ...
        self.surface_update_thread.tfinished.connect(
            self._kluster_surface_update_results
        )
        ...
        self.export_tracklines_thread.tfinished.connect(
            self._kluster_export_tracklines_results
        )
        ...
        self.draw_navigation_thread.tfinished.connect(
            self._kluster_draw_navigation_results
        )
        ...
        self.patch_test_load_thread.tfinished.connect(
            self._kluster_update_manual_patch_test
        )
        ...
        self._patch = (
            None  # retain patch test dialog object while running it for updates
        )
    ...
    def _load_previously_used_settings(self):
        ...
        if not self.settings.get("vdatum_directory"):
            ...
            if possible_vdatum and os.path.exists(possible_vdatum):
                ...
                self.two_d.vdatum_directory = self.settings[
                    "vdatum_directory"
                ]  # used for the 2d vdatum region display
    ...
    def update_on_file_added(self, fil: Union[str, list] = ""):
        ...
        for (
            f
        ) in fil:  # first pass to weed out a potential project, want to load that first
            fnorm = os.path.normpath(f)
            if os.path.split(fnorm)[1] == "kluster_project.json":
                self.open_project(fnorm)
                fil.remove(f)
                self.debug_print(
                    "project file detected, we can't handle loading a new project and adding data at the same time, if a project is added, halt",
                    logging.WARNING,
                )
                return
        ...
        for f in fil:
            ...
            try:
                ...
                self.print(
                    "Unable to load from file {}, {}".format(f, e), logging.ERROR
                )
                updated_type, new_data, new_project = None, True, None

            if new_project:
                self.debug_print(
                    "user added a data file when there was no project, so we loaded or created a new one",
                    logging.INFO,
                )
                new_fqprs.extend(
                    [
                        fqpr
                        for fqpr in self.project.fqpr_instances.keys()
                        if fqpr not in new_fqprs
                    ]
                )
            if new_data is None:
                ...
                if any(
                    [
                        os.path.exists(os.path.join(f, gname))
                        for gname in allowable_grid_root_names
                    ]
                ):
                    self.debug_print(
                        "Got surfaces that match allowed grid root names: {}".format(
                            allowable_grid_root_names
                        ),
                        logging.INFO,
                    )
        ...
        self.refresh_project(
            new_fqprs,
            new_raster=potential_raster_paths,
            new_vector=potential_vector_paths,
            new_mesh=potential_mesh_paths,
        )
        self.open_project_thread.populate(
            force_add_fqprs=potential_fqpr_paths,
            force_add_surfaces=potential_surface_paths,
        )
        self.open_project_thread.start()

    def refresh_project(
        self, fqpr=None, new_raster=None, new_vector=None, new_mesh=None
    ):
        self.redraw(
            new_fqprs=fqpr,
            add_raster=new_raster,
            add_vector=new_vector,
            add_mesh=new_mesh,
        )
    ...
    def _redraw_remove_surface(self, remove_surface, surface_layer_name):
        if remove_surface is not None:
            ...
            if surface_layer_name == "tiles":
                self.debug_print(
                    "Hiding {} tiles layer".format(remove_surface), logging.INFO
                )
                ...
                for resolution in surf_object.resolutions:
                    if surface_layer_name:
                        self.debug_print(
                            "Hiding {} {} {} layer".format(
                                remove_surface, surface_layer_name, resolution
                            ),
                            logging.INFO,
                        )
                        self.two_d.hide_surface(
                            remove_surface, surface_layer_name, resolution
                        )
                    else:
                        self.debug_print(
                            "Removing all {} {} layers".format(
                                remove_surface, resolution
                            ),
                            logging.INFO,
                        )
    ...
    def _redraw_add_surface(self, add_surface, surface_layer_name):
        if add_surface is not None and surface_layer_name:
            if self.surface_update_thread.isRunning():
                self.print(
                    "Surface is currently updating, please wait until after that process is complete.",
                    logging.WARNING,
                )
                return
            ...
            if surface_layer_name == "tiles":
                self.debug_print(
                    "Trying to show {} {} layer".format(
                        add_surface, surface_layer_name
                    ),
                    logging.INFO,
                )
                ...
                if not shown:
                    self.debug_print(
                        "show didnt work, must need to add the surface instead, loading from disk...",
                        logging.INFO,
                    )
                ...
                for resolution in surf_object.resolutions:
                    self.debug_print(
                        "Trying to show {} {} {} layer".format(
                            add_surface, surface_layer_name, resolution
                        ),
                        logging.INFO,
                    )
                    shown = self.two_d.show_surface(
                        add_surface, surface_layer_name, resolution
                    )
                    if not shown:
                        self.debug_print(
            ...
            if needs_drawing:
                self.print(
                    "Drawing {} - {}, resolution {}".format(
                        add_surface, surface_layer_name, needs_drawing
                    ),
                    logging.INFO,
                )
                self.draw_surface_thread.populate(
                    add_surface, surf_object, needs_drawing, surface_layer_name
                )
    ...
    def _redraw_add_raster(self, add_raster, layer_name):
        if add_raster is not None and layer_name:
            self.debug_print(
                "Trying to show {} {} layer".format(add_raster, layer_name),
                logging.INFO,
            )
            shown = self.two_d.show_raster(add_raster, layer_name)
            if not shown:
                self.debug_print(
                    "show didnt work, must need to add the raster instead, loading from disk...",
                    logging.INFO,
                )
    ...
    def _redraw_remove_raster(self, remove_raster, layer_name):
        if remove_raster is not None:
            if layer_name:
                self.debug_print(
                    "Hiding {} {} layer".format(remove_raster, layer_name), logging.INFO
                )
                self.two_d.hide_raster(remove_raster, layer_name)
            else:
                self.debug_print(
                    "Removing all {} layers".format(remove_raster), logging.INFO
                )
    ...
    def _redraw_add_vector(self, add_vector, layer_name):
        if add_vector is not None and layer_name:
            self.debug_print(
                "Trying to show {} {} layer".format(add_vector, layer_name),
                logging.INFO,
            )
            shown = self.two_d.show_vector(add_vector, layer_name)
            if not shown:
                self.debug_print(
                    "show didnt work, must need to add the vector instead, loading from disk...",
                    logging.INFO,
                )
    ...
    def _redraw_remove_vector(self, remove_vector, layer_name):
        if remove_vector is not None:
            if layer_name:
                self.debug_print(
                    "Hiding {} {} layer".format(remove_vector, layer_name), logging.INFO
                )
                self.two_d.hide_vector(remove_vector, layer_name)
            else:
                self.debug_print(
                    "Removing all {} layers".format(remove_vector), logging.INFO
                )
    ...
    def _redraw_add_mesh(self, add_mesh, layer_name):
        if add_mesh is not None and layer_name:
            self.debug_print(
                "Trying to show {} {} layer".format(add_mesh, layer_name), logging.INFO
            )
            shown = self.two_d.show_mesh(add_mesh, layer_name)
            if not shown:
                self.debug_print(
                    "show didnt work, must need to add the mesh instead, loading from disk...",
                    logging.INFO,
                )
    ...
    def _redraw_remove_mesh(self, remove_mesh, layer_name):
        if remove_mesh is not None:
            if layer_name:
                self.debug_print(
                    "Hiding {} {} layer".format(remove_mesh, layer_name), logging.INFO
                )
                self.two_d.hide_mesh(remove_mesh, layer_name)
            else:
                self.debug_print(
                    "Removing all {} layers".format(remove_mesh), logging.INFO
                )
    ...
    def redraw(
        ...
        self.project_tree.refresh_project(
            proj=self.project,
    ...
    def load_console_fqpr(self, pth):
        ...
        self.console.runCmd(
            'data = reload_data(r"{}", skip_dask=True)'.format(absolute_fqpath)
        )
        self.console.runCmd("first_system = data.multibeam.raw_ping[0]")
        self.console.runCmd("att = data.multibeam.raw_att")
        self.console.runCmd(
            '# try plotting surface soundspeed, "first_system.soundspeed.plot()"'
        )
    ...
    def show_layer_properties(self, layertype: str, pth: str):
        ...
        if layertype not in translator_layer_type:
            self.print(
                f"show_layer_properties: got unrecognized layer category: {layertype}",
                logging.ERROR,
            )
            return
    ...
    def kluster_vessel_offsets(self):
        ...
        self.vessel_win.converted_xyzrph_modified.connect(
            self.update_offsets_vesselwidget
        )

        if vessel_file:
            self.debug_print(
                "Load offsets/angles from designated vessel file {}".format(
                    vessel_file
                ),
                logging.INFO,
            )
            ...
            fqpr = self.project.fqpr_instances[
                self.project.path_relative_to_project(fqprs[0])
            ]
            self.debug_print(
                "Load offsets/angles from selected container {}".format(fqprs[0]),
                logging.INFO,
            )
            vess_xyzrph = convert_from_fqpr_xyzrph(
                fqpr.multibeam.xyzrph,
                fqpr.multibeam.raw_ping[0].sonartype,
    ...
    def regenerate_offsets_actions(self, is_modified: bool):
        ...
        if vessel_file:
            self.debug_print(
                "Regenerating processing actions on new vessel file {}".format(
                    self.project.vessel_file
                ),
                logging.INFO,
            )
    ...
    def update_offsets_vesselwidget(self, vess_xyzrph: dict):
        ...
        xyzrph, sonar_type, system_identifiers, source = convert_from_vessel_xyzrph(
            vess_xyzrph
        )
        for cnt, sysident in enumerate(system_identifiers):
            self.debug_print(
                "Searching for multibeam containers that match serial number {}".format(
                    sysident
                ),
                logging.INFO,
            )
            ...
            for fqname, fq in self.project.fqpr_instances.items():
                if fqname == matching_fq:
                    self.print(
                        "Updating xyzrph record for {}".format(fqname), logging.INFO
                    )
                    (
                        identical_offsets,
                        identical_angles,
                        ...
                        data_matches,
                        new_waterline,
                    ) = compare_dict_data(fq.multibeam.xyzrph, xyzrph[cnt])
                    fq.write_attribute_to_ping_records({"xyzrph": xyzrph[cnt]})
                    fq.multibeam.xyzrph.update(xyzrph[cnt])
                    if (
                        not identical_angles
                    ):  # if the angles changed then we have to start over at converted status
                        if fq.multibeam.raw_ping[0].current_processing_status > 0:
                            fq.write_attribute_to_ping_records(
                                {"current_processing_status": 0}
                            )
                            fq.logger.info(
                                "Setting processing status to 0, starting over at computing orientation"
                            )
                    elif (
                        not identical_offsets or new_waterline is not None
                    ):  # have to re-soundvelocitycorrect
                        ...
                        if fq.multibeam.raw_ping[0].current_processing_status >= 5:
                            ...
                            fq.logger.info(
                                "Setting processing status to 4, starting over at uncertainty calculation"
                            )
    ...
    def reprocess_fqpr(self):
        ...
        if fqprs:
            ...
            if current_status == 0:
                self.print(
                    "reprocess_fqpr: Unable to reprocess converted data, current process is already at the beginning (conversion)",
                    logging.ERROR,
                )
                return
            ...
            if dlog.exec_():
                if not dlog.canceled:
                    ...
                    if newstatus is not None:
                        fq.write_attribute_to_ping_records(
                            {"current_processing_status": newstatus}
                        )
                        fq.logger.info(
                            f"Setting processing status to {newstatus}, starting over at computing {kluster_variables.status_lookup[newstatus + 1]}"
                        )
                        self.project.refresh_fqpr_attribution(
                            fqprs[0], relative_path=True
                        )
                        fq.multibeam.reload_pingrecords(skip_dask=fq.client is None)
                        self.intel.regenerate_actions()
                    else:
                        self.print(
                            "reprocess_fqpr: new status is None, unable to set status",
                            logging.ERROR,
                        )
                else:
    ...
    def kluster_basic_plots(self):
        ...
        self.basicplots_win.setWindowFlags(
            self.basicplots_win.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
        )
    ...
    def kluster_advanced_plots(self):
        ...
        if self.project.surface_instances:
            ...
            if os.path.exists(default_plots):
                default_plots = os.path.join(
                    os.path.dirname(first_surf),
                    "accuracy_test_{}".format(datetime.now().strftime("%Y%m%d_%H%M%S")),
                )
        ...
        self.advancedplots_win.setWindowFlags(
            self.advancedplots_win.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
        )
        ...
        if not self.action_thread.error:
            if self.action_thread.action_type != "gridding":
                ...
                if fqpr is not None:
                    ...
                    self.intel.update_intel_for_action_results(
                        action_type=self.action_thread.action_type
                    )
                    ...
                    self.print(
                        "Error running action {}".format(
                            self.action_thread.action_type
                        ),
                        logging.ERROR,
                    )
            else:
                if self.action_thread.result:
                    ...
                    if fq_surf is not None:
                        relpath_surf = self.project.path_relative_to_project(
                            os.path.normpath(fq_surf.output_folder)
                        )
                        self.update_surface(
                            relpath_surf, fq_surf, only_resolutions=oldrez
                        )
                else:
                    self.print(
                        "Error running action {}".format(
            ...
            self.print(
                "Error running action: {}".format(self.action_thread.action_type),
                logging.ERROR,
            )
            self.print(self.action_thread.exceptiontxt, logging.INFO)
            self.print(
                "kluster_action: no data returned from action execution", logging.INFO
            )
            self.intel.update_intel_for_action_results(
                action_type=self.action_thread.action_type
            )
            self.set_auto_processing(
                False
            )  # turn off auto processing if an action fails
    ...
    def kluster_overwrite_nav(self):
        ...
        if not self.no_threads_running():
            self.print(
                "Processing is already occurring.  Please wait for the process to finish",
                logging.WARNING,
            )
            cancelled = True
        else:
                ...
                if opts is not None and not dlog.canceled:
                    ...
                    for fq in fqprs:
                        ...
                        if relfq not in self.project.fqpr_instances:
                            self.print(
                                "Unable to find {} in currently loaded project".format(
                                    relfq
                                ),
                                logging.ERROR,
                            )
        ...
        if cancelled:
            self.print(
                "kluster_import_navigation: Processing was cancelled", logging.INFO
            )
    ...
    def manual_patch_test(self, e):
        ...
        if not self.no_threads_running():
            ...
            (
                ...
                linenames,
                time_segments,
            ) = self.points_view.return_lines_and_times()
            self.debug_print(
                "Using data from points view by system and time\nSystems:{}\nTime Segments{}".format(
                    systems, time_segments
                ),
                logging.INFO,
            )
            if systems:
                datablock = self.project.retrieve_data_for_time_segments(
                    systems, time_segments
                )
                if datablock:
                    ...
                    if dlog_patch.exec_():
                        if dlog_patch.canceled:
                            ...
                            if final_datablock:
                                ...
                                self._manpatchtest = None
                                self._manpatchtest = (
                                    dialog_manualpatchtest.ManualPatchTestWidget(
                                        prefixes=prefixes
                                    )
                                )
                                self._manpatchtest.new_offsets_angles.connect(
                                    self._update_manual_patch_test
                                )
                                self._manpatchtest.patchdatablock = final_datablock
                                self._manpatchtest.populate(
                                    vessel_file_name,
                                    ",".join(final_datablock[1]),
                                    final_datablock[2],
                                    final_datablock[3],
                                    ",".join(final_datablock[4]),
                                    ",".join(final_datablock[5]),
                                    roll,
                                    pitch,
                                    heading,
                                    xlever,
                                ...
                                self._manpatchtest.setWindowFlags(
                                    self._manpatchtest.windowFlags()
                                    | QtCore.Qt.WindowStaysOnTopHint
                                )
                                self._manpatchtest.show()
                            else:
                                self.print(
                                    "Patch Test: no data selected", logging.ERROR
                                )
                else:
                    self.print(
                        "Patch Test: no data selected for running in Patch Test utility",
                        logging.ERROR,
                    )
            else:
    ...
    def _update_manual_patch_test(self):
        ...
        if self._manpatchtest.patchdatablock is not None:
            roll, pitch, heading = (
                self._manpatchtest.roll,
                self._manpatchtest.pitch,
                self._manpatchtest.heading,
            )
            xlever, ylever, zlever = (
                self._manpatchtest.x_lever,
                self._manpatchtest.y_lever,
                self._manpatchtest.z_lever,
            )
            fqprs, tstamps, timesegments, headindex = (
                self._manpatchtest.patchdatablock[0],
                self._manpatchtest.patchdatablock[5],
                self._manpatchtest.patchdatablock[6],
                int(self._manpatchtest.patchdatablock[7]),
            )
            ...
            try:
                ...
                self.print(
                    "Unable to find vdatum_directory attribute for patch test processing",
                    logging.WARNING,
                )
                vdatum_directory = None

            self.patch_test_load_thread.populate(
                fqprs,
                ...
                tstamps,
                serial_num,
                self.load_points_thread.polygon,
                vdatum_directory,
            )
            self.patch_test_load_thread.start()
        else:
            self.print(
                "Unable to load the data for updating the manual patch test",
                logging.ERROR,
            )
    ...
    def _kluster_update_manual_patch_test(self):
        ...
        if results and not self.patch_test_load_thread.error:
            for cnt, (fq, rslt) in enumerate(
                zip(self.patch_test_load_thread.fqprs, results)
            ):
                ...
                self.debug_print(
                    "Loading points from {} : {}".format(newid, linenames), logging.INFO
                )
                self.points_view.remove_points(system_id=newid + "_" + str(headindex))
                self.points_view.add_points(
                    head,
                    x,
                    y,
                    z,
    ...
    def kluster_auto_patch_test(self):
        ...
        if cancelled:
            self.print(
                "kluster_auto_patch_test: Processing was cancelled", logging.INFO
            )
        self._patch = None
    ...
    def _kluster_filter_dialog(self, filter_list, filter_module, fqprs, filter_descrip):
        ...
        if dlog.exec_():
            if not dlog.canceled:
                ...
                if (
                    points_filter_mode
                ):  # we need to get the time/beam of all points in Points View to subset the Fqpr
                    savetodisk = (
                        dlog.save_to_disk_checkbox.isChecked()
                    )  # can skip saving to disk for Poinst View filter
                    pointtime, pointbeam = self.points_view.return_array(
                        "pointtime"
                    ), self.points_view.return_array("beam")
                else:
                    savetodisk = True
                ...
                if filter_name and not dlog.canceled:
                    ...
                    filter_controls = filter_module.return_optional_filter_controls(
                        filter_name
                    )
                else:
                    cancelled = True
    ...
    def _kluster_additional_filter_dialog(self, filter_controls, filter_name):
        cancelled = False
        if filter_controls:
            add_dlog = dialog_filter.AdditionalFilterOptionsDialog(
                title=filter_name, controls=filter_controls, parent=self
            )
            if add_dlog.exec_():
    ...
    def kluster_filter(self):
        ...
        if not self.no_threads_running():
            ...
            try:
                ...
                self.print(
                    "Error: kluster_filter no loaded converted data found to filter, unable to initialize filter list.",
                    logging.ERROR,
                )
                return

            result = self._kluster_filter_dialog(
                filter_list, filter_module, fqprs, filter_descrip
            )
            (
                basic_filter_mode,
                line_filter_mode,
            ...
            if not cancelled:
                kwargs, cancelled = self._kluster_additional_filter_dialog(
                    filter_controls, filter_name
                )
                if not cancelled:
                    fq_chunks = []
                    if (
                        points_filter_mode
                    ):  # still need these even if you are filtering the points view only, we can just grab them.
                        fqprs = [
                            self.project.absolute_path_from_relative(pth)
                            for pth in self.points_view.return_fqpr_paths()
                        ]
                    for fq in fqprs:
                        relfq = self.project.path_relative_to_project(fq)
                        if relfq not in self.project.fqpr_instances:
                            self.print(
                                "kluster_filter: Unable to find {} in currently loaded project".format(
                                    relfq
                                ),
                                logging.ERROR,
                            )
                        ...
                        if points_filter_mode:
                            if relfq not in pointtime or relfq not in pointbeam:
                                self.print(
                                    f"kluster_filter: {relfq} is not currently being used in Points View, skipping filter",
                                    logging.WARNING,
                                )
                                continue
                            fq_chunks.append(
                                [fq_inst, pointtime[relfq], pointbeam[relfq], relfq]
                            )
                        else:
                            fq_chunks.append([fq_inst, relfq])
                        self.filter_thread.populate(
                            fq_chunks,
                            linenames,
                            filter_name,
                            basic_filter_mode,
    ...
    def _kluster_filter_results(self):
        ...
        if self.filter_thread.mode == "points":
            self.debug_print(
                "Updating points view data with filter results", logging.INFO
            )
            ...
            for cnt, (fq, subset_time, subset_beam, fqname) in enumerate(
                self.filter_thread.fq_chunks
            ):
                ...
                for fcnt, ninfo in enumerate(fqinfo):
                    ...
                    pointsview_timebeam = np.column_stack(
                        [
                            base_points_time[matches_sonar],
                            base_points_beam[matches_sonar],
                        ]
                    )
                    results_timebeam = np.column_stack([subset_time, subset_beam])
                    chk = np.intersect1d(
                        pointsview_timebeam.view(dtype=np.complex128),
                        results_timebeam.view(dtype=np.complex128),
                        return_indices=True,
                        assume_unique=True,
                    )
                    ...
                    time_sort = np.argsort(
                        np.argsort(
                            pointsview_timebeam.view(dtype=np.complex128).ravel()
                        )
                    )
                    results_indices = results_indices[time_sort]
                    # now replace with the new values in the correct order
                    base_points_view_status[matches_sonar] = ninfo[fqheadsel][
                        results_indices
                    ]
    ...
    def kluster_surface_generation(self):
        ...
        if not self.no_threads_running():
            ...
            if dlog.exec_():
                ...
                if opts is not None and not cancelled:
                    ...
                    if (
                        dlog.line_surface_checkbox.isChecked()
                    ):  # we now subset the fqpr instances by lines selected
                        fqprspaths, fqprs = self.return_selected_fqprs(
                            subset_by_line=True, concatenate=False
                        )
                        for fq in fqprs:
                            fq_chunks.extend([fq])
                    else:
                        for fq in fqprs:
                            try:
                                ...
                                self.print(
                                    "No project loaded, you must load some data before generating a surface",
                                    logging.ERROR,
                                )
                                return
                            if relfq not in self.project.fqpr_instances:
        ...
        if cancelled:
            self.print(
                "kluster_surface_generation: Processing was cancelled", logging.INFO
            )
    ...
    def kluster_mosaic_generation(self):
        ...
        if cancelled:
            self.print(
                "kluster_mosaic_generation: Processing was cancelled", logging.INFO
            )
    ...
    def _kluster_mosaic_generation_results(self):
        ...
        if pbscatter or avgon:
            ...
            for fqpr in self.mosaic_thread.fqpr_instances:
                self.project.refresh_fqpr_attribution(
                    fqpr.output_folder, relative_path=False
                )
    ...
    def kluster_surface_update(self):
        if not self.no_threads_running():
            ...
            if surfs:
                surf = self.project.surface_instances[
                    self.project.path_relative_to_project(surfs[0])
                ]
                surf_version = [int(vnumber) for vnumber in surf.version.split(".")]
                if (
                    surf_version[0] < 1
                    or (surf_version[0] == 1 and surf_version[1] < 3)
                    or (
                        surf_version[0] == 1
                        and surf_version[1] == 3
                        and surf_version[2] < 5
                    )
                ):
                    self.print(
                        "kluster_surface_update: surface update received a rework in bathygrid 1.3.5, grid created prior to that cannot be updated in Kluster.",
                        logging.ERROR,
                    )
                    return
                ...
                ) = self.project.return_surface_containers(
                    surfs[0], relative_path=False
                )
                dlog = dialog_surface_data.SurfaceDataDialog(
                    parent=self, title=surf.output_folder
                )
                ...
                if dlog.exec_():
                    ...
                    (
                        ...
                        remove_lines,
                        opts,
                    ) = dlog.return_processing_options()
                    add_fqpr = []
                    if not cancelled:
                        if add_container:
                            for fqpr_inst in self.project.fqpr_instances.values():
                                fname = os.path.split(
                                    fqpr_inst.multibeam.raw_ping[0].output_path
                                )[1]
                            ...
                            if add_container:
                                self.print(
                                    "kluster_surface_update: {} must be loaded in Kluster for it to be added to the surface.".format(
                                        add_container
                                    ),
                                    logging.ERROR,
                                )
                                return
                        self.output_window.clear()
                        self.surface_update_thread.populate(
                            surf,
                            add_fqpr,
                            add_lines,
                            remove_container,
                        ...
                        self.print(
                            "kluster_surface_update: Processing was cancelled",
                            logging.INFO,
                        )
    ...
    def _kluster_surface_update_results(self):
        ...
        if fq_surf is not None and not self.surface_update_thread.error:
            ...
            self.update_surface(
                relpath_surf,
                fq_surf,
                only_resolutions=self.surface_update_thread.all_resolutions,
            )
        ...
        if not self.no_threads_running():
            ...
            if surfs:
                ...
                if relsurf in self.project.surface_instances:
                    dlog.update_vert_ref(
                        self.project.surface_instances[relsurf].vertical_reference
                    )
                    dlog.update_isbackscatter(
                        self.project.surface_instances[relsurf].is_backscatter
                    )
            cancelled = False
            if dlog.exec_():
                if not dlog.canceled:
                    ...
                    if relsurf in self.project.surface_instances:
                        ...
                        self.export_grid_thread.populate(
                            surf_inst, export_format, output_path, z_pos_up, opts
                        )
                        self.export_grid_thread.start()
                    else:
                        self.print(
                            "kluster_grid_export: Unable to load from {}".format(surf),
                            logging.ERROR,
                        )
                else:
                    cancelled = True
    ...
    def _kluster_export_results(self):
        ...
        self.export_thread.populate(
            None, None, [], "", False, "comma", "xyz", False, False, True, False, False
        )
        ...
        if not self.no_threads_running():
            ...
            if dlog.exec_():
                ...
                if not dlog.canceled:
                    ...
                    if fq_chunks:
                        self.output_window.clear()
                        self.export_tracklines_thread.populate(
                            fq_chunks,
                            linenames,
                            export_type,
                            basic_export_mode,
    ...
    def new_project(self, directory: str):
        ...
        if (
            self.settings
        ):  # set_settings will set the project settings and save the project
            self.project.set_settings(self.settings.copy())
    ...
    def _kluster_open_project_results(self):
        ...
        if not self.open_project_thread.error:
            for new_fq in self.open_project_thread.new_fqprs:
                ...
                if already_in:
                    self.print(
                        "{} already exists in project".format(new_fq.output_folder),
                        logging.WARNING,
                    )
            ...
            self.redraw(
                new_fqprs=[
                    self.project.path_relative_to_project(fq.output_folder)
                    for fq in self.open_project_thread.new_fqprs
                ]
            )
        else:
    ...
    def _kluster_draw_navigation_results(self):
        ...
        if not self.draw_navigation_thread.error:
            ...
            for ln in self.draw_navigation_thread.line_data:
                self.two_d.add_line(
                    ln,
                    self.draw_navigation_thread.line_data[ln][0],
                    self.draw_navigation_thread.line_data[ln][1],
                )
            self.two_d.set_extents_from_lines()
        else:
            self.print(
                "Error drawing lines from {}".format(
                    self.draw_navigation_thread.new_fqprs
                ),
                logging.ERROR,
            )
    ...
    def _kluster_draw_surface_results(self):
        ...
        if not self.draw_surface_thread.error:
            ...
            if self.draw_surface_thread.surface_layer_name == "tiles":
                if self.draw_surface_thread.surface_data:
                    ...
                    trans = Transformer.from_crs(
                        CRS.from_epsg(self.draw_surface_thread.surf_object.epsg),
                        CRS.from_epsg(self.two_d.epsg),
                        always_xy=True,
                    )
                ...
                for surf_resolution in self.draw_surface_thread.surface_data:
                    for surflayername in self.draw_surface_thread.surface_data[
                        ...
                        data = self.draw_surface_thread.surface_data[surf_resolution][
                            surflayername
                        ][0]
                        geo_transform = self.draw_surface_thread.surface_data[
                            surf_resolution
                        ][surflayername][1]
                        self.two_d.add_surface(
                            [
                                surf_path,
                                surflayername,
                                data,
            ...
            self.print(
                "Error drawing surface {}".format(
                    self.draw_surface_thread.surface_path
                ),
                logging.ERROR,
            )
    ...
    def open_dask_dashboard(self):
        ...
        if not self.project.skip_dask:
            ...
            self.print(
                'Unable to open Dask dashboard when in "No Client" mode', logging.ERROR
            )
    ...
    def open_youtube_playlist(self):
        ...
        webbrowser.open_new(
            r"https://www.youtube.com/playlist?list=PLrjCvP_J9AA_memBs2ZyKXGHG1AMx0GWx"
        )
    ...
    def start_dask_client(self):
        ...
        if dlog.exec_():
            ...
            if dlog.canceled:
                ...
                self.print(
                    "start_dask_client: no client started successfully", logging.ERROR
                )
    ...
    def set_project_settings(self):
        ...
        dlog = dialog_project_settings.ProjectSettingsDialog(
            parent=self, settings=self.settings_object
        )
            ...
            if "designated_surface" in settings and settings["designated_surface"]:
                try:
                    if new_surface_options:
                        bg = kluster_worker.generate_new_surface(
                            None, **new_surface_options
                        )
                    elif (
                        self.project.path
                        and self.project.path_relative_to_project(
                            settings["designated_surface"]
                        )
                        in self.project.surface_instances
                    ):
                        bg = None  # no need to create or reload, it is already in the project
                    else:
                        bg = kluster_worker.reload_surface(
                            settings["designated_surface"]
                        )
                    if bg:
                    ...
                    self.print(
                        f'set_project_settings: Unable to designate surface {settings["designated_surface"]}',
                        logging.ERROR,
                    )
                    settings["designated_surface"] = ""
    ...
    def set_layer_settings(self):
        ...
        if dlog.exec_() and not dlog.canceled:
            ...
            self.two_d.set_background(
                self.settings["layer_background"], self.settings["layer_transparency"]
            )
    ...
    def set_settings(self):
        ...
        if dlog.exec_() and not dlog.canceled:
            ...
            redraw = False
            if (
                "draw_navigation" in self.settings
                and settings["draw_navigation"] != self.settings["draw_navigation"]
            ):
                self.print(
                    f'regenerating all tracklines with draw_navigation={settings["draw_navigation"]}',
                    logging.INFO,
                )
                redraw = True
            self.settings.update(settings)
    ...
    def set_debug(self, check_state: bool):
        ...
        if help_menu:
            ...
            debugaction = [
                mn for mn in help_menu.menu().actions() if mn.text() == "Debug"
            ]
            if debugaction:
                ...
                self.print(
                    "Warning: Can not find the Debug action to set debug control!",
                    logging.WARNING,
                )
        else:
            self.print(
                "Warning: Can not find the help menu to set debug control!",
                logging.WARNING,
            )
    ...
    def set_dark_mode(self, check_state: bool):
        ...
        if check_state:
            try:
                self.app.setStyleSheet(
                    qdarkstyle.load_stylesheet(qt_api=self.app_library)
                )
                ...
                self.print(
                    "Unable to set qdarkstyle style sheet for app library {}".format(
                        self.app_library
                    ),
                    logging.ERROR,
                )
        ...
        if view_menu:
            ...
            darkaction = [
                mn for mn in view_menu.menu().actions() if mn.text() == "Dark Mode"
            ]
            if darkaction:
                ...
                self.print(
                    "Warning: Can not find the Dark Mode action to set dark mode control!",
                    logging.WARNING,
                )
        else:
            self.print(
                "Warning: Can not find the view menu to set dark mode control!",
                logging.WARNING,
            )
    ...
    def tree_fqpr_selected(self, converted_pth):
        ...
        self.two_d.reset_line_colors()
        self.explorer.clear_explorer_data()
        linenames = self.project.return_project_lines(
            proj=os.path.normpath(converted_pth)
        )
        attrs = self.project.fqpr_attrs[converted_pth]
        filtered_attrs = {
            a: attrs[a]
            for a in attrs.keys()
            if a not in kluster_variables.hidden_fqpr_attributes
        }
    ...
    def tree_surf_selected(self, converted_pth):
        ...
        filtered_attrs = {
            a: attrs[a]
            for a in attrs.keys()
            if a not in kluster_variables.hidden_grid_attributes
        }
        combined_source = {}
        remove_keys = []
        for ky, val in filtered_attrs.items():
            if ky[:6] == "source":
                ...
                if (
                    len(splitky) != 2
                ):  # this must be pre bathygrid 1.3.5 where we started combining container name and line name as the key
                    ...
                    if contname not in combined_source:
                        ...
                        for mline in val["multibeam_lines"]:
                            if (
                                mline
                                not in combined_source[contname]["multibeam_lines"]
                            ):
                                combined_source[contname]["multibeam_lines"] += [mline]
        for ky in remove_keys:
            filtered_attrs.pop(ky)
        for ky in combined_source.keys():
            combined_source[ky]["multibeam_lines"] = sorted(
                combined_source[ky]["multibeam_lines"]
            )
    ...
    def select_lines_by_name(self, linenames: list):
        ...
        self.project_tree.select_multibeam_lines(
            linenames, clear_existing_selection=True
        )
        if not linenames:
            self.two_d.reset_line_colors()
            self.explorer.clear_explorer_data()
    ...
    def _kluster_load_points_results(self):
        ...
        if not self.load_points_thread.error:
            ...
            for fqpr_name, pointdata in points_data.items():
                ...
                if vert_ref and new_vert_ref != vert_ref:
                    self.print(
                        f"New vertical reference {new_vert_ref} doesn't match current vertical reference {vert_ref}, using {vert_ref} to determine z sign convention",
        ...
        self.load_points_thread.populate(
            polygon=self.load_points_thread.polygon,
            azimuth=self.load_points_thread.azimuth,
        )
    ...
    def show_points_in_explorer(
        ...
        self.explorer.populate_explorer_with_points(
            point_index, linenames, point_times, beam, x, y, z, tvu, status, id
        )

    def set_pointsview_points_status(
        self, new_status: Union[np.array, int, str, float] = 2
    ):
        """
            ...
            for fqpr_name in selected_points:
                ...
                if isinstance(new_status, dict):
                    fqpr.set_variable_by_filter(
                        "detectioninfo", new_status[fqpr_name], sel_points_idx
                    )
                else:
                    fqpr.set_variable_by_filter(
                        "detectioninfo", new_status, sel_points_idx
                    )
                fqpr.write_attribute_to_ping_records(
                    {"_soundings_last_cleaned": datetime.utcnow().strftime("%c")}
                )
    ...
    def setup_widgets(self):
        ...
        self.splitDockWidget(
            self.explorer_dock, self.attribute_dock, QtCore.Qt.Horizontal
        )
        ...
        self.resizeDocks(
            horiz_docks,
            [int(window_width * 0.2), int(window_width * 0.7), int(window_width * 0.2)],
    ...
    def _action_file_analyzer(self):
        ...
        self._fileanalyzer.setWindowFlags(
            self._fileanalyzer.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
        )
    ...
    def _action_filemenu_add_files(self):
        ...
        """
        msg, fil = RegistryHelpers.GetFilenameFromUserQT(
            self,
            RegistryKey="kluster",
            Title="Add any data file (multibeam, sbet, svp, etc.)",
            AppName="klusterproj",
            bMulti=True,
            bSave=False,
            fFilter="all files (*.*)",
    ...
    def _action_filemenu_add_converted(self):
        ...
        """
        msg, folder = RegistryHelpers.GetDirFromUserQT(
            self,
            RegistryKey="kluster",
            Title="Add any Kluster generated converted data folder",
            AppName="klusterproj",
        )
        if msg:
    ...
    def _action_filemenu_add_surface(self):
        ...
        msg, folder = RegistryHelpers.GetDirFromUserQT(
            self,
            RegistryKey="kluster",
            Title="Add any Kluster generated surface data folder",
            AppName="klusterproj",
        )
        if msg:
            self.update_on_file_added(folder)
    ...
    def _action_new_project(self):
        ...
        msg, folder = RegistryHelpers.GetDirFromUserQT(
            self,
            RegistryKey="kluster",
            Title="Select folder to create a new project from",
            AppName="klusterproj",
        )
        if msg:
    ...
    def _action_new_vessel_file(self):
        if self.project.path is not None:
            default_vessel_file = os.path.join(
                os.path.dirname(self.project.path), "vessel_file.kfc"
            )
            msg, fil = RegistryHelpers.GetFilenameFromUserQT(
                self,
                RegistryKey="kluster",
                Title="New Vessel File",
                AppName="klusterproj",
                bMulti=False,
                bSave=True,
            ...
            self.print(
                "Build a new project or open an existing project before creating a vessel file",
                logging.ERROR,
            )
    ...
    def _action_open_vessel_file(self):
        if self.project.path is not None:
            ...
            self.print(
                "Build a new project or open an existing project before opening a vessel file",
                logging.ERROR,
            )
    ...
    def _action_show_docs(self):
        ...
        """
        doc_html = os.path.join(
            os.path.dirname(kluster_init_file), "docbuild", "index.html"
        )
        if os.path.exists(doc_html):
            webbrowser.open_new(doc_html)
        else:
            self.print(
                "Unable to find documentation at {}".format(doc_html), logging.ERROR
            )
    ...
    def return_selected_fqprs(
        self, subset_by_line: bool = False, concatenate: bool = True
    ):
        """
        ...
        if subset_by_line and linedict:
            fqpr_paths, fqpr_loaded = self.project.get_fqprs_by_paths(
                fqprs, linedict, concatenate=concatenate
            )
        else:
...
def main():
    ...
    if ispyinstaller:
        ...
        kluster_icon = os.path.join(
            curdir, "HSTB", "kluster", "images", "kluster_img.ico"
        )
    else:
        kluster_dir = os.path.dirname(kluster_init_file)
    ...
    if qgis_enabled:
        ...
        if ispyinstaller:
            ...
            if sys.platform == "linux":
                ...
                processing_dir = os.path.join(
                    os.path.dirname(found_path), "python", "plugins", "processing"
                )
            else:
        ...
        try:
            ...
            print(
                f"WARNING: QGIS - Can't find plugin directory at {plugin_dir}, is pyinstaller={ispyinstaller}"
            )
        try:
            assert os.path.exists(prefix_dir)
        except:
            print(
                f"WARNING: QGIS - Can't find prefix directory at {prefix_dir}, is pyinstaller={ispyinstaller}"
            )
        try:
            assert os.path.exists(processing_dir)
        except:
            print(
                f"WARNING: QGIS - Can't find processing directory at {processing_dir}, is pyinstaller={ispyinstaller}"
            )
