...
class BasicPlotDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.plot_lookup = {
            2: [
                ...
                "Line - Mean",
                "Line - Nadir",
                "Line - Port Outer Beam",
                "Line - Starboard Outer Beam",
            ],
        ...
        self.custom_plot_lookup = {
            ...
            "sound_velocity_profiles": [
                "Plot Profiles",
                "Surface SV Comparison",
                "Profile Map",
            ],
            "sound_velocity_correct": [
                "2d scatter, color by depth",
                "2d scatter, color by sector",
                "3d scatter, color by depth",
                "3d scatter, color by sector",
            ],
            "georeferenced": [
                "2d scatter, color by depth",
                "2d scatter, color by sector",
            ...
            "animations": [
                "Uncorrected Beam Vectors",
                "Corrected Beam Vectors",
                "Vessel Orientation",
            ],
        }
    ...
    def load_datasets(self):
        ...
        if self.fqpr is not None:
            ...
            if self.fqpr.multibeam.raw_ping:
                ...
                self.print(
                    "No multibeam dataset(s) found in {}".format(
            ...
            if self.fqpr.multibeam.raw_att:
                ...
                self.print(
                    "No attitude dataset found in {}".format(
                        self.fqpr.multibeam.converted_pth
                    ),
                    logging.WARNING,
                )
            ...
            if (
                "alongtrack" in self.fqpr.multibeam.raw_ping[0]
                or "x" in self.fqpr.multibeam.raw_ping[0]
            ):
                self.datasets["custom"] = self.fqpr.plot
            else:
                self.print(
                    "No svcorrected/georeferenced variables found in {}".format(
                        self.fqpr.multibeam.converted_pth
                    ),
                    logging.WARNING,
                )
            ...
            if ky:
                ...
                if ky in ["multibeam", "raw navigation", "processed navigation"]:
                    if ky == "multibeam":
                        dset = dset[
                            0
                        ]  # grab the first multibeam dataset (dual head will have two) for the lookup
                        variable_names = [
                            nm
                            for nm in list(dset.variables.keys())
                            if nm in self.variable_translator
                        ]
                        variable_names = [
                            vname
                            for vname in variable_names
                            if vname in kluster_variables.variables_by_key[ky]
                        ]
                    else:
                        variable_names = [
                            nm
                    ...
                    variable_names = [
                        self.variable_translator[nm] for nm in variable_names
                    ]
                elif ky == "custom":
                    variable_names = [
                        "uncertainty",
            ...
            if vari:
                ...
                if dset_var:
                    if ky == "custom":
                        ...
                        self.variable_dimtwo.setText("None")
                        self.data_widget.warning_message.setText(
                            "ERROR: only 2d and 1d vars allowed, found {}:{}d".format(
    ...
    def new_ping_count(self, ping_count: int):
        ...
        if self.datasets:
            ...
            if plottype:
                if (
                    plottype
                    in ["3d scatter, color by depth", "3d scatter, color by sector"]
                    and ping_count > 100
                ):
                    self.data_widget.warning_message.setText(
                        "Warning: 3d scatter will be very slow with greater than 100 pings"
                    )
                elif (
                    plottype
                    in ["2d scatter, color by depth", "2d scatter, color by sector"]
                    and ping_count > 500
                ):
                    self.data_widget.warning_message.setText(
                        "Warning: 2d scatter will be very slow with greater than 500 pings"
                    )
                else:
                    self.data_widget.warning_message.setText("")

    def _plot_variable(
        self, dataset, dataset_name, variable, plottype, sonartype, serialnum
    ):
        """
        ...
        if not self.add_to_current_plot.isChecked() and dataset_name != "custom":
            if isinstance(dataset, list) and len(dataset) > 1:  # dual head
                ...
                fig.suptitle(
                    "{}: {} Plot of {}".format(sonartype[0], plottype, translated_var)
                )
            else:
                fig = plt.figure()
        ...
        for cnt, dset in enumerate(dataset):
            ...
            if (
                variable == "geohash"
            ):  # geohash is a beam-wise string, need an integer for these plot methods to work
            ...
            if plottype == "Line":
                if self.zero_center_plot.isChecked():
                    (data - data.mean()).plot.line(
                        ax=self.recent_plot[cnt], label=identifier
                    )
                else:
                ...
                if self.zero_center_plot.isChecked():
                    (newdata - newdata.mean()).plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (mean)"
                    )
                else:
                    newdata.plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (mean)"
                ...
                if self.zero_center_plot.isChecked():
                    (newdata - newdata.mean()).plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (nadir)"
                    )
                else:
                    newdata.plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (nadir)"
                ...
                if self.zero_center_plot.isChecked():
                    (newdata - newdata.mean()).plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (port outer)"
                    )
                else:
                    newdata.plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (port outer)"
                ...
                for vdbms in data.values:
                    for vcnt, vdbm in enumerate(vdbms[::-1]):
                        if not (
                            np.isnan(vdbm) or vdbm == 0
                        ):  # this is an attempt to ensure the last beam in the ping is valid (some sonar vary in beams, so you get empty values in the square array)
                ...
                if self.zero_center_plot.isChecked():
                    (newdata - newdata.mean()).plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (stbd outer)"
                    )
                else:
                    newdata.plot.line(
                        ax=self.recent_plot[cnt], label=identifier + " (stbd outer)"
                ...
                data.plot.hist(
                    ax=self.recent_plot[cnt], bins=bincount, label=identifier
                )
            elif plottype == "Scatter":
                dset.plot.scatter(
                    "time", variable, ax=self.recent_plot[cnt], label=identifier
                )
            elif plottype == "Image":
        ...
        if not self.add_to_current_plot.isChecked() and plottype not in [
            "Vertical Sample",
            "Horizontal Sample",
            "Backscatter Sample",
        ]:
            ...
            if dataset_name != "custom" and len(self.recent_plot) == 2:
                ...
                plt.title(
                    "{} (SN{}): {} Plot of {}".format(
                        sonartype, serialnum, plottype, translated_var
                    )
                )
            # set always on top
        ...
        if self.datasets:
            ...
            if plottype == "Histogram":
                try:
                    ...
                    self.data_widget.warning_message.setText(
                        'Bins must be an integer, user entered "{}"'.format(
                            self.bincount.text()
                        )
                    )
                    return
            ...
            if ky in ["multibeam", "raw navigation", "processed navigation"]:
                dset_var = self.variable_reverse_lookup[
                    self.variable_dropdown.currentText()
                ]
            else:
            ...
            if ky == "multibeam":
                ...
                self._plot_variable(
                    dsets, dset_name, dset_var, plottype, sonartype, serialnum
                )
            else:
    ...
    def export_variable(self):
        ...
        if self.datasets:
            ...
            defvalue = os.path.join(
                self.fqpr.output_folder, "export_{}_{}.csv".format(ky, dvar)
            )
            if plottype[0:4] == "Line":
    ...
    def refresh_explanation(self):
        ...
        if (
            source == "multibeam"
            or source == "processed navigation"
            or source == "raw navigation"
        ):
            source_expl = "Source = From the {} data, or from the Kluster processed intermediate variables.".format(
                source
            )
            if (
                kluster_variables.variable_reverse_lookup[variable]
                in kluster_variables.variable_descriptions
            ):
                variable_expl = "Variable = {}".format(
                    kluster_variables.variable_descriptions[
                        kluster_variables.variable_reverse_lookup[variable]
                    ]
                )
            else:
            ...
            source_expl = (
                "Source = Custom Kluster plots from all converted and processed data"
            )
            if variable == "georeferenced":
                ...
                variable_expl = (
                    "Variable = Build custom animations from the Kluster processed data"
                )
        ...
        if plottype == "Line":
            ...
            plot_expl = (
                "Plot = Line plot of the average value for this variable for each ping"
            )
        elif plottype == "Line - Nadir":
            plot_expl = (
                "Plot = Line plot of the Nadir beam (center beam) for this variable"
            )
        elif plottype == "Line - Port Outer Beam":
            plot_expl = (
                "Plot = Line plot of the first beam (port outer beam) for this variable"
            )
        elif plottype == "Line - Starboard Outer Beam":
            ...
            plot_expl = (
                "Plot = Each successful calculate TPU process will generate a tvu (total vertical uncertainty) sample image.  This is the average tvu across the ping for the first 1000 pings.\n\n"
            ...
            plot_expl = (
                "Plot = Each successful calculate TPU process will generate a thu (total horizontal uncertainty) sample image.  This is the average thu across the ping for the first 1000 pings.\n\n"
            ...
            plot_expl = (
                "Plot = Each successful backscatter process will generate a backscatter sample image.  This is the different backscatter components for the first ping.\n\n"
